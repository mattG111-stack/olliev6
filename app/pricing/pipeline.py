...
def run(for_sale_df: pd.DataFrame, sold: SoldDataset,
    ...
    for _row_idx, r in df.iterrows():
        ...
        deal_value = None if (is_premium or asking_is_placeholder or asking_is_guide
                              or is_stale or no_floor_highvalue
                              or no_advertised_price) else ollie_value
    ...
    if {"listing_type", "price_numeric"} <= set(result.columns):
        ...
        result.loc[~_lt.isin(["fixed", ""]), "price_numeric"] = None

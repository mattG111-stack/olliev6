...
#
# This used to be enforced by the FILE rather than by code: sold exports covered
# roughly the current year, so every row was recent and nothing had to check.
# Once sold files carry full sale history — the same house sold in 2026, 2020,
# 2012, 2002 and 1994 — an unfiltered median prices a $1.4M house at $335,000.
# Measured in years back from the most recent sale in the dataset, not from
# today, so a dataset that is a few months stale does not quietly empty itself.
COMP_MAX_AGE_YEARS = 3

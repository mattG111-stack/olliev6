...
from ..models import ImportBatch, PropertyForSale, PropertyRent, PropertySold
...
def search_listings(
    ...
    with SessionLocal() as s:
        ...
        if max_price is not None:
            where.append((f"asking under {_money(max_price)}",
                          P.asking_price <= max_price))
        if min_price is not None:
            where.append((f"asking over {_money(min_price)}",
                          P.asking_price >= min_price))
        if underpriced_only:

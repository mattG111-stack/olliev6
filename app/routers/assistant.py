...
from ..models import (DEFAULT_DAILY_LIMIT, LLM_API_KEY, LLM_DAILY_LIMIT,
                      LLM_PROVIDER, LLM_WORKSPACE_ID, AppSetting, AssistantLog,
                      User)
...
def save_key(body: SaveKeyIn, me: User = Depends(require_active),
    ...
    try:
        providers.run(
            ...
            workspace_id=settings_store.workspace_id(db),
        )
...
class QuotaOut(BaseModel):
    ...
    used: int
    remaining: int | None
    # ok | missing | unreadable. "unreadable" means a key IS stored but the
    # secret that encrypted it is not the one running — an admin re-enters it;
    # nobody needs to go and buy anything.
    key_state: str = "ok"
...
class SharedKeyStatus(BaseModel):
    ...
    updated_at: datetime | None = None
    daily_limit: int
    # Not a secret: it names a workspace, it does not open one. Returned so the
    # admin panel can show what is set rather than asking blind.
...
class SharedKeyIn(BaseModel):
    ...
    # Which workspace an identity-linked key acts in. Newer Anthropic keys are
    # tied to an identity rather than standing alone and the API refuses them
    # without it — "anthropic-workspace-id is required when authenticating with
    # an identity-linked API key". Optional, because an older key does not need
    # one and is unharmed by its absence. Sent with the verification call as
    # well as stored, or saving a new key of that kind fails on the check
...
def set_shared_key(body: SharedKeyIn, me: User = Depends(require_admin),
    ...
    # Verified with the workspace it is being SAVED with, not the one already
    # stored: an identity-linked key arrives with its workspace in the same
    # request, and checking it against the old value fails the save that was
    # meant to fix things.
    ws = (body.workspace_id or "").strip() or settings_store.workspace_id(db)
    try:
        providers.run(
            provider=body.provider, api_key=api_key,
            system="Reply with the single word: ok",
            messages=[{"role": "user", "content": "ok"}],
            specs=[], dispatch=lambda *_: "", workspace_id=ws,
        )
    except providers.ProviderError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    ...
    try:
        ...
        # Only when one was given. Sending the form without it must not erase a
        # workspace that is already set and working.
        if (body.workspace_id or "").strip():
            settings_store.put(db, LLM_WORKSPACE_ID,
                               body.workspace_id.strip(), by=me.id)
    except Exception as exc:
        raise _explain(exc) from exc

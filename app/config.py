...
class Settings(BaseSettings):
    ...
    # Which hosts the image proxy is allowed to fetch from — set as a
    # comma-separated list in MEDIA_HOSTS.
    #
    # Deliberately EMPTY in the code. A hard-coded allow-list is a list of who
    # we buy data from, written down in a file, and the whole point of the
    # proxy is that nothing names them. Left unset, the proxy works it out from
    # the listings already in the database, which is the same answer without
    # anybody having to type it here.
    media_hosts: str = ""
    # Serve /docs, /redoc and /openapi.json. OFF unless somebody deliberately
    # turns it on: FastAPI publishes them to anyone who asks, and the spec
    # describes every endpoint the system has, admin routes included. Set
    # API_DOCS=1 on a local machine when you want to browse the API.
    api_docs: str = ""

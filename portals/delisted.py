...
def sweep(db: Session, *, limit: int = MAX_PER_RUN, now: datetime | None = None,
    ...
    # ONLY THE BATCH THE SITE IS SERVING. This used to query every for-sale row
    # in the table, and the table holds every batch ever loaded — the live one,
    # a dozen archived ones, and (now that a short upload no longer replaces the
    # book) a full copy of the book per upload. So most of a pass was spent
    # asking a portal about listings nobody can see, at the cost of the ones
    # they can: with twelve batches on file, roughly one check in twelve landed
    # on a row a customer could click. The budget is the scarce thing here, and
    # it was going almost entirely to rows that do not matter.
    live_batch = (db.query(ImportBatch.id)

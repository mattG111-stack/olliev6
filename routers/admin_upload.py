...
class HistoryRow(BaseModel):
    ...
    rows_rejected: int
    is_active: bool
    # NOT LIVE AND SUPERSEDED ARE DIFFERENT THINGS. With only is_active to go
    # on, the history screen printed "superseded" against a file uploaded four
    # minutes ago and not yet published — which reads as "something replaced
    # this", when nothing has. The status says which of the two it is.
    status: str
...
def trademe_fill(
    ...
    nothing else. See trademe.py for why it must never reach a valuation.
...
#   A LISTING LEAVES THE SITE WHEN ITS ADVERTISEMENT WILL NOT OPEN. That is the
#   rule everywhere else: the daily link check in portals/delisted.py needs the
#   same listing missing three days running, one good fetch clears it, and it
#   never deletes anything — it sets link_dead_at, which is reversible. This
#   button was the one thing in the product that could take a house off the site
#   without anybody having opened its advertisement. So it no longer does.
#   Deleting a for-sale load removes only the listings the link check has already
#   retired; anything still advertised is moved to the remaining load and stays
#   on the site.
...
class DeleteBatchResult(BaseModel):
    ...
    filename: str
    rows_deleted: int
    # Listings that were NOT deleted because their advertisement is still up,
    # and the load they now belong to.
    rows_kept: int = 0
    kept_moved_to_batch_id: int | None = None
    jobs_detached: int
    was_active: bool
...
def delete_batch(batch_id: int, _: User = Depends(require_admin),
    ...
    # Read off the row BEFORE it is deleted. Reaching for batch.region after the
    # delete has been flushed is reading an attribute off a row that no longer
    # exists, and what comes back depends on whether the session happens to have
    # expired it.
    btype, fname, bregion = batch.batch_type, batch.filename, batch.region

    kept = 0
    kept_to = None
    if model is PropertyForSale and batch.status in ("published", "archived"):
        # Preserve only previously published listings. Deleting a staged or
        # superseded upload must never publish its unreviewed rows as a side effect.
        # Still advertised = link_dead_at is NULL. The link check has either
        # never found this one missing, or found it and then found it back.
        # Either way nobody has established that the advertisement is gone, and
        # that is the only thing that takes a house off the site.
        live_q = db.query(PropertyForSale).filter(
            PropertyForSale.import_batch_id == batch_id,
            PropertyForSale.link_dead_at.is_(None))
        kept = live_q.count()
        if kept:
            # They need somewhere to live: import_batch_id is NOT NULL, and a
            # listing with no load is a listing nothing filters on. The most
            # recent other load of the same type and region takes them.
            home = (db.query(ImportBatch)
                    .filter(ImportBatch.batch_type == btype,
                            ImportBatch.region == bregion,
                            ImportBatch.id != batch_id,
                            ImportBatch.status.in_(("published", "archived")))
                    .order_by(ImportBatch.is_active.desc(), desc(ImportBatch.id)).first())
            if home is None:
                raise HTTPException(400, (
                    f"'{fname}' holds {kept:,} listings whose advertisements "
                    "are still up, and there is no other load to move them to. "
                    "Deleting it would take them off the site without anybody "
                    "having found them gone. Load another file first."))
            live_q.update({"import_batch_id": home.id},
    ...
    if was_active:
        # A batch that HAS BEEN LIVE — not merely the most recent one.
        #
        # Taking the highest id regardless of status meant deleting the live
        # load could promote a staged file straight to the site: one nobody had
        # previewed, possibly one that inserted no rows at all. Preview exists
        # precisely so that a file is looked at before customers see it, and
        # this walked around it. Falling back means going BACK — to something
        # that was live before — so only published and archived batches qualify.
    ...
    if kept:
        msg += (f" {kept:,} listings were kept — their advertisements are still "
                "up, so they stay on the site.")

    return DeleteBatchResult(
        deleted_batch_id=batch_id, batch_type=btype, filename=fname,
        rows_deleted=int(rows), rows_kept=int(kept), kept_moved_to_batch_id=kept_to,
        jobs_detached=int(jobs), was_active=was_active,
        now_active_batch_id=now_id, now_active_filename=now_name, message=msg)


# ---------------------------------------------------------------------------
# Underground services: load a network from the operator's own service
# ---------------------------------------------------------------------------
#
#     "the under ground services have in there but not filling yet"
#
# It was not filling because nothing was loaded, and loading it meant a shell on
# the deploy host. That is the same friction that made scraping a property page
# look like the easier option — the right source is only the easier source once
# somebody can actually reach it. So it goes where the rest of the data loading
# already lives: an admin presses a button.
#
# Deliberately synchronous and bounded. A region's network is one fetch of a few
# hundred thousand features, run once and then rarely; a background job would add
# a status screen and a failure mode for something that either works or says why.

class LoadNetwork(BaseModel):
    kind: str                    # stormwater | wastewater | water
    url: str
    region: str = "Auckland"


class LoadNetworkResult(BaseModel):
    ok: bool
    lines: list[str] = []


@router.post("/services/load", response_model=LoadNetworkResult)
def load_service_network(
    payload: LoadNetwork,
    db: Session = Depends(get_db),
    _: User = Depends(require_admin),
) -> LoadNetworkResult:
    """Fetch one network from an ArcGIS FeatureServer and stamp the distances.

    Returns what it found AND what it did not, because a load that silently
    filled nothing looks exactly like one that worked until somebody opens a
    property a week later and the panel is still empty.
    """
    from arcgis import fetch_layer
    from services_network import KINDS, load_pipes, stamp_parcels
    ...
    if not url.lower().startswith("http"):
        raise HTTPException(status_code=400, detail="Give the service URL.")

    got = fetch_layer(url)
    out = LoadNetworkResult(ok=False, lines=list(got.lines()))
    if not got.complete:
        # Refused rather than loaded. A network missing part of a city computes
        # wrong distances everywhere, with nothing on any screen saying which
        # part — worse than the empty panel this is meant to fix.
        out.lines.append("NOT LOADED — a partial network is worse than none.")
        return out

    try:
        rep = load_pipes(db, got.features, kind=kind, region=payload.region)
    ...
    out.lines += rep.lines()
    counts = stamp_parcels(db, region=payload.region)
    db.commit()
    for k, n in counts.items():
        out.lines.append(f"{k}: {n:,} sections have one within reach")
    if not any(counts.values()):
        out.lines.append(
            "No section distances were written — the pipes loaded, but no "
            "parcels are loaded for this region to measure from. The property "
            "map will still draw the mains.")
    out.ok = True
    return out

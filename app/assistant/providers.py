...
def _run_anthropic(
    ...
    for i in range(MAX_ITERATIONS):
        ...
        # start with a second left and run for another ninety.
        budget = _left(started)
        if budget < MIN_CALL_SECONDS:
            return Result(text=_out_of_time(used), tools_used=used,
                          iterations=i, queries=queries)
        try:
            resp = _timed(client, min(REQUEST_TIMEOUT, budget)).messages.create(
        ...
        if _left(started) < MIN_CALL_SECONDS:
            return Result(text=_out_of_time(used), tools_used=used,
    ...
    for i in range(MAX_ITERATIONS):
        ...
        if budget < MIN_CALL_SECONDS:
            return Result(text=_out_of_time(used), tools_used=used,
                          iterations=i, queries=queries)
        try:
            resp = _timed(client, min(REQUEST_TIMEOUT, budget)).chat.completions.create(

...
def _features(source: str, label: str) -> list[dict]:
    ...
    if source.lower().endswith((".json", ".geojson")):
        from parcels import load_geojson
    ...
    if source.lower().endswith((".xlsx", ".xlsm")):
        ...
        from services_network import rows_to_features
...
def _autowire(service_urls: list[str]) -> int:
    ...
    from arcgis import find_layers, probe
...
def _probe(url: str) -> int:
    ...
    from arcgis import list_layers, probe
...
def main(argv: list[str]) -> int:
    ...
    with SessionLocal() as db:
        if what == "pipes":
            ...
            from services_network import KINDS, load_pipes
        ...
        if what == "parcels":
            ...
            from parcels import load_parcels
        ...
        if what == "zones":
            ...
            from parcels import apply_zones
        ...
        if what == "distances":
            from services_network import stamp_parcels

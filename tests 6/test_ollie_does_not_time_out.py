...
import main, settings_store
from assistant import providers
from db import get_db
from models import AssistantLog, User, UserRole, UserStatus
from routers import assistant as A
from security import current_user, require_active
...
# ---- why Ollie is locked ----------------------------------------------------
def test_a_key_that_cannot_be_read_is_not_reported_as_a_missing_one(db_session, monkeypatch):
    """THE ONE THAT LOCKED THE BOX.

    A key encrypted under a different secret than the one now running — which is
    what a redeploy with a fresh JWT_SECRET produces — is intact but unreadable.
    shared_key() correctly refuses it, so the page said "Add an API key in
    Settings first" and greyed the box: an instruction to go and buy something,
    sent to the one person who cannot fix it. The administrator simply re-enters
    the key they already have.
    """
    import settings_store as S
    from models import LLM_API_KEY, LLM_PROVIDER

    S.put(db_session, LLM_PROVIDER, "anthropic")
    S.put(db_session, LLM_API_KEY, "not-decryptable-under-any-secret")
    assert S.shared_key_state(db_session) == "unreadable"
    assert S.shared_key(db_session) == (None, None)


def test_no_key_at_all_says_so(db_session):
    import settings_store as S
    assert S.shared_key_state(db_session) == "missing"


def test_a_working_key_reads_as_ok(db_session):
    import settings_store as S
    from assistant import keys
    from models import LLM_API_KEY, LLM_PROVIDER

    S.put(db_session, LLM_PROVIDER, "anthropic")
    S.put(db_session, LLM_API_KEY, keys.encrypt("sk-ant-whatever"))
    assert S.shared_key_state(db_session) == "ok"


def test_the_quota_carries_the_reason(db_session, me):
    """The page needs to know WHICH problem it is, not merely that there is one."""
    import settings_store as S
    out = S.quota_for(db_session, me)
    assert "key_state" in out
    assert out["key_state"] in {"ok", "missing", "unreadable"}


# ---- identity-linked keys ---------------------------------------------------
# A newer Anthropic key is tied to an identity rather than standing alone, and
# the API refuses it outright unless the request names the workspace it acts in:
#
...
#
# Nothing here ever sent that header, because no key had ever needed one. The
# key is the newer kind, not the code being wrong — but the code has to be able
# to carry it, and the awkward part is WHEN: the workspace arrives in the same
# request as the key it belongs to.
def test_the_workspace_header_is_sent_when_there_is_one(monkeypatch):
    import assistant.providers as P
    ...
    class _Stub:
        ...
        def create(self, **_kw):
            raise RuntimeError("stop here — the client is what is being checked")
    ...
    with pytest.raises(P.ProviderError):
        P._run_anthropic(api_key="k", system="s", messages=[], specs=[],
                         dispatch=lambda *_: "", workspace_id="wrkspc_123")
    assert seen.get("default_headers") == {"anthropic-workspace-id": "wrkspc_123"}


def test_an_older_key_sends_no_such_header(monkeypatch):
    """Older keys do not need it, and inventing an empty one is a header that
    means something different from not sending one."""
    import sys, types
...
def test_the_environment_can_unblock_a_deployment(db_session, monkeypatch):
    """When the assistant is down, the admin panel is how you would normally fix
    it — so there has to be a way that does not go through the admin panel."""
    import settings_store as S

    monkeypatch.setenv("ANTHROPIC_WORKSPACE_ID", "wrkspc_from_env")
    assert S.workspace_id(db_session) == "wrkspc_from_env"


def test_the_environment_wins_over_the_stored_value(db_session, monkeypatch):
    import settings_store as S
    from models import LLM_WORKSPACE_ID

    S.put(db_session, LLM_WORKSPACE_ID, "wrkspc_stored")
    assert S.workspace_id(db_session) == "wrkspc_stored"
    monkeypatch.setenv("ANTHROPIC_WORKSPACE_ID", "wrkspc_from_env")
    assert S.workspace_id(db_session) == "wrkspc_from_env"


def test_no_workspace_anywhere_is_none_not_empty_string(db_session):
    """An empty string would be sent as a header meaning "workspace: nothing",
    which is not the same as not sending one."""
    import settings_store as S

    assert S.workspace_id(db_session) is None


# ---- the admin page has to be able to SEND it -------------------------------
#
#     "does the Claude api add work"
#
# The backend took a workspace id, verified the key with it and stored it. The
# admin page never sent one — so an identity-linked key could not be entered at
# all: the save runs a live call first, that call was made without the header,
# Anthropic refused it, and the key was never written. The fix existed and was
# unreachable, which is the same as not existing.
def test_the_admin_page_sends_the_workspace_with_the_key():
    from pathlib import Path

    page = Path("../ollie-v5-frontend/app/admin/assistant/page.tsx")
    if not page.exists():                       # backend checked out on its own
        import pytest
        pytest.skip("frontend not present beside the backend")
    src = page.read_text()
    body = src[src.index("/api/admin/assistant/key"):][:1400]
    assert "workspace_id" in body, "the key form still cannot carry a workspace"
    assert "Workspace ID" in src, "and there is no field to type one into"

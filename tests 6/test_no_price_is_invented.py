...
def test_the_buy_price_is_our_estimate_and_it_is_shown(db_session):
    """    "use our estimate buy price"

    I had withheld it, reasoning that "what you can pay" is capped below the
    asking price and a cap against nothing is invented. That was wrong in a way
    the screenshots made obvious: the number did not stop existing, it only
    stopped being visible. The comp engine still produced one from what
    comparable land sells for, the subdivision still ran on it, and the card
    still published "+19 lots, $4.69M net gain" — while the detail page, which
    reads the STORED buy price, found nothing there and drew a dash through
    every line of the development maths underneath that headline.

    One number or none. It is our estimate either way, and it is labelled as one
    wherever it appears, so showing it claims nothing about what a vendor asks.
    """
    stored = _upload(db_session, [_listing()])["718-remuera-road"]
    assert stored.asking_price is None, "still no invented asking price"
    assert stored.buy_price, "our own estimate is missing"
    assert stored.margin is None, "and it still buys no deal signal"
...
def test_every_budget_on_the_site_means_the_same_thing():
    ...
    from models import BUDGET_PRICE
    ...
    pattern = re.compile(
        ...
        re.IGNORECASE)
    offenders = []
    from source_tree import app_files
    ...
    for path in ("routers/properties.py", "routers/wishlists.py",
                 "routers/preferences.py", "assistant/tools.py"):
...
def test_the_listing_still_reaches_the_site_without_a_price(db_session):
    ...
    from release import _hold_reason, hold_flagged_rows
...
def test_the_whole_book_is_not_held_for_want_of_a_price(db_session):
    ...
    """
    from release import BELOW_MARGIN_REASON, _hold_reason
...
def test_a_buyer_with_a_budget_still_finds_it(db_session):
    ...
    """
    from routers.properties import _active_batch, _filtered_query

...
def test_a_call_cannot_outlive_the_budget_it_was_started_with():
    ...
    src = __import__("pathlib").Path("assistant/providers.py").read_text()
    # `per_call` is REQUEST_TIMEOUT whenever there IS a deadline, and the
    # generous unbounded ceiling only when there is not — so this is the same
    # guarantee the literal used to spell out, written once per provider.
    assert src.count("_timed(client, min(per_call, budget))") >= 2, (
        "calls are no longer bounded by what is left of the deadline")


def test_a_call_is_bounded_by_what_is_left_not_by_the_full_per_call_timeout(
        fake_anthropic, monkeypatch):
    """The same guarantee, behaviourally. With 12 seconds left on the budget, a
    call must be given 12 — not the 40 the constructor was built with, which is
    how the real ceiling became DEADLINE + REQUEST_TIMEOUT."""
    given: list[float] = []
    monkeypatch.setattr(providers, "_timed",
                        lambda client, seconds: given.append(seconds) or client)
    monkeypatch.setattr(providers, "DEADLINE", 12.0)
    with pytest.raises(providers.ProviderError):
        providers._run_anthropic(api_key="k", system="s", messages=[], specs=[],
                                 dispatch=lambda *_: "")
    assert given and given[0] <= 12.0, given
    assert given[0] < providers.REQUEST_TIMEOUT


def test_with_no_deadline_a_call_still_gets_a_ceiling(fake_anthropic, monkeypatch):
    """Unbounded overall does NOT mean unbounded per call — a socket that has
    silently died must not hang the worker thread for ever."""
    given: list[float] = []
    monkeypatch.setattr(providers, "_timed",
                        lambda client, seconds: given.append(seconds) or client)
    with pytest.raises(providers.ProviderError):
        providers._run_anthropic(api_key="k", system="s", messages=[], specs=[],
                                 dispatch=lambda *_: "", deadline=None)
    assert given == [providers.CALL_TIMEOUT_UNBOUNDED]
    assert _Stub.seen.get("timeout") == providers.CALL_TIMEOUT_UNBOUNDED


def test_the_deadline_is_read_fresh_rather_than_frozen_at_import(fake_anthropic,
                                                                 monkeypatch):
    """A default argument is evaluated when the function is DEFINED. Written as
    `deadline=DEADLINE`, the module's value would be frozen at import and every
    later change to it — including a test's — would silently do nothing."""
    monkeypatch.setattr(providers, "DEADLINE", -1.0)
    assert providers._left(0.0) < 0
    result = providers._run_anthropic(api_key="k", system="s", messages=[],
                                      specs=[], dispatch=lambda *_: "")
    assert "longer than I'm allowed" in result.text
...
def test_the_tool_dispatch_is_counted_too():
    ...
    assert src.count("if _left(started, deadline) < MIN_CALL_SECONDS:") >= 2

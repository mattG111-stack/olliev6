...
_ADD_COLUMNS = (
    ...
    # Trade Me's own figure, filled from their sales export (trademe.py).
...
def _backfill_can_subdivide() -> int:
    """Answer the new column for every row that predates it.

    can_subdivide arrives by ALTER TABLE, so every listing already live reads
    NULL until something re-prices it — and the subdivision filter reads this
    column. Deploy alone would therefore empty the Subdividable page and leave
    it empty until the next upload, which is not a plausible thing for anyone
    to conclude except "it broke".

    It does not need the model re-run. Feasibility is already recorded: the
    pipeline stores max_addl_lots for every site it found splittable, including
    the ones it then judged unprofitable, and leaves it NULL for the rest. So
    the fact is in the database already, under a different name, and one
    UPDATE recovers it.

    Idempotent — only ever touches rows that are still NULL — and never raises.
    """
    try:
        with engine.begin() as conn:
            res = conn.execute(text(
                "UPDATE properties_for_sale SET can_subdivide = "
                "  (CASE WHEN (max_addl_lots IS NOT NULL AND max_addl_lots >= 1) "
                # Worth subdividing implies splittable — the profit is computed
                # on the feasible path and nowhere else. Stated here as well so
                # the invariant cannot fail on a row whose lot count went
                # missing for some other reason.
                "         OR is_subdividable = TRUE "
                "        THEN TRUE ELSE FALSE END) "
                "WHERE can_subdivide IS NULL"))
            return int(res.rowcount or 0)
    except Exception as e:                                  # noqa: BLE001
        print(f"[db_bootstrap] can_subdivide backfill skipped: "
...
def main() -> int:
    ...
    backfilled = _backfill_can_subdivide()
    if backfilled:
        print(f"[db_bootstrap] can_subdivide backfilled on {backfilled} row(s) "
              f"from their stored lot counts", flush=True)

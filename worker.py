...
def build_jobs() -> list[Job]:
    ...
    return [
        ...
        # has usually sold — but it may have been withdrawn, expired or
        # relisted, so this records only that the link is gone and never that
        # anything sold. See app/portals/delisted.py for why that distinction
        ...
        # address and a date, which is direct evidence rather than an inference
        # from silence — and needs nothing external, so it runs regardless of
        # whether the portal sweeps are switched on.
        Job("sold-file sweep", 24 * 60 * 60, sold_sweep_run),

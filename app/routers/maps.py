...
class MapConfig(BaseModel):
    """What the browser needs to draw a map. Only the resolved provider's key is
    returned — sending both would hand out a credential the page has no use for.

    Two independent choices live here. `provider` is the AERIAL: the photo behind
    a property. `street_provider` is the STREET basemap every list and trends map
    draws on, which used to be CARTO and now needs a key CARTO did not used to
    require. They resolve separately because an admin can perfectly well want
    Google photography over a LINZ street map.
    """
    provider: str
    google_key: str | None = None
    linz_key: str | None = None
    # "linz" when a LINZ key is configured, "esri" otherwise. Esri needs no key
    # at all, so there is always something to draw.
    street_provider: str = "esri"
...
def map_config(db: Session = Depends(get_db)) -> MapConfig:
    ...
    # The street map takes LINZ whenever there is a LINZ key, whatever the
    # aerial is set to: it is the best map of New Zealand there is, it is free
    # with the key, and the alternative needs no key but is a world dataset.
    street = "linz" if linz else "esri"
    return MapConfig(
        provider=provider,
        google_key=google if provider == "google" else None,
        linz_key=linz if provider == "linz" else None,
        street_provider=street,
        street_key=linz if street == "linz" else None,
    )

...
def test_the_history_api_returns_the_reason(db_session):
    from routers.admin_upload import HistoryRow
...
def test_the_summary_reports_every_sold_row_not_just_this_upload(db_session):
    ...
    from release import staged_summary
...
def test_the_api_returns_the_total(db_session):
    from routers.release import StagedOut
...
def test_the_enrich_result_says_how_many_were_blocked():
    ...
    # The worker is a pass plus a wrapper that re-runs it; the completion is
    # written by the pass, so read both halves rather than only the wrapper.
    src = (inspect.getsource(staged_stages._enrich_pass)
           + inspect.getsource(staged_stages.run_enrich_job))

...
from portals import listings as NL
...
def test_three_selected_sources_are_configured_for_direct_sweeps():
    assert set(NL.NEW_LISTING_SOURCES) == {'trademe','oneroof','homes'}
    assert set(NL.SOLD_SOURCES) == {'trademe','oneroof','homes'}
...
def test_a_sweep_records_what_it_found(db_session):
    ...
    got = NL.sweep(db_session, sources=("oneroof", "realestate"), runner=run)
...
def test_with_no_batch_at_all_nothing_can_be_approved(db_session):
    """There has to be somewhere to put it.

    This used to say "live batch", because an approval always went to the live
    one. It now joins the STAGED batch when there is one — a portal find is the
    least verified row in the system and was the only thing skipping the review
    every weekly-file row goes through — so the refusal is no longer about the
    live batch specifically. The behaviour is unchanged: no batch, no approval,
    ...
    row = NL.pending(db_session)[0]
    ok, why = NL.approve(db_session, row.id, reprice=lambda db, pid: None)
    assert not ok and "no batch" in why
...
def test_a_failing_sweep_does_not_raise(monkeypatch):
    ...
    monkeypatch.setattr("portals.listings.sweep", boom)
...
def test_sales_are_swept_daily():
    from portals import daily
    from worker import build_jobs
    assert daily.SOLD_EVERY_DAYS == 1
    sold_job=next(job for job in build_jobs() if job.name == "sold sweep")
    assert sold_job.every == 24 * 60 * 60
...
def test_a_failing_sold_sweep_does_not_raise(monkeypatch):
    ...
    monkeypatch.setattr("portals.listings.sweep_sold", boom)
...
def test_retired_apify_endpoints_do_not_read_or_change_saved_credentials(db_session):
    from fastapi import HTTPException
    from models import APIFY_TOKEN
    from routers import release
    from settings_store import get, put
    put(db_session, APIFY_TOKEN, 'existing-encrypted-value')
    admin = _admin(db_session)
    with pytest.raises(HTTPException) as err: release.save_apify_token(release.ApifyTokenIn(token='replacement'), admin=admin, db=db_session)
    assert err.value.status_code==410
    with pytest.raises(HTTPException) as err: release.apify_status(admin=admin, db=db_session)
    assert err.value.status_code==410
    assert get(db_session, APIFY_TOKEN)=='existing-encrypted-value'
...
def test_a_sweep_with_no_token_says_so(monkeypatch, db_session):
    ...
    monkeypatch.setattr("portals.direct.configured", lambda db=None: False)
    admin = _admin(db_session)
    with pytest.raises(HTTPException) as e:
        release.sweep_new_listings(admin=admin, db=db_session)
    assert e.value.status_code == 400
    assert "scraper" in e.value.detail.lower()
...
def test_the_sweep_returns_a_job_and_does_not_do_the_work(monkeypatch, db_session):
    ...
    monkeypatch.setattr("portals.direct.configured", lambda db=None: True)
    started = []
...
def test_the_job_records_what_the_sweep_found(monkeypatch, db_session):
    ...
    job = create_stage_job(db_session, stage="listings", batch_id=None,
                           region="Auckland", uploaded_by_id=None)
    monkeypatch.setattr("portals.listings.sweep",
...
def test_a_portal_listing_can_now_be_assessed_for_subdivision(db_session):
    ...
    from pricing.subdivision import compute

            ...
            mover_rows = db.execute(text(
                ...
                WITH a AS (SELECT slug_id, id, address, suburb, asking_price
                           ...
                           WHERE import_batch_id = :prev AND slug_id IS NOT NULL
                             ...
                             -- COALESCE, because a NULL cv or last-sale makes
                             -- the comparison NULL, NOT NULL is NULL, and the
                             -- WHERE drops the row. Three-valued logic turned a
                             -- placeholder filter into a filter that also threw
                             -- out every listing with no council record.

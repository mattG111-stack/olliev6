...
def _row_to_input(p: PropertyForSale) -> dict:
    ...
    return {
        ...
        # The feed's own word for how it sells, which is what decides whether
        # this listing is allowed an asking price at all. Rebuilding the price
        # text from listing_type (above) re-derives the CONCLUSION from itself,
        # so a listing filed wrongly once could never be put right by re-pricing
        # it. Rows loaded before this column was kept have None here and fall
        # back to that older reading, exactly as they did before.
        ...
        # The vendor's own last advertised price, when they have stopped
        # naming one. Carried onto the row by prior_price.carry_forward_prices
        # before this runs.
        "prior_asking_price": p.prior_asking_price,
        "pv_estimate_mid": p.pv_estimate_mid,
        "homes_valuation": p.homes_valuation,
        # A lot count set by hand during the staged review. It has to travel
        # into the pricing run, or the next re-price quietly replaces the
        # operator's number with the model's — and a correction that does not
        # survive the step that comes after it is not a correction.
...
def _apply_outputs(rec: PropertyForSale, row: dict) -> None:
    ...
    # Cleared as well as set — this must always describe the CURRENT pricing
    # run, never a reason left over from the last one.
    rec.asking_basis = _to_str(row.get("asking_basis"))
    # And when the run priced this listing off the vendor's last advertised
    # figure, that figure has to land on the row.
    #
    # This is what made the carry-forward rescue nothing. The load writes the
    # carried price (ingest reads price_numeric, which the pipeline sets to the
    # derived figure) — but the load runs BEFORE carry_forward_prices, so at
    # that point there is no prior price to derive from. The price stage runs it
    # the right way round, and reprice was the one path that never wrote the
    # asking price back. So the row ended up saying "last advertised, less 3%"
    # in one column and showing the council valuation in the next, with a margin
    # computed from a third number that appeared nowhere.
    #
    # Only on the derived path. price_numeric is deliberately blanked for
    # auctions and by-negotiation listings, and copying that over a stored
    # asking price would erase it.
    if rec.asking_basis == DERIVED_BASIS:
        _carried = _to_float(row.get("asking_used"))
        if _carried is None:
        ...
        #
        # A listing the run has just concluded sells by auction, tender or
        # negotiation has no asking price. If one is sitting on the row from an
        # earlier load — the whole 718 Remuera Road fault, where the council
        # valuation was stored as though the vendor had named it — leaving it
        # there means the number survives every re-price and stays on the site
        # for as long as the listing does. It is cleared here, so an operator
        # re-running the pricing actually puts the row right.
        #
        # Guarded on the derived branch above, which owns the case where we DO
        # have a real figure to show for a listing with no advertised price.
        rec.asking_price = None
    rec.deal_block_reason = _to_str(row.get("deal_block_reason"))
...
def reprice_batch(db: Session, batch_id: int, *, region: str = "Auckland",
    ...
    `on_chunk(done, total)` is called after each chunk, if given. The stage that
    runs this is watched by a liveness check that reads a heartbeat, and a
    single call lasting several minutes with nothing written in between looks
    exactly like a hung process — which would get the run declared dead and its
    lock released underneath it. This is how it says it is still working.
    """
    res = RepriceResult()
    ...
    # Loaded ONCE, outside the chunk loop. Inside it, every chunk would reload
    # and re-deserialise the same model, and — worse — a retrain finishing
    # mid-run would price the back half of the batch with a different valuation
    # from the front half.
    from .ml.store import live_model
    model = live_model(db)
    if model is not None:
        print(f"  [reprice] using the trained valuation "
              f"(fitted on {model.n_train:,} sales)")
    ...
    for start in range(0, len(ids), max(1, chunk)):
        ...
        enriched = run_pipeline(df, sold, rents, model=model).reset_index(drop=True)
        res.rows += len(recs)
        ...
        if on_chunk is not None:
            try:
                on_chunk(res.rows, len(ids))
            except Exception:                  # noqa: BLE001
                # A heartbeat that throws must not lose a re-priced batch. The
                # work is already committed; the worst case without it is that
                # the run is mistaken for a dead one.
                pass
...
def reprice_one(db: Session, listing_id: int, *, region: str = "Auckland") -> PropertyForSale | None:
    ...
    from .ml.store import live_model
    enriched = run_pipeline(df, SoldDataset(sold_df), _rent_rates(db, region),
                            model=live_model(db)).reset_index(drop=True)

...
def workspace_id(db: Session) -> str | None:
    """Which workspace an identity-linked key acts in, if one is set.

    The environment wins over the stored setting, so a deployment can be
    unblocked by setting ANTHROPIC_WORKSPACE_ID and restarting — no database
    write, no admin panel round trip, which matters when the assistant is down
    and the admin panel is how you would normally fix it.
    """
    import os
    ...
    if env:
        return env
    return (get(db, LLM_WORKSPACE_ID) or "").strip() or None


def shared_key_state(db: Session) -> str:
    """Why the account-wide key is or is not usable: ok | missing | unreadable.

    "No key" and "a key that cannot be read" are different problems with
    different owners, and the UI was telling everyone the first one. A key
    encrypted under a different secret than the one now running — a redeploy
    with a fresh JWT_SECRET does exactly this — is intact but unreadable, and
    the page said "Add an API key in Settings first", sending a customer to go
    and buy a key when the administrator simply needs to paste theirs back in.
    """
    provider = (get(db, LLM_PROVIDER) or "").strip() or None
    stored = get(db, LLM_API_KEY)
    if not provider or not stored:
        return "missing"
    return "ok" if keys.decrypt(stored) else "unreadable"
...
def _day_start() -> datetime:
    """Midnight this morning in New Zealand, expressed in UTC.

    Returned NAIVE UTC on purpose. created_at is written by the database's own
    now(), which is UTC, and a naive UTC value compares correctly against that on
    both engines. An aware NZ value does not: SQLite stores timestamps as text
    and compares them as text, so "2026-08-16 00:00:00+12:00" sorts after a row
    written at "2026-08-15 19:51:00" even though that row is four hours LATER in
    real time. The allowance then reads as unused for the first twelve hours of
    every New Zealand day.

    This only showed up once the clock crossed into a New Zealand date ahead of
    the UTC one — the tests passed all morning and failed at 07:51 NZ, which is
    exactly the window where the two dates differ.
    """
    now_nz = datetime.now(NZ_TZ)
    nz_midnight = datetime.combine(now_nz.date(), time.min, tzinfo=NZ_TZ)
    return nz_midnight.astimezone(timezone.utc).replace(tzinfo=None)
...
def used_today(db: Session, user_id: int | None) -> int:
    ...
    ANSWERS, not rows. Since asking became a job, the row is written the moment
    the question is ASKED and finished later by a worker — so a row exists, with
    ok still True, for a question that has not been answered yet and, if its
    container died mid-answer, never will be. Counting rows charged people for
    those. `answer IS NOT NULL` is the same rule this docstring always claimed:
    it costs an allowance when something came back. It changes nothing for the
    rows written the old way, which only ever existed once an answer did.
    """
    if user_id is None:
        return 0
    try:
...
def quota_for(db: Session, user: User) -> dict:
    ...
    return {"shared": True, "limit": limit, "used": used,
            ...
            # So the page can say WHICH problem it is rather than guessing at
            # the more embarrassing one.
            "key_state": shared_key_state(db)}

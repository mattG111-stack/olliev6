...
def requested_limit(question):
    ...
    # A result count and a bedroom count can coexist: "three 3-bedroom houses".
    # Remove only the room descriptor, retaining the separate result count.
    question = re.sub(
...
def is_search_followup(question):
    if re.search(r'\b(new search|start over|forget (?:that|the|my)|show all|all matches)\b', question, re.I):
        return False
    return bool(re.search(r'\b(keep everything|keep my|same|remain|retained|drop my budget|raise my budget|make it cheaper|filter tighter|refine|compare these)\b', question, re.I))


def shortlist_context(question, history):
    """Carry disclosure context only within a continuing search, not past resets."""
    if not is_search_followup(question):
...
def render_shortlist(answer, evidence, limit, question='', previous_ids=None, preference_context=None):
    ...
    if re.search(r'\b(renovat\w*|condition|garage|garaging|outdoor|garden|yard)\b', preference_context or question, re.I):

...
from source_tree import ROOT, app_files
...
# Our own top-level names. This used to be the single word "app" — every import
# of ours started with it, so anything that did not was somebody else's package.
# The application is the repository root now, so `from models import Listing` is
# first-party and looks exactly like `from pandas import DataFrame` does. The
# only thing that separates them is whether a file of that name is here, so that
# is what this asks. Derived, not typed out: a new module of ours must not need
# a second edit here to stop being reported as an undeclared dependency.
OURS = {f.split("/")[0].removesuffix(".py") for f in app_files()}


def _imported() -> dict[str, list[str]]:
    """Third-party top-level modules our app imports, and where from."""
    found: dict[str, list[str]] = {}
    for path in [ROOT / f for f in app_files()]:
        try:
        ...
        for node in ast.walk(tree):
            ...
            for name in names:
                top = name.split(".")[0]
                if top in sys.stdlib_module_names or top in OURS:
                    continue
...
def test_httpx_is_declared_in_its_own_right():
    ...
    assert "httpx" in _declared(), (
        "httpx is imported directly by notify.py and five other modules. "

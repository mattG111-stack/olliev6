...
class User(Base):
    ...
    # Asked inside Ollie the first time they open him, and re-checked every
    # fortnight. Deliberately on the user rather than in wish_lists: a wish list
    # is something a customer has to remember to create, and most never do, so
    # we learned nothing about them and personalised nothing. See app/
    # preferences.py, which owns the vocabulary, the clock and the validation —
    # nothing here should be read or written without going through it.
    # Lists are JSON text, so a value containing a comma survives; the counts
    # are aggregated in Python, over hundreds of users, not millions.
    hunt_goals: Mapped[str | None] = mapped_column(Text)
    hunt_suburbs: Mapped[str | None] = mapped_column(Text)
    hunt_districts: Mapped[str | None] = mapped_column(Text)
    ...
    # Last time they confirmed or changed it. NULL means we have never asked,
    # which is what puts the first-run questions in front of them.
    preferences_reviewed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # "Ask me again in a month" — pushes the next ask out past the fortnight.
    preferences_snoozed_until: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class AssistantLog(Base):
    ...
    # A question used to be answered inside the request that asked it, so this
    # row was only ever written once the answer existed. Now the row is created
    # the moment the question is asked and finished by a worker thread, which
    # is what lets an answer take as long as it needs: nothing is holding a
    # socket open waiting for it.
    #
    # Every column below is NULLABLE and every reader treats None as "an old
    # row, answered the old way" — ensure_columns() adds columns nullable
    # whatever the model says, and there are thousands of existing rows.
    status: Mapped[str | None] = mapped_column(String(16), default="done")
    # 0-100. Real milestones, not a fabricated bar — see assistant.progress_for.
    progress_pct: Mapped[int | None] = mapped_column(Integer, default=0)
    phase: Mapped[str | None] = mapped_column(String(80))     # shown to the asker
    phase_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    finished_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    queries: Mapped[str | None] = mapped_column(Text)         # JSON list of SQL
...
class ImportBatch(Base):
    ...
    # Three-stage publish lifecycle:
    #
    #   staged → preview → published → archived
    #
    # STAGED is loaded and priced but not live. PREVIEW is the second look: the
    # batch is finished and inspected as it will appear, and the public site is
    # still serving the PREVIOUS load — is_active is still false, so nothing a
    # customer sees changes. PUBLISHED flips is_active and archives the old one.
    #
    # The middle step exists because "publish" used to be the only door and it
    # was one-way: the moment it was pressed, whatever was wrong with the batch
    # was wrong in front of customers. Now a listing that is not a real house can
    # be removed while the site is still showing last week.
...
class IngestJob(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    started_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # The last time this job did ANYTHING — a heartbeat, written on every
    # progress update.
    #
    # "Is this job still alive" used to be answered with started_at, which is
    # stamped once and never moves. That does not measure whether a job is
    # alive; it measures how long ago it began. A CoreLogic enrich over a full
    # batch runs for well over an hour, so twenty minutes in, a run that was
    # working perfectly was declared abandoned, its row flipped to failed, and
    # a red error appeared on a screen whose job was still going. Worse, the
    # lock came off, so a second enrich could start on top of the first — two
    # heavy workers in one container, which is how it gets OOM-killed.
    #
    # A heartbeat answers the question actually being asked. A job that is
    # working is never declared dead however long it takes; a job that has
    # genuinely stopped is spotted in minutes rather than never.
...
class PropertyForSale(_PropertyMixin, Base):
    ...
    # These live on the SALE table, not only on the staging one, because this is
    # the row a customer clicks. A dead link on a staged row nobody can see is a
    # curiosity; a dead link here is a customer landing on a 404 from our site.
    link_checked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    link_gone_count: Mapped[int | None] = mapped_column(Integer, default=0)
    # Set when the link is confirmed gone. Hides the row from every customer
    # list; cleared the moment the link answers again.
    link_dead_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # THE HOUSE APPEARS IN THE SOLD FILE. A different fact from the one above,
    # and a much stronger one.
    #
    #     "a lot of houses have sold in the last few weeks and we still have
    #      them on the site shouldnt they be gone ?"
    #
    # They should, and the link check was never going to catch them. It fires on
    # 404 and 410, and a New Zealand portal does not delete a page when a house
    # sells — it leaves it up with a SOLD banner on it, answering 200 for ever.
    # So the one signal we were watching is the one signal these houses never
    # send, and a sold house sits on the site until the batch is replaced.
    #
    # Meanwhile the sale arrives in the weekly sold file with an address, a
    # price and a date on it. That is direct evidence, not an inference from
    # silence. Matched here, kept as its own column because "sold" and "the
    # advertisement is gone" are different claims that need different wording on
    # screen and behave differently on a re-check: a link can come back, a sale
    # cannot be un-sold.
    sold_seen_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True)
    # The sale date the match was made on, as the sold file spells it, so an
    # operator can check the match rather than take it on trust.
    sold_record_date: Mapped[str | None] = mapped_column(String(32))
    # THE DAY WE FIRST SAW THIS HOUSE ADVERTISED.
    #
    #     "we should have how many days to sell"
    #
    # We should, and the sold file cannot give it to us: 89,262 of its 116,959
    # rows are reconstructed out of sale-history JSON, and a history entry is a
    # price and a date. There is no campaign behind it to measure.
    #
    # So it is measured here instead, from what we watch ourselves. This is set
    # once, the first week a listing appears, and carried forward untouched
    # every week after — so it survives the house being re-listed into a new
    # batch, which is the whole reason it cannot simply be the row's created_at.
    # Paired with link_dead_at (the day the advertisement stopped answering),
    # the difference is a days-to-sell we observed rather than one we were
    # handed.
    #
    # It starts thin and fills in: nothing has a departure date until listings
    # begin dropping off, so the figure is empty at first and becomes real over
    # the following weeks. That is honest, and better than the alternative,
    # which was a number copied off a different sale.
    first_seen_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), index=True)
    link_last_result: Mapped[str | None] = mapped_column(String(16))

    @property
    def off_market(self) -> bool:
        """The advertisement has come off the portal.

        Derived rather than stored a second time, so it cannot disagree with the
        column it is derived from. Serialisers read it through from_attributes,
        which means every payload that carries a listing carries this — a page
        reached by bookmark or shared link can say the listing is gone instead
        of offering a button through to a page that is not there.
        """
        return self.link_dead_at is not None

    @property
    def days_advertised(self) -> int | None:
        """How long this listing has been on the market, in whole days.

        Runs to the day the advertisement stopped answering where there is one,
        and to today where there is not — so it reads as "sold in 34 days" for a
        finished listing and "advertised 34 days so far" for a live one.

        Derived, like off_market above, so the exports and the site cannot
        disagree about it. Both the workbook and the CSV read this one property
        rather than each working it out; the last time two exports kept their
        own copy of the same idea they drifted, and nobody noticed until the
        columns were compared by hand.
        """
        if self.first_seen_at is None:
            return None
        start = self.first_seen_at
        end = self.link_dead_at or datetime.now(timezone.utc)
    ...
    # How the vendor is selling it, exactly as the feed said — "auction",
    # "deadline treaty", "by negotiation". This one column decides whether
    # asking_price is allowed to hold anything at all, and until now it was
    # read during the load and then thrown away. That is why the fault was
    # unrepairable in place: a listing stored with the wrong listing_type could
    # not be re-classified, because the evidence had not been kept. Stored raw
    # rather than as the classification so a method we do not recognise yet can
    # be found and handled later.
    ...
    # How many lots a PERSON says this site takes, overriding the model.
    #
    # The model reads the lot count off the land area and the zone minimum, and
    # on a site that has already been worked — a wide frontage, a corner, a rear
    # access strip — it takes the cautious reading. The profit is then a floor,
    # not an estimate, and somebody who knows the street can say better.
    #
    # It is a STORED override rather than a what-if, because the point is to fix
    # a listing while it is staged and have the corrected figures be the ones
    # that go live. The scenario calculator on the property page recomputes and
    # saves nothing; this is what survives the publish.
    #
    # NULL means "use the model" — which is different from any number, including
    # a number that happens to equal the model's. Clearing it must restore the
    # model's own reading rather than freezing today's.
    lots_override: Mapped[float | None] = mapped_column(Float)
    ...
    # 32 was exactly the length of "Retain house + sell new sections" — no
    # headroom at all, and the demolish wording is 38. Postgres raises on
    # the overflow while SQLite silently allows it, so a short column here
    # is a production-only failure that every test passes.
    best_strategy: Mapped[str | None] = mapped_column(String(64))
    ...
    # The site CAN be split — zone, title, size and lot count all allow it —
    # whether or not the split makes money at what you would pay. Nothing in it
    # needs a price, so it is answerable for the four listings in five sold by
    # auction or negotiation, which is exactly the land a developer is hunting
    # and which the profit-based flag above could never see.
    can_subdivide: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    ...
    # What this was advertised at BEFORE the vendor withdrew the price.
    #
    # A house listed at $1,250,000 that comes back next week "by negotiation" is
    # not a house with no price. It is a house whose vendor has stopped naming
    # one, which almost always means the number is coming down — and the feed's
    # figure for those listings is a search price, set low so the listing shows
    # up in buyers' filters, not anything the vendor would accept. Without this
    # the listing simply fell out of the deal feed: 455 of them on the last
    # batch, every one a property we had a real price for a week earlier.
    #
    # Carried forward from the most recent earlier load that advertised a price,
    # matched on address. The date is that load's, so the listing can say what it
    # was listed at and when, which is a fact about the listing rather than a
    # guess about the vendor.
    prior_asking_price: Mapped[float | None] = mapped_column(Float)
    prior_asking_seen_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # Where asking_price came from — "advertised" when the vendor named it, or a
    # sentence naming the derivation when we did. NEVER blank on a row that has
    # an asking price: a derived figure that cannot be told apart from an
    # advertised one is the whole risk in doing this at all.
    asking_basis: Mapped[str | None] = mapped_column(String(64))

    # WHY this listing carries no deal signal, in words.
    #
    # The pipeline suppresses the deal on about a dozen distinct grounds — the
    # price is a search price rather than an advertised one, the listing has sat
    # too long to be a find, the discount is too large to believe, there is no
    # floor area to size-check an expensive home, the council record values only
    # the land. Every one of those decisions was made and then thrown away: the
    # row arrived with a blank margin and nothing anywhere said which rule blanked
    # it. On the last batch that was 317 listings, indistinguishable from 317
    # listings we simply failed to price.
    #
    # Written on every priced row that ends up without a deal signal, and cleared
    # on every row that keeps one, so it always describes the current pricing run.
    deal_block_reason: Mapped[str | None] = mapped_column(String(200))
...
# What a budget is measured against, in ONE place because there are three
# callers — the deal lists, a saved wish list, and Ollie's own search — and a
# budget that means something different in each is worse than no budget at all.
#
# Four listings in five sell by auction, tender or negotiation and therefore
# have no asking price: that is the market, not a gap in the data. Reading
# asking_price alone would answer "$1.5M or under" with the fifth of the market
# that advertises a number and silently drop every auction property a buyer
# could comfortably afford.
#
# So a listing is matched on the figure the site actually shows for it — the
# vendor's price where there is one, our own valuation where there is not.
BUDGET_PRICE = func.coalesce(PropertyForSale.asking_price,
                             PropertyForSale.fair_value,
                             PropertyForSale.market_value)
...
class LandParcel(Base):
    """Every section in a region, whether or not anybody is selling it.
    ...
    ParcelCache below answers "what is the boundary under this pin", one
    property at a time, because that is what a property page needs. It cannot
    answer the question above, which is asked of an AREA and is mostly about
    land nobody has listed — and a cache keyed on the pins we happen to have
    looked at only ever holds the houses that were for sale.

    So this is the other shape of the same data: the parcel layer loaded in
    bulk, one row per section, searchable by area and zone. The subdivision
    engine does not care whether a house is for sale — it takes a zone, a land
    area, a title type and a section rate — so once these rows exist every site
    in a suburb can be assessed on exactly the same rules as a listing.

    WHAT IS NOT HERE IS AS IMPORTANT AS WHAT IS. Zoning and title arrive from
    different sources than the boundary, and either can be absent for a given
    parcel. Absent must stay absent: `zone` is NULL until a council layer says
    otherwise, because the engine's first gate is the zone and a guessed one
    turns a Single House section into a development site. Same for
    `title_type`, where a blank is already treated as "not known to be
    freehold" and kept out.
    """
    __tablename__ = "land_parcels"

    id: Mapped[int] = mapped_column(primary_key=True)
    # LINZ's own parcel id, so a reload updates rather than duplicates.
    linz_id: Mapped[str] = mapped_column(String(32), nullable=False, index=True)
    ...
    # The land itself. area_m2 is LINZ's surveyed figure where it has one.
    area_m2: Mapped[float | None] = mapped_column(Float, index=True)
    ring: Mapped[str | None] = mapped_column(Text)          # JSON [[lat, lng], ...]
    lat: Mapped[float | None] = mapped_column(Float, index=True)
    lng: Mapped[float | None] = mapped_column(Float, index=True)

    # From other layers, and nullable on purpose — see the docstring.
    zone: Mapped[str | None] = mapped_column(String(120), index=True)
    title_type: Mapped[str | None] = mapped_column(String(64))
    # Overlays that stop a site being developed whatever the zone allows. In
    # the inner-west suburbs this decides most of the answer: a section can be
    # zoned for terraces and carry a villa that cannot legally come down.
    special_character: Mapped[bool | None] = mapped_column(Boolean)
    # What is already built on it, from the building-outlines layer. A large
    # well-zoned parcel with a small roof is the search.
    building_area_m2: Mapped[float | None] = mapped_column(Float)

    # HOW FAR TO THE NEAREST MAIN, per network, in metres.
    #
    # A big well-zoned flat parcel with no wastewater within reach is not a
    # development site, it is a bill. These are the numbers that tell the two
    # apart, and they are a DISTANCE and nothing more: whether a connection is
    # permitted depends on the main's capacity, its depth, the fall available
    # and the operator's approval, none of which are in any published layer.
    # Named for what they measure so nobody reads consent into them.
    #
    # NULL means not computed or no network loaded — never "nothing nearby".
    # The difference decides whether a developer walks away from a site, so an
    # absent figure must stay absent rather than default to a large number.
    stormwater_m: Mapped[float | None] = mapped_column(Float, index=True)
    wastewater_m: Mapped[float | None] = mapped_column(Float, index=True)
    water_m: Mapped[float | None] = mapped_column(Float, index=True)

    loaded_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        Index("ix_land_parcel_id", "region", "linz_id", unique=True),
        Index("ix_land_parcel_search", "region", "zone", "area_m2"),
    )


class ServicePipe(Base):
    """One length of public main: stormwater, wastewater, or water supply.

        "we just need to add stormwater and water on sections"

    The land tells you what a site could hold. This tells you whether it can be
    connected, which is the question that stops a development after the land has
    already been paid for.

    NOT FROM LINZ. LINZ is the cadastre — parcels, titles, addresses, elevation,
    and natural water. Reticulation belongs to whoever runs the network, and in
    Auckland that is two different operators: Watercare for water supply and
    wastewater, the council's side for stormwater. There is no national
    equivalent, which is why this is per-region and always will be.

    `path` is JSON: [[lat, lng], ...] along the pipe. Stored as the line rather
    than reduced to a point because distance to a MAIN is distance to the
    nearest part of it, and a pipe running the length of a street is nowhere
    near its own midpoint.
    """
    __tablename__ = "service_pipes"
    ...
    # What the operator published about it. Diameter is the one that matters:
    ...
    material: Mapped[str | None] = mapped_column(String(32))
    # WHO OWNS IT DECIDES WHETHER IT IS A CONNECTION AT ALL. The council's
    # export carries STORMWATER, TRANSPORT and PARKS side by side: only the
    # first is the public stormwater network. A TRANSPORT pipe is a road drain
    # and a PARKS one drains a reserve, and a development cannot simply tie into
    # either. Counting them would put a main across the road from every site.
    owner: Mapped[str | None] = mapped_column(String(64))
    # In service, or decommissioned. A retired pipe is still in the file.
    status: Mapped[str | None] = mapped_column(String(16))
    # How deep it runs. A shallow main cannot take a connection from a site that
    # falls away from it, whatever the distance says — so this is the second
    # question after "how far", and the council does publish it, patchily.
    depth_m: Mapped[float | None] = mapped_column(Float)
    loaded_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now())

    __table_args__ = (
        Index("ix_service_pipe_region_kind", "region", "kind"),
    )
...
# Which workspace an identity-linked key acts in.
#
# Newer Anthropic keys are tied to an identity rather than standing alone, and
# the API refuses them unless the request also names a workspace:
#
#   anthropic-workspace-id is required when authenticating with an
#   identity-linked API key; send the id of the workspace this request acts in.
#
# Not a secret — it identifies a workspace, it does not open one — so it is
# stored in the clear and shown in the admin panel, unlike the key beside it.
LLM_WORKSPACE_ID = "llm.workspace_id"
...
class PortalListing(Base):
    ...
    # The rest of the gallery, newline-separated, exactly as the weekly file
    # stores it (ingest._collect_images does the same from image_1_url .. 30).
    #
    # The actors return a LIST of photos and only the first was kept, so an
    # approved portal listing had one picture where a listing from the file has
    # up to thirty. A property page with one photo does not look like a fresh
    # listing, it looks like a broken one — and these rows are the newest
    # properties in the system, which is the whole reason for having them.
    image_urls: Mapped[str | None] = mapped_column(Text)
    image_count: Mapped[int | None] = mapped_column(Integer)
    ...
    # 48 was too short for the message it holds, and Postgres refuses a value
    # that does not fit while SQLite silently truncates — so every test passed
    # and the live insert failed:
    #
    #   "10% of the Flat Bush median — check for a missing digit"   54 chars
    #
    # The flag exists to catch a price with a digit missing or added. Being
    # unable to STORE that flag meant the listing it was about could not be
    # written at all, so the one record most in need of a human eye was the one
    # that fell out of the run. Sized for the longest suburb name in Auckland
    # with room over. (Same fault as best_strategy at 32; see v1.39.)
    price_flag: Mapped[str | None] = mapped_column(String(200))
    ...
    # What this property LAST SOLD FOR, from the council record — not this
    # listing's sale. One of exactly two fields in the whole weekly-file schema
    # that a portal row was missing AND that reaches the pricing engine: it is
    # how a scraped "price" that is really the last sale gets caught, and it is
    # read again when a listing is relisted near what it fetched last time.
    # Named apart from sale_price/sold_date, which belong to a SOLD row.
    prior_sale_price: Mapped[float | None] = mapped_column(Float)
    prior_sale_date: Mapped[str | None] = mapped_column(String(32))

    # That portal's OWN estimate. Stored as theirs, shown as theirs, and never
    # an input to our valuation — see the note in portals/fill.py.
    ...
    # A listing that has come off the portal is USUALLY sold, and sometimes
    # withdrawn, expired, or relisted under a new URL. We cannot tell which from
    # a dead link, so nothing here ever says "sold": that word belongs to the
    # weekly sold file, which carries a price and a date. This says only that
    # the advertisement is gone.
    link_checked_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # Consecutive daily checks that found the page gone. One 404 during a portal
    # deploy proves nothing; the count is what makes it evidence.
    link_gone_count: Mapped[int | None] = mapped_column(Integer, default=0)
    # Set once the count clears the threshold. Nullable and reversible — if the
    # link comes back, this is cleared and the listing is live again.
    delisted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    # The last thing the portal actually said, for anyone auditing a decision:
    # "404", "410", "403", "timeout". Kept even when it is not evidence.
    link_last_result: Mapped[str | None] = mapped_column(String(16))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True),
                                                 server_default=func.now())
    decided_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class RunEvent(Base):
    """One thing that happened to a load, in words, at the moment it happened.

    Everything the four stages decide has always been decided somewhere — rows
    rejected at import, addresses CoreLogic could not reach, valuations that
    took the sold-comps path instead of the council one, deals suppressed
    because the price was a search price. Almost none of it survived the run.
    It went to stdout, or into a counter that was displayed as a total, or
    nowhere at all, and by the time anybody asked "why did this happen" the run
    was over and the only honest answer was to guess.

    So this records it as it goes. Not a debug log — a record of decisions, in
    the words you would use to explain one to somebody, with the numbers that
    drove it attached. Small: one row per notable event plus one per rejection
    reason, not one per listing, so a 2,141-row load writes tens of rows rather
    than thousands. The per-listing detail already lives on the listing.

    Read it three ways: the JSON endpoint, the Excel download, or Ollie, which
    can query this table directly and answer "what happened to Tuesday's load"
    without anybody opening a file.
    """
    ...
    at: Mapped[datetime] = mapped_column(DateTime(timezone=True),
                                         server_default=func.now(), index=True)
    # Which upload this concerns. Nullable because some events (a portal sweep,
    # a model trained) belong to the system rather than to one file.
    batch_id: Mapped[int | None] = mapped_column(ForeignKey("import_batches.id",
                                                            ondelete="SET NULL"),
                                                 index=True)
    job_id: Mapped[int | None] = mapped_column(index=True)
    # load / enrich / price / publish / portals — the four stages plus sweeps.
    stage: Mapped[str] = mapped_column(String(16), nullable=False, index=True)
    # info / warn / error. Warn means "this ran but you should know"; error
    # means a stage stopped.
    level: Mapped[str] = mapped_column(String(8), default="info", nullable=False)
    # A short stable code (rows_rejected, corelogic_blocked, deal_suppressed)
    # so the same kind of event can be counted across runs, alongside...
    event: Mapped[str] = mapped_column(String(48), nullable=False, index=True)
    # ...the sentence a person actually reads.
    detail: Mapped[str | None] = mapped_column(Text)
    # How many listings this event covers, when it covers a countable set.
    count: Mapped[int | None] = mapped_column(Integer)
    # The listing this is about, when it is about exactly one.
    address: Mapped[str | None] = mapped_column(String(512))

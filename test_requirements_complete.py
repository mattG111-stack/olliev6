...
def _imported() -> dict[str, list[str]]:
    ...
    local_modules = {p.stem for p in APP.glob("*.py")} | {p.name for p in APP.iterdir() if p.is_dir()}
    for path in sorted(APP.rglob("*.py")):
        if path.relative_to(APP).parts[0] in ("tests", "scripts"):
            continue
        try:
        ...
        for node in ast.walk(tree):
            ...
            for name in names:
                top = name.split(".")[0]
                if top in sys.stdlib_module_names or top in local_modules:
                    continue

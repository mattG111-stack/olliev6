...
import propertyvalue as pv  # noqa: E402
...
def test_block_on_the_property_fetch_is_blocked(monkeypatch):
    _patch(monkeypatch, {
        "suggestions": _Resp(200, {"suggestions": [{"propertyId": "42", "url": "/p/42", "suggestion": "6 Cassino Terrace"}]}),
        "properties/42": _Resp(429),
    })
    _, status = pv.pv_lookup_status("6 Cassino Terrace")
    assert status == pv.PV_BLOCKED

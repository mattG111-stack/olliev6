...
def derived_asking(prior: float | None) -> float | None:
    ...
    try:
        ...
        return None
    if p <= 0 or p != p:                                   # NaN-safe
        return None
    out = round(p * (1.0 - NEGOTIATION_DISCOUNT_PCT) / ROUND_TO) * ROUND_TO
    return out if out > 0 else None


def address_key(address: str | None, suburb: str | None = None) -> str | None:
    """A form of the address stable enough to match the same house across loads.

    The feeds are not consistent between weeks — "12 Elliot Street, Remuera" one
    load, "12 Elliot St, Remuera " the next — so a raw string comparison finds
    almost nothing and silently reports "no prior price" for a house we have a
    prior price for. Lower-cased, punctuation dropped, whitespace collapsed, and
    the common street-type abbreviations spelled out.
    ...
    if not address:
        return None
    s = f"{address} {suburb or ''}".strip().lower()
    s = re.sub(r"[.,]", " ", s)
    for short, long in (
        (r"\bst\b", "street"), (r"\brd\b", "road"), (r"\bave?\b", "avenue"),
        (r"\bdr\b", "drive"), (r"\bpl\b", "place"), (r"\bcres\b", "crescent"),
        (r"\bter\b", "terrace"), (r"\bln\b", "lane"), (r"\bpde\b", "parade"),
        (r"\bhwy\b", "highway"), (r"\bcl\b", "close"), (r"\bgr[ov]\b", "grove"),
    ):
        s = re.sub(short, long, s)
    s = re.sub(r"\s+", " ", s).strip()
    return s or None


def _was_really_advertised(p: PropertyForSale) -> bool:
    """Did a vendor actually name this number?

    Mirrors release._asking_is_placeholder — a "price" equal to the council
    valuation or to the last sale price is the scraper filling a blank, and
    carrying it forward would launder a guess into a fact.
    """
    ask = p.asking_price
    if not ask or ask <= 0:
        return False
    ...
    if p.cv_numeric and abs(ask - p.cv_numeric) < 0.005 * p.cv_numeric:
        return False
    if p.valuation_last_sold_value and abs(ask - p.valuation_last_sold_value) < 1.0:
        return False
    return True
...
def carry_forward_prices(db: Session, batch_id: int, region: str = "Auckland") -> int:
    ...
    # Which rows need one: no usable asking price of their own. A row that has a
    # real advertised price today is left completely alone.

...
from assistant import sql as sqltool
...
def _gap(*, need: str, ask: str, why: str = "", known: str = "",
         have: str = "", outside_ok: bool = False) -> str:
    ...
    outside_ok — WHETHER LOOKING IT UP COULD HELP, and the reason this exists:

        "if it cant find the answer in the data it look on the internet"

    That is right, and it needed one distinction before it was safe. Most gaps
    here are gaps the USER closes: a suburb spelled differently, a bedroom
    count nobody gave, a price cap set too low. No amount of searching fixes a
    spelling — it just answers a different question confidently — so those stay
    a question back, which is faster anyway.

    A few are not like that. The sold file carries no listing date for most of
    its rows, so "how long do houses take to sell in Papakura" cannot be
    answered from our data by anybody, however they phrase it, and the person
    asking is left with nothing. That is the case the instruction is about, and
    those set this flag.

    Still not a licence to quote somebody else's valuation: the prompt keeps
    every figure about a specific property, and every number we compute
    ourselves, on our own tools. What an outside source may supply is the
    market-wide or statutory fact we simply do not hold — and it has to be
    named as theirs when it does.
    """
    ...
    # Before the Ask, deliberately. The block has always ended on the question
    # to put back — that is a tested contract and it is also the right shape,
    # because the last line is the one the model acts on. A note about whether
    # to search belongs above it, not after it.
    lines.append(
        "Looking it up: ALLOWED — our data cannot produce this for anyone, so "
        "search an official source, lead with what we do know, and say whose "
        "figure it is." if outside_ok else
        "Looking it up: NO — we hold this and the question missed it. A search "
        "would answer a different question confidently. Ask instead.")
...
def get_property(property_id: int) -> str:
    ...
    with SessionLocal() as s:
        ...
        return json.dumps({
            ...
            "parking_evidence": {"recorded_car_spaces": p.cars, "enclosed_garage": "not verified"},
            "condition": "not verified",
            "renovation_requirement": "not verified; build age is not condition evidence",
            "usable_outdoor_space": "not verified; land area alone does not establish usable outdoor space",
            "floor_m2": p.floor_area_m2, "land_m2": p.land_area_m2,
            "year_built": p.building_age,
            "build_date_precision": "may be a decade rather than an exact year; not condition evidence",
...
def renovation_value_by_district() -> str:
    ...
    """
    with SessionLocal() as s:
        # Every sold delivery, not the newest one. This is the tool that came
        # back empty on a hundred thousand sales, because it was reading
        # whichever file arrived last.
        batches = sold_batch_ids(s, "Auckland")
        if not batches:
            return _no_batch("sold")
        rows = by_district(_sold_frame(s, batches))
        return json.dumps({
...
def find_room_to_add_a_bedroom(limit: int = 10) -> str:
    ...
    """
    with SessionLocal() as s:
        sold_ids, live_b = sold_batch_ids(s, "Auckland"), _active(s, "for_sale")
        if not sold_ids or not live_b:
            return _no_batch("sold" if not sold_ids else "for_sale")
        P = PropertyForSale
        live_rows = s.query(
        ...
        found = conversion_opportunities(_sold_frame(s, sold_ids), live)
        return json.dumps({
            "count": len(found),
...
def suburb_days_to_sell(suburb: str) -> str:
    ...
    """
    with SessionLocal() as s:
        # Every sold batch, not the newest one. Sold history accumulates, and
        # resolving "the sold data" as a single batch here answered out of
        # whichever delivery happened to arrive last — a few hundred rows out
        # of a hundred thousand, with nothing on screen to say so.
        batches = sold_batch_ids(s, "Auckland")
        if not batches:
            return _no_batch("sold")
        rows = s.query(PropertySold.sold_date, PropertySold.days_on_market).filter(
            PropertySold.import_batch_id.in_(batches),
            PropertySold.suburb.ilike(f"%{suburb}%"),
            PropertySold.days_on_market.isnot(None),
            PropertySold.days_on_market > 0,
        ).all()
        if not rows:
            # BEFORE GIVING UP, ASK THE OTHER SOURCE.
            #
            # The sold file's listing dates are missing for most of the
            # history, and this tool used to stop there — "183 sales on file
            # and I cannot tell you how long they took". The listings we watch
            # ourselves answer the same question from the other end: appeared
            # on such a date, stopped being advertised on such a date.
            seen = observed_days_to_sell(s, suburb=suburb)
            if seen:
                return json.dumps({
                    "suburb": suburb,
                    "listings_measured": seen.houses,
                    "median_days_to_sell": seen.median_days,
                    "average_days_to_sell": seen.average_days,
                    "fastest": seen.fastest, "slowest": seen.slowest,
                    "still_on_the_market": seen.watching,
                }, default=str)
            ...
            if total:
                # Both our sources have now been asked and neither can answer,
                # and no rephrasing changes that: the sold rows carry no listing
                # date, and nothing we are watching there has come off the
                # market yet. This is the exact case the internet fallback was
                # asked for, so it is flagged for it — with what we DO hold led
                # with, so the answer is ours plus theirs rather than theirs.
                return _gap(
                    need=f"a listing date on {suburb}'s sales",
                    why=(f"{total:,} sales are on file there and none carry one, "
                         f"and no listing we are watching in {suburb} has come "
                         f"off the market yet"),
                    known=f"{total:,} sales on file in {suburb}",
                    ask=(f"I can't work out days-to-sell for {suburb} from our "
                         f"own records yet. Want the published market figure "
                         f"instead, or sale prices and volumes, which I do have?"),
                    outside_ok=True)
            names = [n for (n,) in s.query(PropertySold.suburb).filter(
                PropertySold.import_batch_id.in_(batches)).distinct() if n]
...
def find_address(address: str, suburb: str | None = None) -> str:
    ...
    with SessionLocal() as s:
        live_b, sold_ids = _active(s, "for_sale"), sold_batch_ids(s, "Auckland")
        ...
        sold = _scope(s.query(PropertySold).filter(
            PropertySold.import_batch_id.in_(sold_ids)) if sold_ids else
...
TOOL_SPECS = [
    ...
    _t("suburb_days_to_sell", "How long properties take to sell in a suburb.",
       {"suburb": _STR}, ["suburb"]),
    _t("research_outside",
       "Look up to FOUR separate things up at once, outside our data, each by "
       "its own researcher working in parallel. Use this ONLY for a question "
       "with several independent parts that our data does not cover — rules, "
       "rates, plan changes, consent processes, published national statistics. "
       "For a single fact, use the web search directly instead; four "
       "researchers on one fact all read the same page. NEVER use it for "
       "anything about a property, a suburb we hold sales for, a valuation, a "
       "median or what is on the market: those come from the tools above. The "
       "researchers cannot see our database at all.",
       {"questions": {"type": "array", "items": {"type": "string"},
                      "description": "One narrow question per researcher, up "
                                     "to four, and they must look up DIFFERENT "
                                     "THINGS. Split the question along its own "
                                     "seams — what the rule says, what it "
                                     "costs, how long it takes, what the "
                                     "published numbers show — not four "
                                     "rewordings of the headline. Near-"
                                     "duplicates are detected and only run "
                                     "once, so a rephrasing wastes a "
                                     "researcher rather than adding one."}},
       ["questions"]),
]


def research_outside(questions: list[str]) -> str:
    """Look up to four separate things up at once, outside our data.

    Args:
        questions: One narrow question per researcher, up to four. Each is
            answered independently by its own researcher, so split a broad
            question into its parts rather than sending it whole.
    """
    from assistant import research
    from assistant.providers import CALLER

    who = CALLER.get() or {}
    if who.get("provider") != "anthropic":
        # The researchers use the server-side search tool, which is an
        # Anthropic feature. Said plainly rather than failing: the question is
        # still answerable, just not this way.
        return ("Parallel research is not available on this provider. Answer "
                "from our own tools, or say what could not be checked.")
    return research.research(
        questions,
        api_key=who.get("api_key") or "",
...
_HANDLERS = {
    "research_outside": research_outside,

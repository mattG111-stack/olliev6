...
from prior_price import (ADVERTISED_BASIS, DERIVED_BASIS, ROUND_TO,
...
def test_the_same_house_is_matched_through_a_changed_abbreviation(db_session):
    ...
    # With the suburb passed, which is how the carry-forward always calls it —
    # it reads p.address and p.suburb off the row. The suburb is what lets the
    # run-on form ("...Street Remuera", no comma) be reduced to the same key as
    # the comma form; without it there is nothing to recognise as a suburb, and
    # a key with no suburb could not tell two "12 Queen Street"s apart anyway.
    assert address_key("12 Elliot St, Remuera", "Remuera") == \
        address_key("12 Elliot Street Remuera", "Remuera")
...
def test_a_carried_price_landing_on_the_cv_is_not_mistaken_for_a_fake_one(db_session):
    ...
    from release import _asking_is_placeholder, _hold_reason
...
def test_a_scraper_filling_in_the_cv_is_still_caught(db_session):
    ...
    from models import PropertyForSale
    from release import _asking_is_placeholder
...
def test_a_listing_priced_at_its_council_valuation_needs_a_carried_price(db_session):
    from prior_price import needs_a_carried_price
...
def test_one_placeholder_rule_not_three(db_session):
    ...
    from models import PropertyForSale
    from prior_price import is_placeholder_price
    from release import _asking_is_placeholder
    from routers.properties import _hide_bad_data          # noqa: F401
...
def test_thin_evidence_under_the_floor_is_still_refused(db_session):
    ...
    a lower number."""
    from pricing.pipeline import DEAL_EVIDENCE_COMPS
...
# ---- every branch of the deal guard, actually walked -------------------------
#
# The 1.8x branch read `deal_value` one line AFTER setting it to None:
#
#     deal_value = None
#     deal_block = f"Valuation ${deal_value:,.0f} is ..."
#
# which raises "unsupported format string passed to NoneType.__format__" and
# took a 146 MB load down with it on the first listing that reached it. The
# other two branches of the same guard had tests. This one did not, and that is
# the only reason it shipped.
#
# So this walks all three by construction rather than testing the one that was
# convenient to build a fixture for.
@pytest.mark.parametrize("label,asking,cv", [
    ("valuation far above the asking", 400_000.0, 3_275_000.0),
    ("council valuation over double",  12_500_000.0, 98_750_000.0),
    ("land-only council record",       600_000.0, 900_000.0),
])
def test_every_refusal_branch_produces_a_sentence(db_session, label, asking, cv):
    improvement = None if "land-only" in label else 1_900_000.0
    df = pd.DataFrame([_epsom(
        address=f"1 {label} Road", price_display=f"${asking:,.0f}",
        price_numeric=asking, cv_numeric=cv,
        improvement_value_numeric=improvement,
        land_value_numeric=cv if improvement is None else cv - improvement)])

    out = run_pipeline(df, SoldDataset_matching(), None)   # must not raise

    why = str(out.loc[0, "deal_block_reason"] or "")
    assert why, f"{label}: refused with no reason recorded"
    assert len(why) <= 200, why
    assert not out.loc[0, "is_underpriced"]


# ---- the ceiling has to run whatever the ratio test did ----------------------
#
# Found by running a real case, not by a test. The evidence override was written
# as the ratio test's `elif`, so a listing that CLEARED the override skipped the
# too-good-to-be-true ceiling entirely:
#
#     asking $500,000 against a $1,000,000 CV, 38 sold comps at high confidence,
#     valued $1,100,000 — 2.2x the asking, a 120% margin — published as a deal.
#
# The override exists to let honest cheap houses through. It let that through
# with them, and a 120% margin sorts to the top of the deal page.
def test_the_evidence_override_does_not_skip_the_ceiling(db_session):
    sold = SoldDataset_matching_at(1_100_000.0, cv=1_000_000.0)
    df = pd.DataFrame([_epsom(
        address="1 Half Price Road", price_display="$500,000",
        price_numeric=500_000.0, cv_numeric=1_000_000.0,
        improvement_value_numeric=600_000.0, land_value_numeric=400_000.0)])

    out = run_pipeline(df, sold, None)

    assert not out.loc[0, "is_underpriced"], \
        "a 2.2x valuation reached the deal feed through the evidence override"
    assert "x the" in str(out.loc[0, "deal_block_reason"])


def test_a_believable_discount_under_the_ratio_still_shows(db_session):
    """The counterweight — the whole point of the override. 0.77 of CV, well
    inside the ceiling, backed by sold evidence."""
    sold = SoldDataset_matching_at(1_210_000.0, cv=1_100_000.0)
    df = pd.DataFrame([_epsom(
        address="2 Genuinely Cheap Road", price_display="$850,000",
        price_numeric=850_000.0, cv_numeric=1_100_000.0,
        improvement_value_numeric=600_000.0, land_value_numeric=500_000.0)])

    out = run_pipeline(df, sold, None)
    assert out.loc[0, "is_underpriced"]


def SoldDataset_matching_at(price: float, cv: float):
    """Sixty size-matched sales at a chosen price, so a test can put the
    valuation exactly where it needs it to reach a specific guard."""
    from pricing.comps import SoldDataset

    return SoldDataset(pd.DataFrame([{
        "address": f"{i} Comparable Road", "suburb": "Remuera", "district": "Auckland",
        "property_type": "House", "price_numeric": price + i * 1_000,
        "sale_price": price + i * 1_000, "cv_numeric": cv,
        "key_floor_area": 122.0, "key_land_area": 1053.0,
        "key_bedrooms": 3, "key_bathrooms": 2, "sold_date": "2026-06-01",
        "days_on_market": 30,
    } for i in range(60)]))

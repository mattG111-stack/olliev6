...
def admin_from_header(authorization: str | None, db: Session) -> User | None:
    """The admin behind a raw Authorization header, or None.

    current_user is a dependency and rejects an anonymous caller outright, which
    is right for a protected route and wrong for a PUBLIC one that has an
    admin-only extra. This answers the softer question — "is there an admin
    here?" — without turning a public endpoint into a private one.
    """
    ...
    if len(parts) != 2 or parts[0].lower() != "bearer":
        return None
    try:
        payload = jwt.decode(parts[1], settings.jwt_secret,
...
def _ensure_seed_admin(db: Session) -> None:
    ...
    if admin is None:
        ...
        return
    log.info("seed admin already exists; account unchanged")

...
class StagedSummary:
    ...
    #
    #     "in the dashbaord i need the numbers of what is live and what has
    #      been deleted via our sold check"
    #
    # Everything above this line describes the batch being REVIEWED. None of it
    # answers the question an operator actually opens this page to ask, which is
    # how many houses a customer can see — and the one week that number quietly
    # fell from 9,281 to 657 there was nothing on any screen that said so. The
    # figure was in the database the whole time and on no page at all.
    #
    # Counted off the LIVE batch, not the staged one, so they are here whether
    # or not an upload is in progress.
    live_total: int = 0        # rows in the live load
    live_visible: int = 0      # what a customer can actually see
    live_gone: int = 0         # advertisement confirmed gone — the sold check
    live_held: int = 0         # held back at review
    live_other_hidden: int = 0 # hidden for anything else (bad price, no floor)
    # Week by week, how many houses the link check took off the site.
    #
    # The single number says the site is smaller; it does not say whether that
    # is a normal week's churn or something breaking. Auckland turns over a few
    # hundred a week — a week of forty is a check that has stopped working, and
    # a week of four thousand is a portal that blocked us. Neither is legible
    # without the weeks either side of it, so the shape is the point rather than
    # any one figure.
    #
    # Counted across every load, not just the live one, and by DISTINCT HOUSE:
    # a listing that was carried forward for a month appears in several loads
    # and left the market once.
    gone_weekly: list[dict] = field(default_factory=list)
...
def _staged_batch(db: Session, batch_type: str, region: str | None) -> ImportBatch | None:
    """The pre-live batch a publish should promote.

    NEWEST IS NOT THE SAME AS THE ONE WITH THE DATA, and the gap between those
    two is a way to publish nothing without being told.

    Upload a sold file, then upload it again before publishing. The second load
    checks every sale against what is already stored — which includes the first
    load's rows, staged or not — finds all 16,424 of them on file, and inserts
    none. That is correct: a sale should not be recorded twice. But it leaves an
    empty batch sitting at the top of the list, and taking the highest id made
    THAT the batch that went live. The sales were still in the table under the
    earlier batch; the page that reports what is live read zero.

    So an empty pre-live batch never wins over one that actually loaded rows.
    Only if none of them loaded anything does the newest stand — a genuinely
    empty file should still publish, and say so, rather than silently promote
    something from last week.
    """
    working = (db.query(ImportBatch)
               .filter(ImportBatch.batch_type == batch_type,
                       *([ImportBatch.region == region] if region is not None else []),
                       ImportBatch.status.in_(WORKING_STATUSES))
               .order_by(ImportBatch.id.desc()).all())
    for b in working:
        if (b.rows_inserted or 0) > 0:
            return b
...
def staged_summary(db: Session, region: str = "Auckland") -> StagedSummary:
    ...
    # The live load — independent of whatever is staged.
    ...
    if live is not None:
        # Deferred: routers.properties imports this module, so a top-level
        # import would be a cycle. _hide_bad_data is the SAME filter the site
        # applies, reused rather than restated — a second copy of the rule is
        # how the dashboard ends up disagreeing with the page it describes.
        from routers.properties import _hide_bad_data

        base = db.query(PropertyForSale).filter(
            PropertyForSale.import_batch_id == live.id)
        s.live_total = base.count()
        s.live_visible = _hide_bad_data(base).count()
        s.live_gone = base.filter(PropertyForSale.link_dead_at.isnot(None)).count()
        s.live_held = base.filter(PropertyForSale.is_held.is_(True),
                                  PropertyForSale.link_dead_at.is_(None)).count()
        s.live_other_hidden = max(
            0, s.live_total - s.live_visible - s.live_gone - s.live_held)
        s.gone_weekly = _gone_by_week(db, region)
    if fs:
...
def send_to_preview(db: Session, region: str = "Auckland") -> dict:
    ...
    for bt in (BatchType.SOLD.value, BatchType.FOR_SALE.value):
        # _staged_batch, not a query of its own. THE TWO MUST AGREE ABOUT WHICH
        # BATCH IS THE BATCH, and for one build they did not.
        #
        # This picked the newest row with status "staged". The review screen and
        # the publish both went through _staged_batch, which skips a pre-live
        # batch that loaded no rows. Upload a file, upload it again before
        # publishing — the second load finds every row already on file and
        # inserts none — and the two disagree: this moved the EMPTY batch to
        # preview while the screen went on reporting the full one as staged.
        #
        # On screen that is a dead button. Press "Finish and preview", nothing
        # visible happens, the green "Go live" button never appears, press it
        # again and nothing happens again. No error, nothing in the log, and no
        # way to publish the data sitting right there. One selector, used
        # everywhere, or this comes back.
...
def publish_release(db: Session, region: str = "Auckland", *, commit: bool = True) -> dict:
    ...
    """
    # Serialize publishers on Postgres, including the first-ever release where
    # there is no active row to lock. The lock lasts until commit/rollback.
    if db.get_bind().dialect.name == "postgresql":
        db.execute(text("SELECT pg_advisory_xact_lock(hashtext(:key))"),
                   {"key": f"ollie:publish:{region}"})
    now = datetime.now(timezone.utc)
    published = []
    for bt in (BatchType.SOLD.value, BatchType.FOR_SALE.value):
        staged = _staged_batch(db, bt, region)
        ...
        # Consume the other working batches in this release. Otherwise the
        # next click can publish an empty duplicate or revert to an older load.
        # Sold rows accumulate, so retain those batches as published history.
        for other in (db.query(ImportBatch).filter(
                ImportBatch.batch_type == bt, ImportBatch.region == region,
                ImportBatch.status.in_(WORKING_STATUSES),
                ImportBatch.id != staged.id).all()):
            other.status = "published" if bt == BatchType.SOLD.value else "superseded"
            other.is_active = bt == BatchType.SOLD.value
            if bt == BatchType.SOLD.value:
                other.published_at = now
...
# ---------------------------------------------------------------------------
# Putting a book back that a short upload replaced
# ---------------------------------------------------------------------------
# "becuase the other data is still there right ?" — yes, for now, and that
# qualifier is the important part. ARCHIVING a batch only hides it, so a book
# that vanished because a 672-row file went live over a 9,281-row one is
# recoverable without re-uploading anything:
#
#     "im not reup loading the data it too muck work if the data is still there
#      just make it show"
#
# What it does is exactly what the weekly carry does, sourced from the archive
# instead of from the live batch: every listing in a previously-live batch that
# is NOT in the live one and whose advertisement has not been found gone is
# copied across.
#
# COPIED, not re-priced. The rows were priced when they were loaded, against the
# sold data of the day, and they are already sitting there complete — valuation,
# subdivision, photographs, holds, link-check history. Re-running the pipeline
# over 8,000 rows to arrive at almost the same numbers would turn a repair into
# an hour-long job that can fail halfway. Press Re-price afterwards if you want
# them against this week's comps.
#
# Safe to press twice: a slug already in the live batch is skipped, so a second
# run restores nothing and reports nothing restored.
#
# IT IS NOT A BACKUP, AND THE WINDOW IS FINITE. Retention keeps the newest
# BATCH_RETENTION_LIMIT batches per type and region (twelve by default) and
# DELETES the rest, rows and all — the one place in the ingest that loses data
# for good rather than hiding it. An archived book therefore survives about a
# dozen more uploads, not for ever. Restore it and then it is live, and the live
# batch is pinned against pruning.

# Columns that belong to the row's identity or its place in a batch, and must
# never be copied onto the new row.
_NOT_COPIED = frozenset({"id", "import_batch_id"})


def restorable_count(db: Session, region: str = "Auckland") -> dict:
    """How many listings a restore would bring back, without touching anything.

    Answering this cheaply matters: the button should be able to say "restore
    8,609 listings" rather than asking somebody to press it and find out.
    """
    live = _live_batch(db, BatchType.FOR_SALE.value, region)
    if live is None:
        return {"restorable": 0, "from_batches": [], "live_batch_id": None}

    have = {s for (s,) in db.query(PropertyForSale.slug_id)
            .filter(PropertyForSale.import_batch_id == live.id).all() if s}
    seen: set[str] = set()
    per_batch: list[dict] = []
    for b in _archived_for_sale(db, region):
        rows = (db.query(PropertyForSale.slug_id, PropertyForSale.link_dead_at)
                .filter(PropertyForSale.import_batch_id == b.id)
                .order_by(PropertyForSale.id.desc()).all())
        n = 0
        for slug, dead_at in rows:
            if slug and slug not in have:
                have.add(slug)  # a newer dead copy also shadows older copies
                if dead_at is None:
                    seen.add(slug)
                    n += 1
        if n:
            per_batch.append({"batch_id": b.id, "filename": b.filename, "listings": n})
    return {"restorable": len(seen), "from_batches": per_batch,
            "live_batch_id": live.id}


def restore_missing_listings(db: Session, region: str = "Auckland") -> dict:
    """Copy every still-advertised listing from the archive into the live batch."""
    live = _live_batch(db, BatchType.FOR_SALE.value, region)
    if live is None:
        return {"restored": 0, "reason": "nothing is live to restore into"}

    have = {s for (s,) in db.query(PropertyForSale.slug_id)
            .filter(PropertyForSale.import_batch_id == live.id).all() if s}
    cols = [c.key for c in PropertyForSale.__mapper__.column_attrs
            if c.key not in _NOT_COPIED]

    restored = 0
    from_batches: list[int] = []
    for b in _archived_for_sale(db, region):
        ...
        n = 0
        for p in rows:
            # A listing with no slug cannot be matched against the live batch,
            # so restoring it risks a duplicate of a house already on the site.
            if not p.slug_id or p.slug_id in have:
                continue
            have.add(p.slug_id)
            if p.link_dead_at is not None:
                continue
            db.add(PropertyForSale(import_batch_id=live.id,
                                   **{c: getattr(p, c) for c in cols}))
            n += 1
        if n:
            from_batches.append(b.id)
            restored += n
    if not restored:
        return {"restored": 0, "reason": "the live batch already has every "
                                         "listing the archive still shows"}

    db.flush()
    live.rows_inserted = (live.rows_inserted or 0) + restored
    live.rows_total = (live.rows_total or 0) + restored
    _record(db, stage="publish", event="listings_restored", batch_id=live.id,
            count=restored, level="warn", commit=False,
            detail=(f"{restored:,} listing(s) restored into '{live.filename}' from "
                    f"batch(es) {', '.join(str(i) for i in from_batches)} — rows a "
                    f"short upload had archived, whose advertisements are still up"))
    db.commit()
    return {"restored": restored, "from_batches": from_batches,
            "live_batch_id": live.id}


def _live_batch(db: Session, batch_type: str, region: str) -> ImportBatch | None:
...
def _archived_for_sale(db: Session, region: str) -> list[ImportBatch]:
    """Previously-live for-sale batches, newest first.

    Newest first because a house can appear in several of them and the most
    recent copy is the one with the freshest price. Staged batches are excluded:
    a batch that never went live was never part of the book.
    """
    return (db.query(ImportBatch)
            .filter(ImportBatch.batch_type == BatchType.FOR_SALE.value,
                    ImportBatch.region == region,
                    ImportBatch.is_active.is_(False),
                    ImportBatch.status == "archived")
            .order_by(ImportBatch.id.desc()).all())


def _gone_by_week(db: Session, region: str, weeks: int = 8) -> list[dict]:
    """Houses the link check took off the site, newest week first.

    Bucketed in Python rather than in SQL on purpose: date_trunc is Postgres
    and strftime is SQLite, and a dashboard figure that is right in production
    and wrong in the tests is worse than no figure. The volume is small — only
    rows the check has actually retired — so there is nothing to gain by
    pushing it into the database.

    Distinct by listing reference, because a house carried forward across four
    loads is four rows and one departure.
    """
    ...
    rows = (db.query(PropertyForSale.slug_id, PropertyForSale.link_dead_at)
            ...
            .filter(ImportBatch.batch_type == BatchType.FOR_SALE.value,
                    ImportBatch.region == region,
                    PropertyForSale.link_dead_at.isnot(None))
            .all())
    seen: set[str] = set()
    ...
    out = [{"week_starting": k, "taken_off": v}

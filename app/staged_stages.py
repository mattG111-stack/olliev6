...
def stage_running(db: Session, batch_id: int, stage: str) -> bool:
    """True if an enrich/price stage is genuinely running for this batch.

    Prevents a second (heavy) background worker from stacking on the first — the
    main cause of an out-of-memory container kill. A job that has stopped
    beating past STAGE_NO_PROGRESS_MINUTES (its container died) is auto-cleared
    to "failed" so the operator can start again, instead of being locked out.
    """

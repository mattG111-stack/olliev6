...
These run the REAL pricing pipeline (pricing.pipeline.run) over tiny
...
# settings instantiate at import; give them values so the lazy ingest import
...
from pricing.comps import SoldDataset  # noqa: E402
from pricing.pipeline import run  # noqa: E402
...
def test_external_avm_cannot_inflate_a_bare_cv_section():
    ...
    value up to 0.95×AVM ≈ $1.22M.

    Nothing published for this row may go near the AVM. A $70k CV on a 40m²
    dwelling reads as land-only, and no sold comp in this suburb is remotely
    like it, so the pipeline now HOLDS the row (insufficient_comps) instead of
    valuing it — a stronger outcome than the capped number this test was
    written against, and the reason fair_value is blank rather than ~$70k.
    """
    sold = _sold([{
    ...
    row = run(fs, sold).iloc[0]
    assert row.get("expected_sale_path") == "insufficient_comps", (
        f"expected the row to be held, got {row.get('expected_sale_path')}"
    )
    for field in ("fair_value", "expected_sale", "market_value", "buy_price"):
        v = row.get(field)
        assert v is None or v <= 1.6 * 70000, (
            f"AVM inflated a bare-CV section: {field}={v}"
        )
...
def test_grid_hides_margin_dollars_when_engine_withheld_the_margin():
    ...
    from routers.release import _grid_row
...
def test_corrected_cv_overrides_a_wrong_scraped_cv():
    ...
    from staged_stages import corrected_cv
...
# ---------------------------------------------------------------------------
# "vs CV" on a CV we have already ruled untrustworthy
# ---------------------------------------------------------------------------
def test_a_land_only_cv_publishes_no_vs_cv_figure():
    """The +206.70% / +2,296.9% headline.

    31 Pekanga is valued from comps precisely because its $610k CV is the land
    alone. Dividing that comp value by the same discarded CV and printing the
    result — "+228% vs CV" — reports the council's missing improvement value as
    if it were an opportunity. Across 23 staged exports 2,925 rows (1.27%) sat
    above +100% here, on a median CV of $3,721 per m² of floor against $8,000
    for the median row. They are broken records, not finds.

    The valuation stands; the comparison does not get published.
    """
    sold = _sold([{
        "suburb": "Millwater", "district": "Rodney", "property_type": "House",
        "key_bedrooms": 5, "key_bathrooms": 4,
    ...
    fs = pd.DataFrame([{
        ...
        "key_floor_area": 307, "key_land_area": 765, "key_bedrooms": 5,
        "key_bathrooms": 4, "type_of_title": "Freehold",
        "land_value_numeric": 610000, "price_display": "Negotiation",
    }])
    row = run(fs, sold).iloc[0]
    assert row.get("fair_value"), "the valuation itself should survive"
    assert row.get("pred_vs_cv") is None, (
        f"published a vs-CV figure against a land-only CV: {row.get('pred_vs_cv')}")


def test_an_ordinary_home_still_shows_its_vs_cv():
    """Withhold the broken comparison, not the working one — this is the figure
    the deal page leads with on every normal listing."""
    sold = _sold([{
        "suburb": "Normalton", "district": "Normalton", "property_type": "House",
        "key_bedrooms": 3, "key_bathrooms": 2,
        "key_floor_area": f"{140 + i} sqm", "key_land_area": f"{500 + i * 3} sqm",
        "cv_numeric": 800000, "price_numeric": 800000 + i * 5000,
        "sale_price": 800000 + i * 5000, "land_value_numeric": 450000,
        "improvement_value_numeric": 350000,
        "type_of_title": "Freehold", "sold_date": "2025-03-01",
    } for i in range(6)])
    fs = pd.DataFrame([{
        "address": "5 Normal St", "suburb": "Normalton", "district": "Normalton",
        ...
        "key_floor_area": 145, "key_land_area": 505, "key_bedrooms": 3,
        "key_bathrooms": 2, "type_of_title": "Freehold",
        "land_value_numeric": 450000, "improvement_value_numeric": 350000,
    }])
    row = run(fs, sold).iloc[0]
    assert row.get("pred_vs_cv") is not None, "withheld a perfectly good vs-CV figure"
    assert abs(row["pred_vs_cv"]) < 0.4, f"vs CV out of range: {row['pred_vs_cv']}"

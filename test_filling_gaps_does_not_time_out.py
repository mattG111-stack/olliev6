...
def _no_worker_outlives_its_test(monkeypatch):
    """Wait for the fill worker before letting the test end.

    THE FLAKE THIS FIXES, AND IT FAILED A DIFFERENT TEST THAN THE ONE AT FAULT.
    The endpoint answers immediately and does the work on a daemon thread. That
    thread outlives the test — so monkeypatch reverts the stubbed lookup while
    it is still going, it calls the REAL one, and it writes into the database
    the NEXT test has just rebuilt. The test that fails is whichever one the
    stray writes happen to land in, which is why it moved around between runs
    and passed every time the file was run on its own.

    Autouse, because the rule is about every test in the file rather than about
    the ones anybody remembered.

    IT TAKES `monkeypatch` TO GET THE ORDER RIGHT, and without that it does not
    work at all. Fixtures tear down in reverse order of setup, so an autouse
    fixture declared here is built FIRST and torn down LAST — after monkeypatch
    has already put the real lookup back. Joining then is joining a thread that
    is by now calling out to the internet, which is the thing being prevented.
    Depending on monkeypatch puts this fixture after it, so the join happens
    while the stub is still in place.
    """
    import threading

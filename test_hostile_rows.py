...
from pricing.glm import _features  # noqa: E402
...
def test_checking_a_file_survives_a_row_with_no_floor_area():
    ...
    The load aliases pricing.assumptions as `A` and calls A.is_vacant_type.
    The preflight copied the call and aliased pricing.audit — same letter,
...
def test_an_infinite_cell_costs_that_cell_and_not_the_load():
    ...
    from ingest import _to_float, _to_int
    from prior_price import derived_asking
...
# ---- a house we have no size for --------------------------------------------
def test_a_dwelling_with_no_floor_area_is_not_called_a_deal():
    """787 listings of 9,102 had no floor area on a real batch, and 89 of them
    were flagged as deals — 38 at high confidence.

    The rule existed and only fired above $2.5M, on the reasoning that a missing
    floor most distorts the margin at the top of the market. True, and not the
    point: a $900,000 house with no floor area is exactly as unsized as a $4.6M
    one, and there are far more of them. The size cap cannot fire, the comps are
    matched on size, and nothing independent checks the number.

    The valuation still shows. What a place is worth is a fair question either
    way; calling it a DISCOUNT is the claim that needs the size.
    """
    out = run(_listing(key_floor_area=None, price_numeric=700_000,
                       cv_numeric=900_000), _sold()).iloc[0]

    assert out.get("margin") is None
    assert "floor area" in (out.get("deal_block_reason") or "").lower()


def test_bare_land_keeps_its_deal_signal():
    """The exception the rule needs. A section has no floor area because it has
    no building — that is the data, not a gap in it. Refusing those would throw
    away every piece of vacant land in the file."""
    out = run(_listing(property_type="Section", key_floor_area=None,
                       key_land_area=600, price_numeric=700_000,
                       cv_numeric=900_000), _sold()).iloc[0]

    reason = (out.get("deal_block_reason") or "").lower()
    assert "floor area" not in reason


def test_a_house_with_a_floor_area_is_untouched():
    """Guard on the guard: a rule that blocks everything is not a fix."""
    out = run(_listing(key_floor_area=140), _sold()).iloc[0]
    assert "floor area" not in (out.get("deal_block_reason") or "").lower()

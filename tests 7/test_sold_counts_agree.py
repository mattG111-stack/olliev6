...
from routers.properties import map_points, suburb_stats, suburbs
...
def test_the_map_plots_every_delivery_too(db_session):
    ...
    # sold_months=None asks for every sale on record, because what this test is
    # about is BATCH ACCUMULATION — the map used to read only the newest sold
    # batch and plot 51 of 111. The six-month default the map now ships with is
    # a separate question, covered in tests/test_map_recency.py; leaving it on
    ...
    res = map_points(region="Auckland", dataset="sold", suburb="Mount Eden",
                         ...
                         max_price=None, search=None, limit=20000,
                         sold_months=None, db=db)

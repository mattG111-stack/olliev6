...
from prior_price import ADVERTISED_BASIS
...
class ForSaleRow(BaseModel):
    ...
    # otherwise a sentence naming the derivation. Sent on every listing so the
    # page can never show a derived figure as though a vendor had asked it.
    asking_basis: str | None = None
    # What the vendor last advertised, and the day we saw it — shown as
    # "was listed at $X on <date>" beside a listing that is now by negotiation.
    prior_asking_price: float | None = None
    prior_asking_seen_at: datetime | None = None
    ...
    # THE MODEL'S OWN WORKINGS — pred_v35, pred_v38, z_weight, beta_tier,
    # cv_anchor, cv_ratio_tier, correction_used — are no longer sent here.
    #
    # Seven fields describing which branch of the valuation ran, what it was
    # anchored to, which correction was applied and how the two model versions
    # were blended: taken together, a description of how the thing works, on
    # every listing, to every logged-in customer. Not one of them was read by a
    # single page — checked across the whole frontend, admin included, before
    # removing them — so nothing on screen changes and nobody loses anything
    # except somebody reading the network tab to learn the method.
    #
    # They are still stored on the row and still on the ADMIN response
    # (StagedGridRow), where they are diagnostics for the operator, which is
    ...
    # The advertisement has come off the portal. Lists already exclude these,
    # but a bookmark, a shared link or a wish list opens the detail page
    # directly — so the page has to be able to say so, and stop offering a
    # button through to a dead page.
    off_market: bool = False
    # The alternative plan is NOT here on purpose: it is not a stored column, so
    # a field here would be None on every row and read as "there is no other
    # option" rather than "we did not fetch it". The comparison comes from the
    # live scenario endpoint, which recomputes both. The headline itself already
    # arrives through best_strategy, which now says "Demolish…" where that wins.
    is_underpriced: bool
    is_cashflow_positive: bool
    is_subdividable: bool
    # Whether the site CAN be split, as against whether it is worth splitting.
    # The lot count is a fact about the land and does not need a price; without
    # this the page a listing reaches through the subdivision filter has no way
    # to say why it is there.
    #
    # OPTIONAL, and that is not tidiness. The column is added to an existing
    # table by ALTER TABLE, so every row already in the database reads NULL
    # until it is re-priced — and a plain `bool` here turns each of those into a
    # validation error, which is the whole listing endpoint answering 500 on the
    # first request after a deploy. None is the honest value for "not assessed"
    # and is falsy on the page.
    can_subdivide: bool | None = False
    # THE SUPPLIER'S OWN ADDRESS, and it used to be printed on every page.
    #
    # `url` pointed at the portal the listing came from and every photograph
    # loaded FROM that portal, so the browser named our data source on page
    # load — no click required, just the network tab. `image_urls` handed over
    # the whole gallery path for every listing in one field.
    #
    # Both are rewritten on the way out now (see the serialisers below): photos
    # become paths on our own domain, and the outbound link becomes a redirect
    # we own. Nothing downstream changes — the browser is handed a URL and
    # renders it, and it has no way to tell which host is behind it.
    url: str | None
    image_url: str | None
    ...
    # === OneRoof's own estimate, in its own columns ===
    # It used to be written over third_party_valuation, so a page labelled one
    # portal could be showing another's number with nothing to say so.
    # THE SUPPLIERS' NAMES WERE IN THE FIELD NAMES.
    #
    # A payload does not have to contain a URL to give the game away: every
    # listing carried keys reading oneroof_valuation, oneroof_url, tm_valuation
    # — a list of who we buy from, on every row, to every logged-in customer,
    # readable in the network tab without clicking anything. Caught by a test
    # that searched the serialised JSON for supplier names rather than checking
    # the fields I happened to think of.
    #
    # Deleting them was the wrong fix and a test said so: "write-only columns
    # are the same as no columns" — an estimate we fetch, store and never
    # publish is an estimate we wasted the lookup on. So the NUMBER stays and
    # the NAME goes. The database columns keep their names (no migration, and
    # internally it matters which portal said what); only the wire key is
    # neutral, and the screen labels the source from its own text.
    portal_a_valuation: float | None = Field(default=None,
                                             validation_alias="oneroof_valuation")
    portal_a_valuation_low: float | None = Field(default=None,
                                                 validation_alias="oneroof_valuation_low")
    portal_a_valuation_high: float | None = Field(default=None,
                                                  validation_alias="oneroof_valuation_high")
    portal_a_url: str | None = Field(default=None, validation_alias="oneroof_url")

    # === "Trade Me says" — for display only, never an input (see trademe.py) ===
    portal_b_valuation: float | None = Field(default=None,
                                             validation_alias="tm_valuation")
    portal_b_valuation_low: float | None = Field(default=None,
                                                 validation_alias="tm_valuation_low")
    portal_b_valuation_high: float | None = Field(default=None,
                                                  validation_alias="tm_valuation_high")
    portal_b_valuation_date: str | None = Field(default=None,
                                                validation_alias="tm_valuation_date")
    ...
    # ---- nothing leaves here pointing at a supplier -------------------------
    @field_serializer("image_url")
    def _proxy_image(self, v: str | None) -> str | None:
        return media.tokenise(v) or None

    @field_serializer("image_urls")
    def _proxy_gallery(self, v: str | None) -> str | None:
        if not v:
            return v
        out = [media.tokenise(u.strip()) or "" for u in str(v).split("\n")]
        return "\n".join(u for u in out if u)

    @field_serializer("url")
    def _proxy_link(self, v: str | None) -> str | None:
        return media.link(v)

    # The neutral KEY is only half of it: the value is an address on the
    # portal's own domain, so publishing it raw names the supplier just as
    # loudly as the old field name did. Same redirect as the listing link.
    @field_serializer("portal_a_url")
    def _proxy_portal_a(self, v: str | None) -> str | None:
        return media.link(v)
...
# Preview mode, held per request.
#
# "I need to be able to view the listings like they are Live but they're not
# Live because I need to see if we've got all the information right and pricing"
#
# The review grid cannot answer that. A grid shows you a batch; it does not show
# you what a customer will see, and the things that go wrong — a valuation that
# reads absurdly next to the asking price, a photo missing, a suburb panel built
# on four sales — are things you notice by LOOKING at the page.
#
# So the whole site can be pointed at the batch that is in preview. One switch,
# read here, and every list, panel and property page follows it: there is exactly
# one function that decides which batch a reader sees, and this is it.
#
# A ContextVar rather than a parameter threaded through thirteen call sites and
# their endpoint signatures. FastAPI runs each request in its own task, so the
# value cannot leak between two requests being served at once.
_PREVIEW = contextvars.ContextVar("apex_preview_mode", default=False)


async def preview_mode(preview: int = 0,
                       authorization: str | None = Header(None),
                       db: Session = Depends(get_db)) -> bool:
    """Turn preview on for this request, for an admin, on request.

    ASYNC, and it has to be. FastAPI runs a SYNC dependency in a threadpool with
    a COPY of the request context, so a ContextVar set inside one is discarded
    the moment it returns and the endpoint reads the default. Written `def`, this
    dependency ran, authorised correctly, set the flag — and every endpoint still
    saw preview as off, so the page came up empty with a 200 and nothing in the
    logs to suggest anything was wrong.

    Nothing catches that except running it. The unit tests below call this
    function directly, in one thread, where the set does survive; they all
    passed. It took an actual HTTP request to see it, which is why there is now
    one of those too.

    Refuses LOUDLY rather than falling back to live data. Someone checking a
    batch before it goes live, who is quietly shown the live batch instead, will
    sign off on the wrong thing — and the whole point of the step is that it is
    the last chance to catch something.
    """
    on = bool(preview) and admin_from_header(authorization, db) is not None
    if preview and not on:
        raise HTTPException(
            status_code=403,
            detail="Preview is for admins. Sign in as an admin to see the "
                   "batch before it goes live.")
    _PREVIEW.set(on)
    return on


# On the ROUTER, not on each endpoint. Thirteen call sites read _active_batch and
# a preview that works on twelve of them is worse than none — you would be
# checking a page that is half this week and half last.
#
# Appended here rather than passed to APIRouter() because the function has to
# exist first; FastAPI copies router.dependencies as each route is registered,
# and the first @router decorator in this file is far below.
router.dependencies.append(Depends(preview_mode))


def _active_batch(db: Session, batch_type: str, region: str) -> int | None:
    """The batch a reader sees: the previewed one in preview mode, else live."""
    if _PREVIEW.get():
        ...
        # No preview batch of this type is not a reason to show the live one —
        # sold data in particular is cumulative and the live batch is correct.
        if b:
            return b.id
    b = (
...
def _fit_rooms(rows: list, value: str = "sale_price") -> dict[str, RoomFit]:
    ...
    if len(keep) < ROOM_MIN_SALES:
        return {k: RoomFit(n=len(keep),
                           note=f"only {len(keep)} sales with the four inputs — "
                                f"{ROOM_MIN_SALES} needed to measure")
                for k in ("beds", "baths")}
    # Whether the modal-type guard above could run at all. It is the difference
    # between fitting houses against houses and pooling a 2-bed unit at $600k
    # with a 5-bed house at $3M — which widens the residuals until every
    # coefficient's interval swallows zero and the honest answer becomes "no
    # measurable effect". The newer sold export carries no property_type column,
    # so the guard silently does nothing, and the failure looks like a market
    # with no pattern in it rather than a missing field.
    mixed_types = not typed
    ...
    for room, i in (("beds", 3), ("baths", 4)):
        ...
        if not (d == d) or se <= 0:
            ...
            #
            # Say WHICH sales, and say when the property-type guard could not
            # run. "No measurable effect here" on its own reads as a finding
            # about the suburb; most of the time it is a finding about the data,
            # and there was no way to tell the two apart from the screen.
            note = f"no measurable effect across {int(len(y))} sales"
            if mixed_types:
                note += " — property type not recorded, so all dwelling types are pooled"
            out[room] = RoomFit(n=int(len(y)), note=note)
        else:
            out[room] = RoomFit(dollars=d, low=d - half, high=d + half, n=int(len(y)))
    return out


# A cell needs this many sales before its median means anything. Below it one
# unusual sale sets the number for the whole comparison.
ROOM_CELL_MIN = 6


def _room_effect_by_cell(rows: list, room: str) -> RoomFit:
    """What a 3-bed sells for here against what a 4-bed sells for. That is all.

    This replaces a regression that held FLOOR AREA CONSTANT while varying the
    room count — which asks "what if this same house were chopped into more
    rooms", not "what if it had another bedroom". It is a different question with
    a different answer, and the answer to the one nobody asked came out at
    minus $210,000 a bedroom.

    A four-bedroom house IS bigger than a three-bedroom one. Letting size come
    with the room is the whole point: the buyer is not adding a wall, they are
    adding a room, and the market prices the result.

    Compared at the suburb's most common count, so the pair with the most sales
    behind it decides the figure, and both cells must clear ROOM_CELL_MIN.
    """
    import statistics

    attr = "beds" if room == "beds" else "baths"

    # For BATHROOMS, compare within one bedroom count.
    #
    # Left alone, 1-bath houses are simply the 2-beds and 2-bath houses are the
    # 4-beds, so the comparison measures the extra bedrooms and calls the result
    # a bathroom — $524k in testing, against a bedroom worth $400k. Holding beds
    # fixed asks the question people mean: this house, one more bathroom.
    #
    # Bedrooms need no such control. A 4-bed IS bigger than a 3-bed, and that
    # extra size is exactly what the buyer is paying for.
    held = None
    if attr == "baths":
        counts: dict[int, int] = {}
        for r in rows:
            b = getattr(r, "beds", None)
            if b is not None and getattr(r, "sale_price", None):
                counts[int(b)] = counts.get(int(b), 0) + 1
        if counts:
            held = max(counts, key=lambda k: counts[k])

    # ONE KIND OF HOME, or this measures the wrong thing.
    #
    # Without it a 3-bed apartment sits in the same cell as a 3-bed villa and a
    # 4-bed villa sits in the next, so the "bedroom" is really the difference
    # between an apartment and a house. Mount Eden published +$950k a bedroom on
    # a $1.79M median that way. The comparison is held to the suburb's most
    # common type — houses nearly everywhere — so it is like against like.
    type_counts: dict[str, int] = {}
    for r in rows:
        if getattr(r, "sale_price", None):
            t = canonical_type(getattr(r, "property_type", None))
            type_counts[t] = type_counts.get(t, 0) + 1
    modal_type = max(type_counts, key=lambda k: type_counts[k]) if type_counts else None

    cells: dict[int, list[float]] = {}
    for r in rows:
        n = getattr(r, attr, None)
        price = getattr(r, "sale_price", None)
        if n is None or not price:
            continue
        if modal_type and canonical_type(getattr(r, "property_type", None)) != modal_type:
            continue
        if held is not None:
            b = getattr(r, "beds", None)
            if b is None or int(b) != held:
                continue
        try:
            n = int(n)
            ...
            continue
        if 0 < n <= 10:
            cells.setdefault(n, []).append(float(price))

    usable = {n: v for n, v in cells.items() if len(v) >= ROOM_CELL_MIN}
    if len(usable) < 2:
        got = sum(len(v) for v in cells.values())
        return RoomFit(n=got, note=f"not enough sales at each {attr[:-1]} count to compare "
                                   f"(need {ROOM_CELL_MIN} either side)")

    # The pair with the most sales behind it, among counts that sit next to each
    # other. Adjacent only: comparing a 2-bed with a 5-bed measures a different
    # kind of house, not another bedroom.
    pairs = [(n, n + 1) for n in sorted(usable) if n + 1 in usable]
    if not pairs:
        return RoomFit(n=sum(len(v) for v in usable.values()),
                       note=f"no two neighbouring {attr[:-1]} counts both have "
                            f"{ROOM_CELL_MIN} sales")
    lo, hi = max(pairs, key=lambda p: len(usable[p[0]]) + len(usable[p[1]]))
    a, b = sorted(usable[lo]), sorted(usable[hi])
    diff = statistics.median(b) - statistics.median(a)
    where = f" in {held}-bedroom homes" if held is not None else ""
    kind = f", {modal_type.lower()}s only" if modal_type else ""
    return RoomFit(
        dollars=diff,
        n=len(a) + len(b),
        note=f"{lo}→{hi} {attr}{where}{kind}: {len(a)} vs {len(b)} sales",
    )
...
def _method_bucket(raw: str | None) -> str | None:
    ...
    if not v:
        # No method recorded. Counted as private treaty for the same reason
        # _method_label does: auction, tender and private treaty are effectively
        # the whole universe, and the two that get recorded are the two that are
        # publicly scheduled. Returning None here dropped the row from the panel
        # entirely, which is why the newer export — carrying no method field at
        # all — produced a suburb of nothing but auctions.
        return "negotiation"
    if "auction" in v:
        return "auction"
    # "treaty" and "private" matter as much as "negoti": the value this data
    # actually carries is "P - Private Treaty(Neg.)", which contains "neg." but
    # not "negoti", so it matched nothing and fell through to None. Every
    # private-treaty sale ever loaded was dropped from this panel — not because
    # the method was missing, but because the word for it was never listed.
    if ("negoti" in v or "treaty" in v or "private" in v or "pbn" in v
            or "enquir" in v or "eoi" in v or "offers over" in v):
        return "negotiation"
    if "tender" in v:
        return "tender"
    if "deadline" in v:
    ...
    if "price" in v or "asking" in v or "fixed" in v or "buy now" in v:
        return "fixed"
    if len(v) == 1:
        # "p" is Private Treaty in the source's own coding — the same letter
        # _METHOD_LABELS reads as Price by Negotiation. It was mapped to "fixed"
        # here, so one code meant two different things depending on which
        # function looked at it.
        return {"a": "auction", "n": "negotiation", "t": "tender",
                "d": "deadline", "p": "negotiation", "f": "fixed"}.get(v)
    # Unrecognised, but something was recorded. Private treaty is the safe
    # bucket: it is the default way a house sells when nothing scheduled it.
    return "negotiation"


# A sale outside this band of its council valuation is not a market transaction:
# a part share, a family transfer, a CV attached to the wrong property. Applied
# wherever sale-vs-CV is measured, because one such row can carry a whole method.
...
# Sales needed inside the pooled window before a point is drawn at all. The
# headline series used to default to ONE, so a month with a single sale plotted
# that house's price as the suburb's median.
MIN_MONTH_SALES = 6
...
# Each point pools this month and the two before it.
#
# A suburb sells a handful of homes a month. A median of three sales — or, at
# the old default of one, a median of a single sale — is not a market level, it
# is whichever house happened to change hands. Plotted month by month that drew
# a 30% sawtooth and called it "where the market is heading", when a market does
# not do that; the mix of what sold does.
#
# Three months is what property reporting uses on suburb-level data for exactly
# this reason. It is a real median of real sales, over a window wide enough to
# hold still.
# How far back the SOLD map reaches by default.
#
# It reached back forever, which in Flat Bush is 2,950 dots on one map. That is
# not a picture of a market — every street is solid grey and the live listings
# you came to look at are lost inside it. Six months is what a buyer means by
# "what has been selling around here", and the map offers 3 / 6 / 12 / 24 / all
# on top of it.
SOLD_MAP_MONTHS = 6


def _recent_months(months: int, today: datetime | None = None) -> set[str]:
    """The last `months` calendar months as YYYY-MM, ending with this one.

    Anchored on TODAY rather than on the newest sale in the file. If the feed is
    a fortnight stale, "last 6 months" should show five and a half months of
    sales — quietly sliding the window back to wherever the data ends would hide
    exactly the gap worth noticing.
    """
    now = (today or datetime.now(timezone.utc).astimezone(NZ_TZ))
    end = f"{now.year:04d}-{now.month:02d}"
    return set(_months_range(_shift_period(end, months - 1), end))


def _in_months(sold_date, wanted: set[str]) -> bool:
    """Did this sale happen in one of those months? Format-agnostic."""
    ym = sold_year_month(sold_date)
    return bool(ym) and f"{ym[0]:04d}-{ym[1]:02d}" in wanted


MONTH_SMOOTHING = 3

# Sales a window aims to hold before its median is trusted, and how far back it
# may reach to find them.
#
# A fixed three months is not enough where a suburb sells three homes a month:
# measured on a market held deliberately FLAT all year, sampling alone still
# produced a 33% peak-to-trough swing. The window therefore widens itself —
# back one month at a time until it holds MONTH_TARGET_SALES — so a busy suburb
# keeps a responsive three-month line while a quiet one reaches back far enough
# to stop reporting its own mix as a market movement.
# Measured and rejected. Widening until a window held 25 sales did cut the noise
# in a flat market (33% -> 17%), and it flattened a REAL rise of 19% a year into
# -7% at three sales a month, because the early and late windows ended up pooling
# almost the same sales. Trading a visible trend for a smooth line is not a
# trade worth making, so the widening is off and the window is a plain three
# months. Kept as a parameter because the machinery is harmless and the
# measurement is worth not repeating.
MONTH_TARGET_SALES = 0
MONTH_MAX_WINDOW = 12


def _month_series(by_month: dict, months: list[str], *, min_sales: int = 1,
                  window: int = MONTH_SMOOTHING,
                  target: int = MONTH_TARGET_SALES,
                  max_window: int = MONTH_MAX_WINDOW) -> list[MonthPoint]:
    """One point per calendar month, each pooling the months behind it.

    The pool starts at `window` months and grows until it holds `target` sales
    or hits `max_window`. Every point is a real median of real sales; the only
    thing that varies is how much history it had to reach through to get a
    sample worth taking a median of.

    `sales` stays the count for THAT month — it answers "how busy was this
    month", a different question from "what were homes worth around then", and
    blending it would misreport activity.
    """
    ...
    for per in months:
        ...
        while span < max_window:
            ...
            if j < 0:
                break
            pooled.extend(by_month.get(months[j], []))
            span += 1
            if span >= window and len(pooled) >= target:
                break
        this_month = by_month.get(per, [])
        got = pooled
        enough = len(got) >= max(min_sales, 1)
        m_svc = _median([r.sale_price / r.cv_numeric for r in got
                         if r.sale_price and r.cv_numeric
                         and ARMS_LENGTH_LO <= r.sale_price / r.cv_numeric
                         <= ARMS_LENGTH_HI]) if enough else None
        out.append(MonthPoint(
            month=per,
            sales=len(this_month),
...
class SuburbStats(BaseModel):
    ...
    # The window every sold figure above was computed over, and the full span
    # available to choose from. Without these a median is a number with no
    # stated basis — and the same suburb reads differently at one year than at
    # ten, for real reasons.
...
def suburb_stats(suburb: str, region: str = "Auckland",
                 ...
                 ptype: str | None = Query(None, max_length=32),
                 db: Session = Depends(get_db)) -> SuburbStats:
    """Suburb summary + empirical 'what moves value' effects, from sold data.

    `ptype` narrows every figure to one kind of home — House, Townhouse,
    Apartment, Unit. A suburb median across houses AND apartments moves when the
    MIX of what sold changes, not when the market does, and that mix effect is
    large: measured on generated sales, a median price line swung 74% in a market
    held deliberately flat. Holding the type still removes most of it.

    Matched through canonical_type() rather than on the raw string, because the
    source portal serves a Chinese-NZ audience and emits types in Chinese —
    独立屋, 独立别墅 and 独立式住宅 are all houses, and a filter comparing raw
    strings would quietly return a fraction of them.
    """
    name = suburb.strip()
    ...
    # Every sold batch for the region — sold history accumulates, so keying this
    # to one batch reported a slice of the suburb as the suburb.
    sold_batch_ids = live_sold_batch_ids(db, region) or [sold_batch]
    rows = (db.query(PropertySold)
            .filter(PropertySold.import_batch_id.in_(sold_batch_ids),
                    PropertySold.suburb.in_(
                        _area_values(db, PropertySold, sold_batch, "suburb", name)),
                    PropertySold.sale_price.isnot(None))
            .limit(6000).all())
    # isinstance, not truthiness: called straight as a function (which the tests
    # do) the default is FastAPI's Query object, which is truthy and has no
    # .strip() — so a bare `if ptype:` crashed every direct caller.
    want = ptype.strip().lower() if isinstance(ptype, str) and ptype.strip() else None
    if want:
        rows = [r for r in rows if canonical_type(r.property_type).lower() == want]
    # Which years these figures describe.
    #
    # Every figure below — the medians, the price per m2, the sale-vs-CV ratio,
    # the fitted room effects — treats a sale as evidence about the market. That
    # was safe while sold files covered a recent window. With decades of history
    # loaded it is not: a median across thirty years is meaningless, and the room
    # fit collapses outright because the same house recurs with identical rooms
    # against 1994 and 2026 prices.
    #
    ...
    lo_year = from_year or to_year or newest_year
    hi_year = to_year or newest_year
    if lo_year and hi_year and lo_year > hi_year:
        lo_year, hi_year = hi_year, lo_year
    if lo_year and hi_year:
        rows = [r for r in rows
                if (ym := sold_year_month(r.sold_date)) is None
                or lo_year <= ym[0] <= hi_year]
    ...
    # Guarded like every other sale-vs-CV figure: a sale at 6% of its council
    # valuation is not a market transaction, and in a thin month it moves the
    # headline number on its own.
    svc = [r.sale_price / r.cv_numeric for r in rows
           if r.sale_price and r.cv_numeric
           and ARMS_LENGTH_LO <= r.sale_price / r.cv_numeric <= ARMS_LENGTH_HI]

    median_sold = _median(prices)
    effects: list[MarginalEffect] = []
    for key, room in (("bedroom", "beds"), ("bathroom", "baths")):
        # What a 3-bed sells for against what a 4-bed sells for. The regression
        # this replaces held floor area constant, so it answered "what if this
        # house were divided into more rooms" — a question nobody asked, whose
        # answer was minus $210,000 a bedroom.
        cell = _room_effect_by_cell(rows, room)
        # The guard that says "a bedroom is not a third of a house" lived in
        # _room_effect(), and when the method changed to this comparison nothing
        # called it any more. So a figure worth half the suburb's median printed
        # as fact: +$950k a bedroom and +$730k a bathroom on a $1.79M median,
        # where the ceiling is $626k. Above the bar something the comparison
        # cannot see — street, condition, a run of new builds — is riding on the
        # room count, and saying so is the honest answer.
        too_big = (cell.dollars is not None and median_sold
                   and abs(cell.dollars) > ROOM_MAX_SHARE * median_sold)
        d = None if too_big else _round_to(cell.dollars)
        eff = MarginalEffect(key=key, dollars=d, low=None, high=None,
                             n_sales=cell.n or None,
                             basis="cell" if d is not None else None,
                             note=("sales here are too varied to separate this "
                                   "from the house") if too_big else cell.note)
    ...
    for m in ("auction", "negotiation", "tender", "deadline", "fixed"):
        ...
        if not got:
            continue
        # Same arm's-length guard the property page applies. Without it a sale
        # at 6% of CV — a part share, a transfer between relatives, a bad record,
        # anything but a market transaction — counts as evidence about how a
        # method performs. With one tender sale in a suburb that single row IS
        # the median, and the panel reported tender at -93.4% vs CV and ranked
        # the methods on it.
    ...
    # The window ENDS THIS MONTH. Never later.
    #
    # It used to end at the later of "now" and the newest month in the data, so
    # that a quiet recent month showed as a genuine zero rather than vanishing —
    # a quiet month IS the signal when you are asking which way the market is
    # going. That half still holds, and taking "now" as the end keeps it.
    #
    # What it must not do is follow the data into the FUTURE. A sold file
    # carrying a date months ahead — a typo, a settlement date, a bad export —
    # stretched the window to that month, so the chart ran to "Dec 26 · part
    # month" in August and the three "most recent completed months" behind the
    # verdict were months that have not happened. A handful of stray rows then
    # decided whether the suburb read as improving or softening.
    window_end = current_month
    months = (_months_range(_shift_period(window_end, MONTH_WINDOW - 1), window_end)
              if by_month else [])
    # A pooled window still needs enough sales in it to be a median rather than
    # a coin toss. Six across three months is roughly two a month.
    monthly = _month_series(by_month, months, min_sales=MIN_MONTH_SALES)
    ...
    med_svc = _median(svc)
    # DAYS TO SELL, FROM WHICHEVER SOURCE HAS ONE.
    #
    # The sold file's own column is preferred when it is populated — those are
    # campaigns the portal measured. It is empty for most of the history, and
    # the tile read "—" next to a sold count of 183, which is the shape of a
    # broken screen rather than of a market with nothing to say about it.
    #
    # Falling back to what we watched ourselves: listings that appeared in our
    # book and later stopped answering. Same suburb spellings, so the two
    # figures describe the same place.
    median_days = _median(days)
    if median_days is None:
        seen = observed_days_to_sell(
            db, region,
            suburbs=_area_values(db, PropertyForSale, fs_batch, "suburb", name)
                    if fs_batch else [name],
            # The same narrowing the rest of the screen is under. A figure that
            # ignored it would describe apartments on a page showing houses.
            ptype=want)
        if seen:
            median_days = seen.median_days
    return SuburbStats(
        ...
        from_year=lo_year, to_year=hi_year, years_available=years_available,
    )
...
def suburbs(region: str = "Auckland",
            dataset: str = Query("any", pattern="^(any|for_sale|sold)$"),
            district: str | None = None,
            db: Session = Depends(get_db)) -> list[SuburbOption]:
    ...
    for kind, model, field in (("sold", PropertySold, "sold"),
        ...
        # Scope to what the calling screen can actually show. The sold archive
        # covers far more suburbs than any one week of listings, so a merged list
        # offers the properties page dozens of suburbs with nothing live in them
        # — pick one and the page goes blank, which reads as a broken filter.
        # The trends panel wants the merged list, because it reads sold data.
        if dataset != "any" and dataset != kind:
            continue
        # SOLD counts come from every live batch, not just the newest one.
        #
        # Sold history accumulates — each upload is a delivery, not the dataset —
        # so counting one batch reported a slice of the suburb as the suburb.
        # On screen that put "Mount Eden — 111 sold" in the picker beside a panel
        # reading "231 sold in 2026" and a room figure fitted on 143 sales: three
        # numbers for one thing, none of them agreeing, on the same page.
        # For-sale is different and stays on the active batch: that one IS a
        # weekly snapshot, and "live" means live now.
        if kind == "sold":
            batch_ids = live_sold_batch_ids(db, region)
            batch_id = batch_ids[0] if batch_ids else None
        else:
            batch_ids = [bid] if (bid := _active_batch(db, kind, region)) else []
            batch_id = batch_ids[0] if batch_ids else None
        if not batch_ids:
            continue
        q = (db.query(model.suburb, func.count(model.id))
             .filter(model.import_batch_id.in_(batch_ids)))
        # Narrow to the district in play. Choosing a district and then a suburb
        # outside it is two filters that cannot both be satisfied, and the screen
        # goes blank with nothing to say the two disagree. Offering only the
        # suburbs inside the chosen district makes that combination impossible to
        # build in the first place.
        if district:
            q = q.filter(model.district.in_(
                _area_values(db, model, batch_id, "district", district)))
...
# ---- the zones a developer works in --------------------------------------------
class ZoneOption(BaseModel):
    """One entry in the zoning picker."""
    zoning: str
    live: int = 0                # listings in this zone right now
    can_subdivide: int = 0       # of those, sites that can take another lot


@router.get("/zones", response_model=list[ZoneOption])
def zones(region: str = "Auckland",
          db: Session = Depends(get_db)) -> list[ZoneOption]:
    """Every zone the live batch actually covers, with how much is in it.

    Built from the data rather than from the zone table, for the same reason the
    suburb picker is: a list you cannot type wrong, which doubles as the answer
    to "what is in here". Ordered by how many sites in the zone can take another
    lot, because that is what somebody opening this list is looking for.
    """
    batch_id = _active_batch(db, "for_sale", region)
    if batch_id is None:
        return []
    rows = (db.query(PropertyForSale.zoning,
                     func.count(PropertyForSale.id),
                     func.sum(func.cast(PropertyForSale.can_subdivide, Integer)))
            .filter(PropertyForSale.import_batch_id == batch_id,
                    PropertyForSale.zoning.isnot(None),
                    PropertyForSale.zoning != "")
            .group_by(PropertyForSale.zoning).all())
    out = [ZoneOption(zoning=z, live=int(n or 0), can_subdivide=int(c or 0))
           for z, n, c in rows]
    out.sort(key=lambda o: (-o.can_subdivide, -o.live, o.zoning))
    return out
...
def export_for_sale_xlsx(
    ...
    A CSV of nine thousand rows is not an audit, it is nine thousand rows. This
    leads with a sheet of checks — each one a question, the answer beside it,
    and what the answer should look like — because every fault this system has
    shipped was visible in the data at the time and nobody could see it.

    Includes rows a customer CANNOT see, with a column saying why. Those are
    where a fault hides: held back at review, advertisement gone, a price the
    scraper invented. Filter on that column in Excel for either set.
    """
    from fastapi.responses import Response

    from audit_export import build

    data = build(db, region)
    stamp = datetime.now(timezone.utc).strftime("%d-%m-%Y")
    return Response(
        content=data,
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition":
                 f'attachment; filename="apex_live_data_{stamp}.xlsx"'},
    )
...
def export_for_sale_csv(
    ...
    """The live data as a CSV — the same rows and the same columns as the
    workbook, in the format a spreadsheet opens without thinking about it.

    THE COLUMNS COME FROM ONE PLACE. This export used to keep its own list, so
    the CSV and the workbook described the same batch with different fields and
    different headings, and adding a column to one silently left the other
    behind. They share audit_export.COLUMNS now, which is also what decides the
    order, so the two can no longer drift.

    Includes the listings a customer CANNOT see, with a column saying whether
    each row is visible and one saying why not — held at review, advertisement
    confirmed gone, a price the scraper invented. Filter on it in Excel and you
    have either set.
    """
    import csv
    import io
    ...
    from audit_export import COLUMNS, _aware, _visible_reason
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition":
                 f'attachment; filename="apex_live_data_{stamp}.csv"'},
    )
...
def _is_placeholder_asking():
    """A price the scraper invented rather than a vendor named.

    Never applied to a price we derived ourselves: asking_basis records where
    the number came from, so guessing at its provenance on top of that is both
    unnecessary and wrong. It matters because the carried-forward figure is
    rounded to the nearest $10,000 and lands on a round council valuation often
    enough to hide exactly the listings the carry-forward exists to rescue.
    """
    ask = PropertyForSale.asking_price
    cv = PropertyForSale.cv_numeric
    ls = PropertyForSale.valuation_last_sold_value
    ours = PropertyForSale.asking_basis
    derived = and_(ours.isnot(None), ours != ADVERTISED_BASIS)
    return and_(
        not_(derived),
        or_(
...
def _hide_bad_data(q):
    ...
    during the pre-publish review, any placeholder-asking ("fake") listing whose
    price was guessed off the CV or last sale, and any row claiming a margin
    above the believable ceiling — belt-and-suspenders so a fake listing can
    never surface even if it wasn't held.

    The margin ceiling is here as well as in the publish gate for two reasons:
    batches published before that gate existed are still live and still carry
    those rows, and the hero card on the deal page picks the single largest
    margin in the batch — so without it the most broken row in the data is the
    headline. (+206.70%, +2,296.9%: every one a council record that valued the
    land and not the house.)"""
    return q.filter(
        PropertyForSale.is_held.is_(False),
        # The advertisement is gone. Whatever happened to it — sold, withdrawn,
        # relisted elsewhere — sending somebody from our site to a page that is
        # not there is the one outcome we can definitely avoid, so the row comes
        # off every list the moment a check confirms the link is dead. Set by
        # portals/delisted.py and cleared by it the moment the link answers
        # again, so a portal's bad afternoon costs a listing one day, not its
        # place in the product.
        PropertyForSale.link_dead_at.is_(None),
        # The house is in the sold file with a sale newer than the listing.
        # Stronger evidence than a dead link and it arrives for the houses the
        # link check can never catch — a portal leaves a sold page up with a
        # banner on it, answering 200 for ever. Set by sold_sweep.py.
        PropertyForSale.sold_seen_at.is_(None),
        not_(_is_placeholder_asking()),
        or_(PropertyForSale.margin.is_(None),
            PropertyForSale.margin <= MARGIN_MAX_PCT),
        or_(
...
def _area_values(db: Session, model, batch_id: int, column: str, name: str) -> list[str]:
    ...
    try:
        rows = [v for (v,) in db.query(col)
                .filter(model.import_batch_id == batch_id).distinct().all() if v]
    except Exception:
        return [name]

    hits = [v for v in rows if v.strip().lower() == want]
    if hits:
        return hits

    # Still nothing. The two feeds may not spell an area the same way: a sold
    # archive that says "Remuera" against listings that say "Remuera, Auckland"
    # is one suburb written two ways, and an exact comparison calls them
    # different places. The picker is built from one vocabulary and the filter
    # runs against the other, so EVERY option matches nothing — which is not an
    # empty result, it is a control that appears dead.
    #
    # Compare only the part before the first comma, which is the suburb itself
    # with any region qualifier removed. Deliberately a FALLBACK: it can never
    # widen a filter that already matched, and it cannot confuse two suburbs,
    # because "Mount Eden" and "Mount Albert" still differ once the qualifier is
    # gone.
    head = want.split(",")[0].strip()
    hits = [v for v in rows if v.split(",")[0].strip().lower() == head]
    if hits:
        log.info("area %r matched %d stored value(s) only after dropping the "
                 "region qualifier — the feeds spell this differently", name, len(hits))
        return hits

    # Nothing matched at all — pass the original through so the filter still
    # excludes everything, rather than silently becoming "no filter".
    return [name]
...
def _filtered_query(
    ...
    *,
    ...
    if subdividable is not None:
        # FEASIBILITY, not profitability. Finding development land is what this
        # product is for, and a 1,200 m2 freehold site in Mixed Housing Urban is
        # development land whether or not a vendor has named a price — four in
        # five have not. Filtering on the profit flag hid every one of them from
        # the people specifically looking for them.
        #
        # Worth subdividing is still its own answer: is_subdividable drives the
        # buy score and the deal badge, and subdivision_profit is right there to
        # sort on. What a developer decides is theirs to decide; what they
        # cannot do is judge a site we never showed them.
        q = q.filter(PropertyForSale.can_subdivide.is_(subdividable))
    if zoning:
        # Exact zone names, any of several — a developer works in the zones they
        # know, and "Mixed Housing Urban or Suburban" is one search, not two.
        wanted = [z.strip() for z in str(zoning).split(",") if z.strip()]
        if wanted:
            q = q.filter(PropertyForSale.zoning.in_(
                [v for w in wanted
                 for v in _area_values(db, PropertyForSale, batch_id, "zoning", w)]))
    ...
    if min_price is not None:
        q = q.filter(BUDGET_PRICE >= min_price)
    if max_price is not None:
        q = q.filter(BUDGET_PRICE <= max_price)
...
# What a deal list can be ordered by. ONE definition, used by the row list, by
# the summary that picks the headline card, and by the validation on both — so
# a page cannot be sorted on an axis the hero card has never heard of, which is
# exactly how the subdividable page came to be headed by its biggest
# underpricing while the rows underneath were ordered by subdivision profit.
SORTABLE = ("opportunity_score_pct|asking_price|market_value|fair_value|"
            "buy_price|cash_on_cash|breakeven_deposit_pct|max_addl_lots|"
            "best_net_gain|subdivision_profit|predicted_days|days_on_market|"
            "address|margin|margin_dollars")
_SORT_PATTERN = f"^({SORTABLE})$"


# The furthest anyone can page. There is a floor on `page` and there was no
# ceiling, so a number typed into the address bar went straight through
# validation and became an OFFSET: page=999999999999999999999 multiplied by the
# page size overflows what the database can hold, and the endpoint answers 500.
# Postgres rejects an out-of-range bigint offset exactly as SQLite does, so this
# is a public URL that returns a server error to anyone who edits a digit.
#
# 10,000 pages at the maximum page size is five million listings — past any
# batch this will ever hold, and small enough that the offset stays ordinary.
MAX_PAGE = 10_000


def _sort_expression(order_by: str):
    """The column a deal list is ordered by.

    "margin_dollars" is the dollar gap (fair value − asking) and is computed on
    the fly; everything else is a real column.
    """
    if order_by == "margin_dollars":
        return case((PropertyForSale.margin.isnot(None),
                     PropertyForSale.fair_value - PropertyForSale.asking_price),
                    else_=None)
    return getattr(PropertyForSale, order_by)
...
def summarise_for_sale(
    ...
    order_by: str = Query("margin", pattern=_SORT_PATTERN),
    db: Session = Depends(get_db),
) -> ForSaleSummary:
    batch_id = _active_batch(db, "for_sale", region)
    if batch_id is None:
    ...
    dollars = [r.fair_value - r.asking_price for r in rows
               if r.margin is not None and r.fair_value is not None and r.asking_price is not None]
    lots = [r.max_addl_lots for r in rows]

    # The hero card shows the single sharpest deal, on the SAME axis the list
    # below it is sorted by — the same expression, from the same function, so
    # the two cannot drift apart.
    #
    # They had. This was a two-case guess: max_addl_lots, or else margin. The
    # subdividable page sorts by best_net_gain, which is neither, so it fell
    # through to margin — and the headline card on that page was the biggest
    # UNDERPRICING in the list rather than the biggest subdivision profit, with
    # the rows underneath ordered by something else entirely. The top of the
    # page disagreed with the page.
    top_row = (q.with_entities(PropertyForSale.id)
               .order_by(desc(_sort_expression(order_by)).nullslast())
               .first())
    top = top_row[0] if top_row else None
...
class MapPoint(BaseModel):
    ...
    # How it is being sold, when there is no price to show. A pin that says
    # "Asking: —" claims we do not know; one that says "Auction" is the truth
    # about the listing and the reason there is no number beside it.
    listing_type: str | None = None   # for-sale only: auction | tender | negotiation | fixed
...
def map_points(
    ...
    district: str | None = None,
    search: str | None = None,
    sold_months: int | None = Query(SOLD_MAP_MONTHS, ge=1, le=120),
    ...
    `sold_months` bounds how far back those sales reach — six by default. Every
    sale a suburb has ever recorded is 2,950 dots on one map, which is not a
    picture of a market, it is a texture. Ignored for dataset=for_sale, where
    every point is live by definition. Pass a bigger number for a longer view;
    the frontend offers 3 / 6 / 12 / 24 / all.""" 
    P = PropertyForSale
    if dataset == "sold":
        # Every live sold batch, like every other reader of sold data. Pinned to
        # the newest one, the map showed a slice: "Sold sale · 111" in the legend
        # beside a panel counting 231 sales in the same suburb.
        sold_ids = live_sold_batch_ids(db, region)
        batch_id = sold_ids[0] if sold_ids else None
        if not sold_ids:
        ...
        q = db.query(
            ...
            S.import_batch_id.in_(sold_ids),
        ...
        # Recency window. sold_date is a STRING and the feed carries two formats
        # ("5/14/2026" and "2026-05-14"), so the exact test has to be done in
        # Python by sold_year_month. What CAN be done in SQL is the year: it
        # appears verbatim in both formats, so a LIKE on the two or three years
        # the window spans cuts a region-wide query down before it is fetched,
        # and the precise month filter then runs over a small set.
        # Three cases, and they are genuinely different:
        #   an int          -> that many months
        #   None            -> no window, every sale on record (direct callers)
        #   anything else   -> an unpassed Query(6) default, so use the default
        #
        # isinstance rather than truthiness, because called directly as a
        # function the unpassed default arrives as the Query OBJECT, which is
        # truthy and then fails the first time it meets arithmetic. This
        # codebase has been bitten by exactly that on `limit` already.
        if isinstance(sold_months, int):
            window = sold_months
        elif sold_months is None:
            window = None
        else:
            window = SOLD_MAP_MONTHS
        wanted = _recent_months(window) if window else None
        if wanted:
            years = {m[:4] for m in wanted}
            q = q.filter(or_(*[S.sold_date.like(f"%{y}%") for y in sorted(years)]))
        rows = q.limit(limit).all()
        if wanted:
            rows = [r for r in rows if _in_months(r.sold_date, wanted)]
        points = [
    ...
    points = [
        MapPoint(
            ...
            suburb=r.suburb, price=r.asking_price, listing_type=r.listing_type,
...
def external_estimates(property_id: int, db: Session = Depends(get_db)) -> ExternalEstimates:
    ...
    def _fresh(stamp, ttl: timedelta) -> bool:
        """Was this checked within `ttl`? Works whatever the driver hands back.

        The column is DateTime(timezone=True), and what comes out of it depends
        on the database: Postgres returns an aware datetime, SQLite a naive one.
        Subtracting a naive from an aware raises TypeError, so this endpoint
        500'd on every property that had ever been checked — everywhere except
        production, which is the worst place for a bug to hide. A stored stamp
        is always UTC, so a naive one is simply told so.
        """
        if not stamp:
            return False
    ...
    if p.address and not _fresh(p.homes_checked_at, ttl):
    ...
    if p.address and not _fresh(p.pv_checked_at, pv_ttl):
...
class ScenarioIn(BaseModel):
    ...
    # How many lots the site really takes.
    #
    # Every other field here is an assumption about COST. This one is about the
    # site, and it is the one the model is least able to see: the count is land
    # area divided by the zone minimum less a road allowance, which cannot know
    # about a corner, an existing right of way, a boundary that already suits
    # three, or a consent somebody has been granted. Profit is close to linear
    # in the count, so a model that is one lot low understates the deal by about
...
class ScenarioOut(BaseModel):
    ...
    # The plan NOT chosen, and what it would have earned. A headline that says
    # "demolish" is a big thing to tell somebody; showing the number it beat is
    # what makes it an argument rather than an instruction.
    alternative_strategy: str | None = None
    alternative_profit: float | None = None
    demolish: bool = False                   # the headline plan knocks the house down
...
def subdivision_scenario(
    ...
    return ScenarioOut(
        ...
        alternative_strategy=getattr(sd, "alternative_strategy", None),
        alternative_profit=getattr(sd, "alternative_profit", None),
        demolish=bool(getattr(sd, "demolish", False)),
        ...
        defaults={
            ...
            "demolition_allowance": defaults.demolition_allowance,
...
def get_for_sale(property_id: int, db: Session = Depends(get_db)) -> PropertyForSale:
    ...
    # THE SAME TWO RULES THE LIST APPLIES. This endpoint used to return whatever
    # row carried that id, and it is the one a bookmark, a shared link or a
    # guessed number points at. Two things got through:
    #
    #   A HELD LISTING. An operator presses Hide on a listing that is wrong, it
    #   disappears from every list, and the direct link still served it —
    #   impossible margin and all. Hide did not hide.
    #
    #   A BATCH THAT HAS NEVER GONE LIVE. A staged or preview row has an
    #   ordinary sequential id, so next week's unreviewed data was readable by
    #   any signed-in customer who typed a number. That is worse than publishing
    #   something wrong: it is publishing without deciding to.
    #
    # Re-asked as a filtered query rather than checked field by field, so this
    # cannot drift from what the browse list hides. _active_batch honours
    # preview mode, so an admin previewing still sees the preview batch and a
    # customer still cannot.
    if active_batch_id is None:
        raise HTTPException(status_code=404, detail="Not found")
    visible = _hide_bad_data(
        db.query(PropertyForSale)
        .filter(PropertyForSale.id == p.id,
...
class MethodMixCoverage(BaseModel):
    """What the sell-method table could NOT account for, and why.

    The table only ever showed sales that had a usable council CV *and* a
    recognised method code. Everything else vanished without trace, so the
    counts never reconciled with the suburb's sale count and the gap read as a
    method nobody had named. Reporting the shortfall turns "the rest must be
    private treaty" from a guess into something the screen can answer.
    """
    total: int = 0             # sold records in the suburb with a sale price
    shown: int = 0             # rows behind the table's method rows
    no_cv: int = 0             # no council CV to measure a sale against
    implausible_cv: int = 0    # sale/CV outside 0.3-3.0 — a broken CV, not a sale
    no_method: int = 0         # no sale method recorded on the row at all
    # Of the rows shown, how many carry an INFERRED method rather than a
    # recorded one. Surface this wherever the negotiation figure is quoted: a
    # split resting mostly on inference is a weaker basis for an offer than one
    # measured from labelled sales, and the table cannot show the difference.
    inferred: int = 0


class MethodShare(BaseModel):
    method: str            # "Auction" / "By negotiation" / "Tender"
    count: int
...
class ComparablesResponse(BaseModel):
    ...
    # What that split could not account for. Without it the table's counts do
    # not reconcile with the suburb's sale count and the difference looks like
    # an unnamed method.
    suburb_method_coverage: MethodMixCoverage | None = None
...
# One definition, because this string is not only displayed — the auction-vs-
# negotiation gap looks the bucket up BY LABEL. Written out twice, renaming it
# in one place would leave that lookup silently finding nothing, and the gap
# would just stop appearing with nothing to say why.
#
# "By negotiation" rather than "Price by Negotiation": the bucket holds every
# negotiated sale plus every row with no method recorded, which is private
# treaty in the broad sense. PBN specifically means a listing advertised with no
# price, and claiming that precision would overstate what the data says.
NEGOTIATION_LABEL = "By negotiation"

_METHOD_LABELS = {"A": "Auction", "P": NEGOTIATION_LABEL, "T": "Tender"}

# A sale with no recorded method is counted as private treaty.
#
# Auction, tender and private treaty are effectively the whole universe of ways
# a house changes hands here, and the two that get recorded are the two that are
# publicly scheduled — so an unlabelled sale is overwhelmingly likely to be a
# negotiated one. The newer export carries no method field at all, which would
# otherwise send every row it loads to "Unknown" and out of the table.
#
# It remains an INFERENCE, and it moves a number people negotiate against, so
# the count of inferred rows is reported alongside (MethodMixCoverage.inferred)
# rather than folded in silently.
_UNRECORDED_METHOD = NEGOTIATION_LABEL


def _method_label(raw: str | None) -> str:
    return _METHOD_LABELS.get((raw or "").split("-")[0].strip().upper(),
                              _UNRECORDED_METHOD)


def _method_recorded(raw: str | None) -> bool:
    """Did the row actually carry a method, or are we inferring one?"""
    return (raw or "").split("-")[0].strip().upper() in _METHOD_LABELS
...
def _method_mix_for_suburb(db: Session, sold_batch: int | None, suburb: str | None):
    ...
    if not sold_batch or not suburb:
        return [], MethodMixCoverage()
    # EVERY sold batch for the region, not just one. Sold history accumulates
    # now, so keying this to a single batch showed a slice of the suburb's
    # sales and called it the suburb.
    region = db.query(ImportBatch.region).filter(ImportBatch.id == sold_batch).scalar()
    ...
    rows = db.query(PropertySold.sale_method, PropertySold.sale_price,
        ...
        PropertySold.import_batch_id.in_(batch_ids),
        PropertySold.suburb.in_(_area_values(db, PropertySold, sold_batch, "suburb", suburb)),
        PropertySold.sale_price.isnot(None),
    ).all()

    # Three filters used to drop rows in silence, so the table's counts never
    # added up to the suburb's sale count and nothing said why. A reader seeing
    # "66 auctions" against 111 sales can only guess what the other 45 were —
    # and guessing "they must be negotiation" is exactly the wrong conclusion
    # when the real answer is that the export carries no sale method at all.
    cover = MethodMixCoverage(total=len(rows))
    ...
    for method, price, cv in rows:
        ...
        if not (0.3 <= ratio <= 3.0):     # broken council CVs
            cover.implausible_cv += 1
            continue
        if not _method_recorded(method):
            cover.no_method += 1
            cover.inferred += 1
    ...
    cover.shown = sum(m.count for m in out)
    return out, cover
...
def property_comparables(property_id: int, db: Session = Depends(get_db)) -> ComparablesResponse:
    ...
    # Both stored spellings. '%/2024' matches the scraper's M/D/YYYY; '2024-%'
    # matches the ISO form ingest writes now. Matching only the first returned
    # zero comparables for every listing the moment sale dates were
    # canonicalised — the panel went blank with nothing logged, because an empty
    # comp set is indistinguishable from a genuinely thin suburb.
    _RECENT_YEARS = ("2024", "2025", "2026")
    # Every sold delivery, not the newest. A batch is a delivery and sold
    # history accumulates: reading one of them drew the comparable sales on a
    # property page out of whichever file happened to arrive last, which is
    # indistinguishable on screen from a thin suburb.
    _sold_ids = live_sold_batch_ids(db, p.region or "Auckland") or [sold_batch]
    base_all = (
        db.query(PropertySold)
        .filter(PropertySold.import_batch_id.in_(_sold_ids))
        .filter(PropertySold.suburb == p.suburb)
        .filter(PropertySold.sale_price.isnot(None))
        .filter(PropertySold.sale_price >= 50_000)
        .filter(or_(*(
            [PropertySold.sold_date.like(f"%/{y}") for y in _RECENT_YEARS]
            + [PropertySold.sold_date.like(f"{y}-%") for y in _RECENT_YEARS]
        )))
    )
    ...
    # Pools, on the same principle as title: a house with one and a house
    # without are not the same product, and blending them puts a number on
    # screen that the reader cannot check. Same status when there are enough of
    # them; otherwise mixed, and said so.
    subject_pool = detect_pool(p)
    same_pool = [c for c in comps if bool(c.has_swimming_pool) == subject_pool]
    if len(same_pool) >= 3 and len(same_pool) < len(comps):
        comps = same_pool
        matched_using["pool"] = "with a pool" if subject_pool else "no pool"
    elif any(bool(c.has_swimming_pool) != subject_pool for c in comps):
        matched_using["pool"] = "mixed"
    ...
    def shape(c: PropertySold) -> ComparableSale:
        ...
        return ComparableSale(
            ...
            has_pool=bool(c.has_swimming_pool),
    ...
    for c in comps:
        label = _method_label(c.sale_method)
    ...
    for label, rs in by_method.items():
        n = sum(1 for c in comps
                if _method_label(c.sale_method) == label)
        med = _q(sorted(rs), 0.5)
        # count is every comp that sold this way; the percentage can only come
        # from those with a usable CV. Flag on the SECOND number — a row reading
        # "5 sales, -12%" where the -12% is one property is the kind of thing a
        # buyer takes to a negotiation.
    ...
    smix, scover = _method_mix_for_suburb(db, sold_batch, p.suburb)
    _by = {m.method: m for m in smix}
    auc, neg = _by.get("Auction"), _by.get(NEGOTIATION_LABEL)
...
def property_value_add(property_id: int, db: Session = Depends(get_db)) -> ValueAddResponse:
    ...
    ).filter(PropertySold.import_batch_id.in_(
                 live_sold_batch_ids(db, p.region or "Auckland") or [sold_batch]),
    ...
    ups = value_add(
        ...
        # Detected here as well as at ingest, so a listing loaded before the
        # text was read is not offered a pool it already has.
        has_pool=detect_pool(p),
...
def list_sold(
    ...
    # The whole sold history, not the newest delivery. A list that silently
    # showed one file's worth of sales looked like a small dataset rather than
    # like a filtered one.
    q = db.query(PropertySold).filter(PropertySold.import_batch_id.in_(
        live_sold_batch_ids(db, region) or [batch_id]))
    if suburb:

...
# copy — see addresses.py for what that cost. One definition now.
...
# A number the vendor published as a FLOOR rather than as a price — "Enquiries
# over $699,000", "Offers over", "Buyer enquiry over". It is a real advertised
# figure, put there by the agent and printed on the listing, so it belongs in
# the price field: showing nothing for 10 Fernbird Place when the advertisement
# says $699,000 is withholding something the vendor said out loud.
#
# It is not a firm asking price, and the difference is not decoration. The house
# will sell ABOVE it — that is what the wording is for — so a margin measured
# against it flatters every one of these listings, and the valuation must not be
# anchored to it either. The number is shown; the deal is not.
GUIDE_BASIS = "enquiries over"

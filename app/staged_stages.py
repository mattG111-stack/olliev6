...
def run_enrich_job(job_id: int, batch_id: int, region: str,
    ...
    try:
        ...
        print(f"  [enrich] {len(todo)} rows to look up "
              f"({_from_held} of them held for a data gap a lookup can fill)")
        del scan
        need = len(todo)
        ...
        for pid, address, suburb in todo:
            ...
            if status == PV_BLOCKED:
                ...
                for wait in BLOCK_BACKOFF:
                    _update(db, job_id, stage=f"rate-limited, waiting {wait}s")
                    time.sleep(wait)
                    try:
                        pv, status = pv_lookup_status(q)
                    except Exception:

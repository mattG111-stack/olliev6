...
_REGION_ALIASES = {
    ...
    "waikato": {"waikato", "waikato region", "怀卡托"},
    "wellington": {"wellington", "wellington region", "惠灵顿"},
    "canterbury": {"canterbury", "canterbury region", "坎特伯雷"},
    "bay of plenty": {"bay of plenty", "bayofplenty", "丰盛湾"},
    "northland": {"northland", "northland region", "北地"},
}
...
def _region_key(value) -> str:
    ...
    for canon, spellings in _REGION_ALIASES.items():
        if s in spellings or s.replace(" ", "") in {x.replace(" ", "") for x in spellings}:
            return canon
    return s


def region_matches(value, target: str) -> bool:
    """Is this row's region the one being loaded? A blank region is accepted —
    it means the feed did not say, not that it said somewhere else."""
    if value is None or str(value).strip() == "":
        return True
    return _region_key(value) == _region_key(target)
    ...
    *,
    ...
    if rejected_reasons:
        ...
        extra_note_parts.append(
            "rejected — " + "; ".join(
                f"{v:,} {_WHY.get(k, k)}" for k, v in _ranked))

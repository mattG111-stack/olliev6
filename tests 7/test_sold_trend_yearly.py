...
from routers.dashboards import _sold_trend_json
...
def _years(db, suburb="Remuera", region="Auckland"):
    yearly, _monthly, _cov = _sold_trend_json(db, suburb, region)
...
def test_a_year_held_back_for_being_thin_says_so(db_session):
    """Silence is what made a missing current year look like a broken chart.

    A year with fewer sales than the bar is still not plotted — two sales are
    not a median — but the payload now carries what was held back and why, so
    the page can say "2026 isn't on the line yet, 5 sales so far" instead of
    just ending early.
    """
    _seed(db_session, {2022: _flat(1_100_000), 2023: _flat(1_200_000),
                       2024: _flat(1_300_000), 2025: _flat(1_400_000),
                       2026: _flat(1_500_000, n=5)}, last_month=8)
    yearly, _m, _cov = _sold_trend_json(db_session, "Remuera", "Auckland")
    data = json.loads(yearly)
    assert [p["year"] for p in data["points"]] == [2022, 2023, 2024, 2025]
    assert data["withheld"] == {"year": 2026, "sales": 5, "min": 8}


def test_nothing_is_reported_withheld_when_the_newest_year_is_on_the_chart(db_session):
    _seed(db_session, {2023: _flat(1_200_000), 2024: _flat(1_300_000),
                       2025: _flat(1_400_000), 2026: _flat(1_500_000)},
          last_month=8, suburb="Epsom")
    yearly, _m, _cov = _sold_trend_json(db_session, "Epsom", "Auckland")
    data = json.loads(yearly)
    assert data["points"][-1]["year"] == 2026
    assert "withheld" not in data


def test_two_years_of_our_own_sales_is_enough_to_draw(db_session):
    """The Massey case: sold history that starts last year.

    This chart answers one question — is this year above last year — and two
    years is exactly what that takes. Demanding a third sent the suburb to the
    portal's aggregate, which stops whenever the scrape did, so the reader was
    shown a line ending in 2025 while their own 2026 sales sat in the database.
    """
    _seed(db_session, {2025: _flat(1_000_000), 2026: _flat(1_100_000)},
          last_month=8, suburb="Massey")
    yearly, _m, coverage = _sold_trend_json(db_session, "Massey", "Auckland")
    assert yearly, "two years of our own sales drew nothing"
    pts = json.loads(yearly)["points"]
    assert [p["year"] for p in pts] == [2025, 2026]
    assert pts[-1]["partial"] is True
    # The coverage figures are computed either way; the endpoint only passes
    # them to the page when it had to fall back, so they can explain why.
    assert coverage == {"sales": 20, "years": 2, "need_years": 2, "need_per_year": 8}


def test_one_year_is_still_not_a_trend_but_says_what_it_has(db_session):
    """One year cannot show a direction, so the portal series is still used —
    and the page can now say why instead of just ending early."""
    _seed(db_session, {2026: _flat(1_100_000, n=12)}, last_month=8,
          suburb="Hobsonville")
    yearly, _m, coverage = _sold_trend_json(db_session, "Hobsonville", "Auckland")
    assert yearly is None
    assert coverage == {"sales": 12, "years": 1, "need_years": 2, "need_per_year": 8}


def test_no_sold_records_at_all_reports_nothing_rather_than_zero(db_session):
    """Zero would read as "we have data and it is empty", which is different."""
    _seed(db_session, {2025: _flat(1_000_000)}, last_month=8, suburb="Massey")
    _yearly, _m, coverage = _sold_trend_json(db_session, "Nowhere", "Auckland")

...
from pricing.assumptions import (CV_IMPLAUSIBLE_VS_ASKING,
                          MAX_VALUE_VS_ASKING, MIN_VALUE_VS_ASKING)
from pricing.v38_loader import TABLES, V38Tables
...
def title_code(raw) -> int:
    ...
    # "Fee simple" IS freehold — the same estate, the older legal name for it,
    # and what a fair amount of real listing data actually says. This mapping is
    # the one every other title rule ends up going through, and it did not know
    # the word, so those rows were classed "Other": priced as an unknown estate,
    # and — because the subdivision model asks this same question — never
    # subdividable, so their property pages showed no subdivision figures and no
    # way to edit them. ml/features.py already knew the synonym, which is how
    # the valuation and the subdivision model came to disagree about the same
    # house.
    if s in ("freehold", "fee simple", "fee-simple", "1"):
        return 1
    if s in ("leasehold", "2"):
        return 2
...
def _features(*, cv: float, floor: float, land: float | None, beds: float, baths: float,
    ...
    return [
        ...
        # β4. The +1.0 handles a land area of zero; it does NOT handle a
        # negative one, and log(-4) raises rather than returning anything. That
        # is not a bad number in one row, it is the whole load dead on the row
        # that carries it — the same failure that took a 146 MB ingest down.
        #
        # The two lines above already say what to do with a size that makes no
        # sense: treat it as not recorded. A negative area is not a small area,
        # it is a parse artefact, and this now agrees with them.
        math.log(max(float(land or 0.0), 0.0) + 1.0),  # β4
        float(beds or 0),                             # β5
        float(baths or 0),                            # β6
        float(cars or 0),                             # β7
        float(age or 0),                              # β8
...
class Prediction:
    ...
    market_value is this model's own answer. IT IS NOT WHAT THE CUSTOMER SEES —
    it was, when this was written, and the sentence that used to say so is the
    reason the confidence band ended up printed beside a different figure for
    months. What a customer reads as "Our valuation" is pipeline's fair_value,
    which is CV-anchored and can differ from this by a long way.

    range_low / range_high give an honest band around MARKET_VALUE — ±4% on the
    asking path, ±14% on the v3.5 fallback path (per client spec, June 2026).
    Because the band is a percentage, pipeline rescales it onto the published
    figure before anyone sees it (_rescale_band). Anything reading these two
    fields directly is reading a band for market_value and must say so.
    """
...
def predict(
    *,
    ...
    if cv_f > 0 and not is_bare_land:
        ...
        # exp() of a large enough sum RAISES rather than returning infinity, and
        # an exception here does not spoil one valuation — it ends the load, the
        # same way a format string on a None did.
        #
        # Infinity is the honest answer, and it needs no new handling: the sanity
        # gate below already refuses anything that has run away from the CV, so
        # this rejoins the path a merely-absurd number already takes and comes
        # out as no valuation with hedonic_implausible_vs_cv against it.
        try:
    ...
    if fair_value is not None and asking_f > 0:
        ...
        if not (MIN_VALUE_VS_ASKING * asking_f <= fair_value
                <= MAX_VALUE_VS_ASKING * asking_f):
            fair_value = None
            warnings.append("fair_value_implausible_vs_asking")
        elif cv_f > CV_IMPLAUSIBLE_VS_ASKING * asking_f:

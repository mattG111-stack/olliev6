...
async def lifespan(app: FastAPI):
    ...
    # worker.py, or behind the Price button. AUTO_REPRICE_ON_BOOT=1 brings it
    ...
    if os.environ.get("AUTO_REPRICE_ON_BOOT", "").lower() in ("1", "true", "yes"):
        ...
        log.warning("auto re-price running INSIDE the API on boot — "
                    "this is the job that OOM-killed 8.997; see worker.py")

    # Scheduled work belongs in the worker process (worker.py), not here.
    ...
    if os.environ.get("PORTALS_IN_API", "").lower() in ("1", "true", "yes"):
        try:
            ...
            start_daily_portals()
            log.warning("daily portal pass running INSIDE the API — see worker.py")
        except Exception:

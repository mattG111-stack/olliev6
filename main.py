...
from .routers import (activity, admin_metrics, admin_upload, assistant, auth,
                      billing, bugs, customer_intel, dashboards, geo, maps,
                      offmarket, preferences, promoters, properties, release,
                      wishlists)
...
async def lifespan(app: FastAPI):
    ...
    # ...and every COLUMN the models declare that its table lacks.
    #
    # create_all above builds missing tables and will not alter one that already
    # exists, so a column added to an existing model reached production only
    # through db_bootstrap — the module this same comment block notes is never
    # run here. Four tm_valuation columns went in that way and took three pages
    # down at once, because every ORM query selects every mapped column:
    #
    #   ProgrammingError: column properties_for_sale.tm_valuation does not exist
    #
    # Derived from the models, so it covers the next column too, and nobody has
    # to remember anything.
    try:
        from .db_bootstrap import ensure_columns

        added = ensure_columns(engine)
        if added:
            log.warning("added %d missing column(s): %s", len(added), ", ".join(added))
    except Exception:
        log.exception("could not ensure the columns")
    ...
    # Say, in the boot log, whether this deploy can actually serve — and if not,
    # which part is wrong. Three days of instability had four different causes
    # and not one of them announced itself; each cost a round of guessing before
    # a fix that took a minute. Never fatal: a degraded app that can explain
    # itself beats a dead one that cannot.
    try:
        from .preflight import check as _preflight, ready as _ready

        checks = _preflight()
        for c in checks:
            (log.warning if not c.ok else log.info)("preflight %s", c.line())
        log.warning("preflight: %s", "READY" if _ready(checks) else "NOT READY")
    except Exception:
        log.exception("preflight could not run")
    ...
    #
    # It re-prices every staged batch: ten thousand listings against the whole
    # sold history, through pandas and the full pricing pipeline. That is the
    # heaviest job the system has, and it was running inside the API container
    # on EVERY boot. The symptom is not a stack trace — the app starts, reports
    # preflight READY, begins serving, and is then killed from outside while the
    # thread is in pandas. What reaches the log is a CancelledError from the
    # lifespan being torn down, which names nothing and points at nothing.
    #
    # It cost a deploy on 8.997. The comment fifteen lines below already says
    # the same thing about the portal pass — "a long, memory-hungry job ... is
    # the opposite shape, and sharing a container means every one of those is a
    # way for the site to go down" — and this was exactly that, three lines
    # above it, started unconditionally.
    #
    # The work is still worth doing; it belongs where the portal pass went, in
    # app/worker.py, or behind the Price button. AUTO_REPRICE_ON_BOOT=1 brings it
    # back for a deployment that would rather take the risk.
    if os.environ.get("AUTO_REPRICE_ON_BOOT", "").lower() in ("1", "true", "yes"):
        from .staged_stages import auto_reprice_stale_batches
        threading.Thread(target=auto_reprice_stale_batches, daemon=True).start()
        log.warning("auto re-price running INSIDE the API on boot — "
                    "this is the job that OOM-killed 8.997; see app/worker.py")

    # Scheduled work belongs in the worker process (app/worker.py), not here.
    # The API's job is to answer requests; a long, memory-hungry job that
    # reaches out to other people's servers is the opposite shape, and sharing a
    # container means every one of those is a way for the site to go down. It
    # already has — see the OOM note in staged_stages.run_enrich_job.
    #
    # PORTALS_IN_API exists for a deployment that has no second service and
    # would rather take the risk than have no daily pass at all.
    if os.environ.get("PORTALS_IN_API", "").lower() in ("1", "true", "yes"):
        try:
            from .portals.daily import start as start_daily_portals

            start_daily_portals()
            log.warning("daily portal pass running INSIDE the API — see app/worker.py")
        except Exception:
            log.exception("could not start the daily portal pass")
    ...
    try:
        ...
        #
        # A container killed from outside — OOM, a failed healthcheck, a
        # platform restart — tears the lifespan down and leaves an
        # asyncio.CancelledError with no message, no frame of ours in the
        # traceback, and nothing to act on. That is what 8.997 left behind after
        # the boot-time re-price ate the memory: a stack trace that named
        # Starlette and said nothing about us.
        #
        # A clean shutdown looks identical in the log. So it is worth one line
        # to separate them, with how long the process lived — seconds means a
        # crash loop, minutes after a deploy means something killed it.
        log.warning("shutting down after %.0fs — %s. If this was not a deploy, "
                    "the process was killed from outside (memory limit or "
                    "healthcheck); check the memory graph.",
                    time.monotonic() - started, type(exc).__name__)
        raise
    else:
        log.warning("shutting down cleanly after %.0fs", time.monotonic() - started)


# THE INTERACTIVE DOCUMENTATION IS OFF IN PRODUCTION.
#
#     "nothing should be public"
#
# FastAPI serves /docs, /redoc and /openapi.json to anyone who asks, and nothing
# had turned them off. The specification was 263 KB describing all 151 endpoints
# — every admin route, every parameter, every field name — handed to anybody who
# typed the address. That is a map of the whole system, and of the parts a
# customer is not supposed to know exist.
#
# Off by default, and ON only where somebody has deliberately asked for it, so
# the safe state is the one you get by not thinking about it. Set API_DOCS=1
# locally when you want to browse the API.
_DOCS = (settings.api_docs or "").strip().lower() in ("1", "true", "yes", "on")
...
# The image and link proxies sit OUTSIDE the paywall on purpose: a browser
# loading an <img> or following a link does not send our auth header, so a
# guarded proxy is a page of broken pictures and dead links. The encrypted
# token is the protection — it names one photograph and only this server can
# read it. See app/media.py.
app.include_router(media.router)
app.include_router(media.link_router)
...
# NOT behind PAYWALL. A promoter, a trialist and a user mid-onboarding all use
# pages worth measuring, and a 402 here would be answered by the frontend's
# paywall routing — which is exactly how a background poll once redirected the
# promoter dashboard to /onboarding on a loop.
app.include_router(activity.router)
app.include_router(release.router)
app.include_router(billing.router)
app.include_router(geo.router, dependencies=PAYWALL)
app.include_router(offmarket.router, dependencies=PAYWALL)
app.include_router(wishlists.router, dependencies=PAYWALL)
# What each customer is hunting. Behind the paywall like the rest of the
# product: the questions are asked inside Ollie, and Ollie is a paid feature.
app.include_router(preferences.router, dependencies=PAYWALL)
# The aggregate of those answers. Admin-only, counts only, no names.
app.include_router(customer_intel.router)
# The maps key reaches the browser — it has to, the browser is what calls Google
# — but only for someone already entitled to see the listing it draws. As a
# NEXT_PUBLIC_ build variable it was in public JavaScript for anyone at all.
app.include_router(maps.router, dependencies=PAYWALL)
app.include_router(maps.admin_router)
# NOT paywalled. A promoter is not a customer — they have no subscription and
# see no listings, so the paywall would lock them out of the only page they
# exist to look at.
app.include_router(promoters.router)
app.include_router(promoters.admin_router)


class DataProbeOut(BaseModel):
    """Everything the free layers hold for one address, verbatim."""
...
@app.get("/api/admin/data-probe", response_model=DataProbeOut, tags=["admin"])
async def data_probe(address: str | None = None,
                     lat: float | None = None,
                     lng: float | None = None,
                     _: User = Depends(require_admin_dep)) -> DataProbeOut:
    """Ask the free data sources what they actually hold for one property.

    Exists so "what do we get for nothing, and what must we buy" is answered
    against live responses rather than anyone's memory of a schema. Admin-only:
    it spends the account's LINZ quota and prints raw field values.
    """
    from .data_probe import ProbeUnavailable, probe

    try:
        return DataProbeOut(**await probe(address=address, lat=lat, lng=lng))
    except ProbeUnavailable as exc:
        # 422, not 500: nothing is broken — the question could not be asked.
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except Exception as exc:
        log.exception("data probe failed")
        raise HTTPException(
            status_code=503,
            detail=f"The probe could not run: {type(exc).__name__}: {exc}") from exc
...
def health() -> dict[str, str]:
    ...
    Railway restarts a service whose healthcheck fails, so a check that fails
    when Postgres hiccups turns a blip into a restart loop. Whether it can
    actually serve is /health/ready.
    """
...
def health_ready() -> Response:
    """Readiness: can this deploy serve requests, and if not, which part is wrong.

    Answers 200 or 503 with a line per check — settings, imports, database,
    schema, data. It has to stay open: the platform's own health probe cannot
    sign in, and the moment this needs a login it stops being usable for the
    thing it exists for, which is finding out why the site is down.

    WHAT IT NO LONGER SAYS OUT LOUD is the `detail` on each check.

        "nothing should be public"

    Which check failed is a status. WHY it failed was a description of the
    inside of the system: table names ("parcel_cache (whole table), users
    (whole table), app_settings…"), fragments of the actual SQL, and the
    exception text underneath. Anybody who typed the address got the data model
    and a list of what was currently broken — which is a map for somebody
    looking for a way in, and it was 846 bytes of it.

    So the name and the verdict stay, because that is what a probe needs, and
    the detail is shown only to an administrator. Nothing is lost: the same
    check runs, and the reason is one sign-in away rather than nowhere.
    """
    from .preflight import check as run_checks
    from .preflight import ready as is_ready
    ...
    return JSONResponse(
        ...
        content={
            "ready": ok,
            "version": VERSION,
            "checks": [{"name": c.name, "ok": c.ok, "fatal": c.fatal}
                       for c in checks],
        },
    )


@app.get("/api/admin/ready", tags=["health"])
def health_ready_detail(_: User = Depends(require_admin_dep)) -> Response:
    """The readiness check WITH the reasons — the half /health/ready stopped
    publishing. Same checks, same verdict, plus the detail that says which table
    is missing and what the database actually complained about."""
    from .preflight import check as run_checks
    from .preflight import ready as is_ready
...
def diagnostics(suburb: str | None = None,
                _: object = Depends(require_admin_dep), db=Depends(get_db_dep)) -> dict:
    ...
    # Trace one suburb all the way through, on the real data.
    #
    # "Pick a suburb and nothing happens" has now been explained four different
    # ways from here, each plausible and none confirmed, because I cannot see
    # this database. This answers it instead of theorising about it: what the
    # picker offers, what the filter resolves that name to, and how many rows
    # come back. Whichever number is zero is the fault.
    if suburb:
        try:
            from .routers.properties import (_active_batch, _area_values,
                                             _filtered_query, _hide_bad_data)
            from .models import PropertyForSale, PropertySold

            probe: dict = {"requested": suburb}
            fs_batch = _active_batch(db, "for_sale", "Auckland")
            sold_batch = _active_batch(db, "sold", "Auckland")
            probe["for_sale_batch"] = fs_batch
            probe["sold_batch"] = sold_batch

            if fs_batch:
                resolved = _area_values(db, PropertyForSale, fs_batch, "suburb", suburb)
                probe["resolved_to"] = resolved[:10]
                probe["resolved_count"] = len(resolved)
                probe["rows_matching"] = (
                    _filtered_query(db, fs_batch, suburb=suburb).count())
                probe["rows_in_batch"] = (
                    db.query(PropertyForSale)
                    .filter(PropertyForSale.import_batch_id == fs_batch).count())
                probe["rows_after_quality_filter"] = _hide_bad_data(
                    db.query(PropertyForSale)
                    .filter(PropertyForSale.import_batch_id == fs_batch)).count()
                # Anything whose name merely CONTAINS the request, so a spelling
                # difference the resolver missed is still visible here.
                like = [v for (v,) in db.query(PropertyForSale.suburb)
                        .filter(PropertyForSale.import_batch_id == fs_batch)
                        .distinct().all()
                        if v and suburb.strip().lower() in v.strip().lower()]
                probe["listings_suburbs_containing_it"] = sorted(like)[:10]
            if sold_batch:
                probe["sold_rows_matching"] = (
                    db.query(PropertySold).filter(
                        PropertySold.import_batch_id == sold_batch,
                        PropertySold.suburb.in_(
                            _area_values(db, PropertySold, sold_batch, "suburb", suburb)))
                    .count())
            info["suburb_probe"] = probe
        except Exception as exc:
            db.rollback()
            info["suburb_probe"] = f"{type(exc).__name__}: {exc}"
    return info
...
@app.get("/selftest")
def selftest(_: User = Depends(require_admin_dep)) -> Response:
    ...
    changes nothing. Returns HTML so it's readable in a browser.

    ADMINS ONLY, and it did not use to be.

        "nothing should be public"

    This page was open to anyone who typed the address, and what it prints is
    two real properties by name, their council valuations, what we value them
    at, and a paragraph of how the pricing guard decides — the workings, on the
    open internet, with addresses attached.

    It stays a page rather than becoming a log line because reading it in a
    browser is the whole point, and an operator checking whether a deploy took
    can sign in. /api/version is still open for the case where nobody can: it
    answers which build is running and nothing else.
    """
    ...
    reprice_can_run = sold_status in ("staged", "preview", "published") and sold_rows > 0
...
def root() -> dict[str, str]:
    # No longer advertises /docs. It used to point at the interactive
    # documentation, which is now off in production — and an address that
    # answers 404 is a worse answer than not mentioning it.
    return {"name": "Apex Property API"}

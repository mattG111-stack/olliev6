...
from portals.complete import _FILLABLE, fill_pending, needs_filling
from propertyvalue import PV_ERROR, PV_NOT_FOUND, PV_OK
...
def test_the_keys_it_reads_are_the_keys_the_lookup_returns(db_session):
    ...
    from propertyvalue import _shape
...
def test_approving_a_listing_carries_its_parking(db_session):
    ...
    from portals.listings import approve
...
def test_an_unreadable_date_leaves_the_age_unknown(db_session):
    ...
    from portals.listings import _days_since
...
def test_no_field_that_exists_on_both_sides_is_silently_dropped(db_session):
    ...
    from portals.listings import _CARRIED
    ...
    #
    # The link_* columns are here for the same reason: they are not scraped data
    # that could be thrown away, they are each table's own note of when IT last
    # opened ITS copy of the link and what came back. The staged row and the
    # live row check independently and act on it differently — the live one
    # comes off the site after a single dead answer, the staged one is only
    # called gone after three — so carrying the numbers across would import a
    # verdict reached under the other table's rules.
    its_own = {"id", "created_at",
               "link_checked_at", "link_gone_count", "link_last_result",
               "delisted_at", "link_dead_at"}
...
def test_the_two_pricing_fields_are_the_only_ones_that_matter(db_session):
    ...
    src = "".join(open(f).read() for f in sorted(glob.glob("pricing/*.py")))
    read = set(re.findall(r'\.get\(\s*["\']([a-z0-9_]+)["\']', src))
    read |= set(re.findall(r'getattr\(\s*\w+\s*,\s*["\']([a-z0-9_]+)["\']', src))

    # These names are read off a row and are not on any file. They are not gaps.
    #
    # market_value and confidence come out of comps/glm and are read back by the
    # audit; prior_asking_price is written by the carry-forward, which runs over
    # the live batch whatever the listing's source was — all three produced by
    # pricing itself moments earlier.
    #
    # lots_override is different in kind: it is produced by a PERSON, typed into
    # the property page during the staged review to correct a lot count the
    # model read too cautiously. A portal listing being approved is a property
    # we have never seen before, so there is nothing to carry — nobody has had
    # the chance to override anything yet. Carrying it would mean inventing one.

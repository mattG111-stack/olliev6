...
# Columns added to bug_reports after it first shipped, with DDL that is safe to
# run against a table that already holds rows: every one is nullable with a
# default, so no ALTER can fail on existing data.
_LATER_COLUMNS: tuple[tuple[str, str, str | None], ...] = (
    ("source", "VARCHAR(16)", "'manual'"),
    ("occurrences", "INTEGER", "1"),
    ("last_seen_at", "TIMESTAMP", None),
    ("fingerprint", "VARCHAR(200)", None),
)
...
def _ensure_table() -> None:
    """Make bug_reports exist AND match what this build queries.

    create_all(checkfirst=True) creates a missing table and does nothing at all
    to one that is already there — including a table built by an earlier build
    that lacks columns this one selects. The result is not a graceful
    degradation: EVERY query fails with "no such column: bug_reports.source", so
    the bug log is the one screen that cannot report its own fault.

    That is exactly what happened. The table shipped in one build, gained four
    columns in the next, and the second build only added them through
    db_bootstrap — which helps if it ran, on a dialect that supports ADD COLUMN
    IF NOT EXISTS. Depending on that is depending on a boot script nobody watches.

    Done once per process: the inspector round-trip is cheap but not free, and
    this sits in front of an error handler.
    """
    ...
    try:
        ...
        have = {c["name"] for c in _inspect(engine).get_columns("bug_reports")}
    except Exception:
        log.exception("could not inspect bug_reports")
        return

    for name, ddl_type, default in _LATER_COLUMNS:
        if name in have:
            continue
        # TIMESTAMP means "with time zone" on Postgres for these columns; the
        # bare form silently stores a naive value and comparisons then go wrong.
        col_type = ("TIMESTAMPTZ" if ddl_type == "TIMESTAMP"
                    and engine.dialect.name == "postgresql" else ddl_type)
        clause = f"ALTER TABLE bug_reports ADD COLUMN {name} {col_type}"
        if default:
        ...
        try:
            with engine.begin() as conn:
                ...
                if default:
                    # Existing rows predate the column and would read NULL.
                    conn.execute(text(
                        f"UPDATE bug_reports SET {name} = {default} WHERE {name} IS NULL"))
            log.warning("bug_reports: added missing column %s", name)
        except Exception:
            log.exception("could not add bug_reports.%s", name)
...
class BugOut(BaseModel):
    id: int
    # Optional on purpose. A single row with a null timestamp — from a database
    # that drifted, or a hand-written insert — used to fail validation and take
    # the ENTIRE list down with it, which makes the bug log the one screen that
    # cannot report its own fault. One odd row should cost that row's timestamp,
    # nothing more.

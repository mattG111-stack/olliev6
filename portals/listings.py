...
from portals.sources import _all_urls, _first_url  # noqa: E402
...
def _from_oneroof(item: dict) -> dict:
    ...
    pricing/pipeline.py reads off a row, this actor supplies all of them;
...
def target_batch(db: Session) -> "ImportBatch | None":
    ...
    staged = _staged_batch(db, BatchType.FOR_SALE.value, None)
...
_CARRIED = (
    ...
    # How the vendor is selling it. Scraped since the portals shipped and then
    # dropped at the door, which meant an approved portal listing arrived with
    # no way of knowing whether its price was a price at all — the same fault
    # the weekly file had, on the one path where the number is freshest.
    "sale_method",

...
from ..models import (AgentContact, AssistantLog, ImportBatch, PageView,
                      PropertyForSale, User, UserStatus)
...
class Metrics(BaseModel):
    ...
    # A listing whose advertisement is gone: hidden from every customer list so
    # nobody clicks through to a 404, but kept, not deleted, and restored the
    # moment the link answers again.
    off_market_now: int = 0
    off_market_7d: int = 0
    # When the link check last ran. A zero above means nothing about the market
    # unless this is recent — without it, "0 off market" reads the same whether
    # nothing dropped off or the checker has not run for a fortnight.
    links_last_checked: str | None = None
    links_never_checked: int = 0
...
def metrics(days: int = Query(7, ge=1, le=365),
    ...
    week_ago = datetime.now(timezone.utc) - _td(days=7)
    off_now = (db.query(func.count(PropertyForSale.id))
               .filter(PropertyForSale.link_dead_at.isnot(None)).scalar() or 0)
    off_7d = (db.query(func.count(PropertyForSale.id))
              .filter(PropertyForSale.link_dead_at >= week_ago).scalar() or 0)
    last_check = (db.query(func.max(PropertyForSale.link_checked_at)).scalar())
    never = (db.query(func.count(PropertyForSale.id))
             .filter(PropertyForSale.url.isnot(None),
                     PropertyForSale.link_checked_at.is_(None)).scalar() or 0)
    ...
    return Metrics(
        ...
        off_market_now=int(off_now), off_market_7d=int(off_7d),
        links_last_checked=last_check.isoformat() if last_check else None,
        links_never_checked=int(never),
    )

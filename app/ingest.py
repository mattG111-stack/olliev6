...
def canonical_sale_date(v) -> str | None:
    ...
    Day-first parsing: this is NZ data, so 03/04/2026 is 3 April.
    ...
    # in the deploy log.
    day_first = not _ISO_ISH.match(s)
    try:
...
def ingest_sold(db: Session, sold_df: pd.DataFrame, filename: str, *, region: str = "Auckland", uploaded_by_id: int | None = None, publish: bool = True, append: bool = True) -> IngestResult:
    ...
    inserted = 0; rejected = 0; already = 0

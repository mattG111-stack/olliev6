...
ONEROOF_ITEM = {
    ...
    "propertyType": "House",
    "priceValue": 1_150_000, "priceText": "Enquiries over $1,150,000",
    "ratingValuation": "$1,100,000",          # the CV, under its real name
    "landValue": "$700,000", "improvementValue": "$400,000",
    "floorArea": "182m²", "landArea": "612m²",
    "bedrooms": 4, "bathrooms": 2, "parking": 2,
    "latitude": -37.06, "longitude": 174.94,
    "images": ["https://img.oneroof.co.nz/a.jpg"],
    "listedDate": "2026-08-21", "description": "Family home with a pool.",
...
def test_oneroof_is_capped_because_it_cannot_filter_by_date():
    """It has no date filter, so the cap is the only thing standing between a
    daily run and the whole city at half a cent a row."""
    p = NL._payload("oneroof", cap=300)
    assert p["maxResults"] == 300
    assert p["includeDetails"] is True       # what carries the rating valuation
    assert p["mode"] == "houses-for-sale"    # a value in the actor's enum
...
def test_a_sweep_records_what_it_found(db_session):
    ...
    run = runner_for({NL.ACTORS["oneroof"]: [ONEROOF_ITEM],
                      NL.ACTORS["realestate"]: [REALESTATE_ITEM]})
...
def test_the_addresses_are_matched_however_each_portal_spells_them(db_session):
    ...
    item = dict(ONEROOF_ITEM, address="3 / 107 Donovan St, Blockhouse Bay",
                suburb="Blockhouse Bay")
    run = runner_for({NL.ACTORS["oneroof"]: [item]})
    got = NL.sweep(db_session, sources=("oneroof",), runner=run)
...
def test_no_portal_carries_zoning_or_title(db_session):
    """The known gap, stated as a test so nobody discovers it as a mystery: an
    approved row has neither, so it reads as not subdividable until the weekly
    file catches up. That is a missing input, not a verdict."""
    _live_batch(db_session)
    run = runner_for({NL.ACTORS["oneroof"]: [ONEROOF_ITEM]})
    NL.sweep(db_session, sources=("oneroof",), runner=run)
    row = NL.pending(db_session)[0]
    ...
    assert prop.type_of_title is None
...
def test_the_raw_answer_is_kept(db_session):
    ...
    NL.sweep(db_session, sources=("oneroof",), runner=run)
    row = NL.pending(db_session)[0]
    assert json.loads(row.raw_json)["ratingValuation"] == "$1,100,000"
...
ONEROOF_SOLD = dict(
    ONEROOF_ITEM,
    address="9 Silvana Park Drive, Flat Bush", suburb="Flat Bush",
    soldPrice=1_240_000, soldDate="2026-08-15", daysOnMarket=27,
    priceMethod="Auction",
)
...
def test_the_sold_sweep_asks_each_portal_for_sales():
    ...
    assert NL._sold_payload("oneroof")["mode"] == "sold"
...
def test_the_sold_sweep_still_asks_for_the_valuations():
    ...
    CV on the record and the sale cannot be read as a percentage of anything."""
    assert NL._sold_payload("realestate")["get_valuations"] is True
    assert NL._sold_payload("oneroof")["includeDetails"] is True
...
def test_the_sold_cap_is_a_weeks_worth_not_a_days():
    ...
    assert NL._sold_payload("oneroof", cap=1000)["maxResults"] == 1000
...
def test_a_sale_with_no_price_or_no_date_is_not_a_comp():
    ...
    and nothing else."""
    assert NL.to_listing("oneroof", dict(ONEROOF_SOLD, soldPrice=None,
                                         lastSalePrice=None), kind="sold") is None
    assert NL.to_listing("oneroof", dict(ONEROOF_SOLD, soldDate=None,
                                         lastSaleDate=None), kind="sold") is None
...
def test_a_missing_digit_is_flagged_against_the_suburbs_own_median(db_session):
    ...
    run = runner_for({NL.ACTORS["oneroof"]: [dict(ONEROOF_SOLD, soldPrice=124_000)]})
...
def test_an_extra_digit_is_flagged_too(db_session):
    _sold_batch(db_session, *[(f"{i} Silvana Park Drive", "Flat Bush",
                               1_200_000, "2026-05-01") for i in range(6)])
    run = runner_for({NL.ACTORS["oneroof"]: [dict(ONEROOF_SOLD, soldPrice=12_400_000)]})
    NL.sweep_sold(db_session, sources=("oneroof",), runner=run)
...
def test_an_ordinary_sale_is_not_flagged(db_session):
    _sold_batch(db_session, *[(f"{i} Silvana Park Drive", "Flat Bush",
                               1_200_000, "2026-05-01") for i in range(6)])
    run = runner_for({NL.ACTORS["oneroof"]: [ONEROOF_SOLD]})
    NL.sweep_sold(db_session, sources=("oneroof",), runner=run)
...
def test_pending_can_be_asked_for_one_kind(db_session):
    ...
    NL.sweep(db_session, sources=("oneroof",),
             runner=runner_for({NL.ACTORS["oneroof"]: [ONEROOF_ITEM]}))
    NL.sweep_sold(db_session, sources=("oneroof",),
                  runner=runner_for({NL.ACTORS["oneroof"]: [ONEROOF_SOLD]}))

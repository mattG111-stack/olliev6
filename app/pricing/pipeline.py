...
LAND_ONLY_MAX_CV_PER_FLOOR_M2 = 4000.0
    ...
    for _, r in df.iterrows():
        ...
        if not is_premium:
            if ctype in ("Section", "Lifestyle Section"):
                ...
                # Only consider the land-only path when the CV is implausibly low
                # for the floor area (a real land-only/new-build signature). A
                # normal full CV — e.g. 7 Wynne Gray at $8,476/m² — never qualifies,
                # so it stays under the CV guard instead of floating to comps.
                if (floor_v and _pos_num(r.get("key_bedrooms"))
                        and ctype not in ("Section", "Lifestyle Section")
                        and cv_v / floor_v < LAND_ONLY_MAX_CV_PER_FLOOR_M2):

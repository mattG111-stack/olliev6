...
def _all_urls(v) -> list[str]:
    """Every image in a field that is sometimes a URL and sometimes a list.

    _first_url used to be the whole of this and it threw the rest away, so an
    approved portal listing carried one photo against the thirty a listing from
    the weekly file carries. The actors were returning them all along.
    """
    out: list[str] = []
    if isinstance(v, (list, tuple)):
        ...
        out.append(v.strip())
    # Order is the portal's own — the first is the one it leads with — and a
    # gallery that shows the same photo twice looks broken.
    seen, uniq = set(), []
    for u in out:
        if u not in seen:

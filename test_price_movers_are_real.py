...
prior_price.is_placeholder_price is the rule for "did a vendor actually name
...
from routers.dashboards import MAX_BELIEVABLE_PRICE_MOVE
...
# The statement under test, imported rather than scraped.
#
# This used to regex the SQL back out of the module source. That broke the
# moment the statement stopped being one literal — the shared "is this a price a
# vendor named" rule is now concatenated into both halves of it — and the test
# failed with an empty result set, which reads exactly like the panel being
# broken. A test that identifies its subject by pattern-matching source fails
# for reasons that have nothing to do with the behaviour it guards.
from routers.dashboards import MOVERS_SQL as _MOVERS_SQL

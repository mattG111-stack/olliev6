...
from assistant.providers import _out_of_time, _ran_out
from assistant.tools import (_gap, _near, _no_batch, search_listings,
...
def test_days_to_sell_separates_wrong_spelling_from_no_listing_dates(db_session):
    ...
    # A spelling is ours to fix by asking. Searching would answer confidently
    # about a real place that is not the place the person meant.
    assert "Looking it up: NO" in misspelled

    real = suburb_days_to_sell("Riverhead")
    assert GAP in real
    assert "none carry one" in real
    # Both are now gap blocks, and the line that tells them apart is the one
    # that decides whether looking it up could help. This one no rephrasing
    # fixes, so it is the case the internet fallback exists for.
    assert "Looking it up: ALLOWED" in real
...
def test_it_names_the_filter_that_emptied_the_search(db_session):
    ...
    assert GAP in out
    # The default cap is inclusive: "at most $900,000". Four listings in five have

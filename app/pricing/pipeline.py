...
def run(for_sale_df: pd.DataFrame, sold: SoldDataset,
    ...
    for _row_idx, r in df.iterrows():
        ...
        asking_basis = ADVERTISED_BASIS if asking else None
        prior_ask = _pos_num(r.get("prior_asking_price"))
        if asking is None and prior_ask:
        ...
        asking_is_placeholder = bool(asking and asking_basis != DERIVED_BASIS and (
        ...
        if is_premium:
            ...
            deal_block = (f"No advertised price ({listing_type}) — the figure in the "
                          f"feed is a search price, not what the vendor is asking")
        ...
        if asking_v and cv_v and cv_v > 2.5 * asking_v:
            ...
            deal_block = ("The council valuation is more than double the asking "
                          "price — one of the two is wrong and we cannot tell which")
        elif asking_v and deal_value is not None:
            if cv_v and asking_v < DEAL_ASKING_CV_FLOOR * cv_v:
                deal_value = None
                deal_block = ("Asking price is far below the council valuation — "
                              "almost always a lure, a defect, or a non-standard sale")
            elif deal_value > 1.8 * asking_v:
                deal_value = None
                deal_block = ("The discount would be more than 80%, which is not a "
                              "find — something in the inputs is wrong")
        ...
        _cv_untrusted = cv_is_land_only or cv_incomplete

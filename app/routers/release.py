...
class FillOut(BaseModel):
    ...
    blocked: int
    unreachable: int
    # Where the next call carries on from. `scanned == 0` means there is
    # nothing left; `remaining` is only for the progress line.
    last_id: int = 0
    remaining: int = 0
    scanned: int = 0
...
def fill_new_listings(kind: str = "for_sale", after_id: int = 0, limit: int = 0,
                      admin: User = Depends(require_admin),
    ...
    ONE CHUNK PER CALL. This used to look up as many as 400 rows in a single
    request — one real lookup each, a second or more apiece — and the gateway
    closed the connection minutes before it finished. What reached the browser
    was "HTTP 500 with no response body", which reads as a crash and was really
    a timeout; it is in the bug log four times over. The caller now repeats
    with `after_id` until `scanned` comes back 0, which is the same shape the
    portal sweep beside it already uses.
    """
    from ..portals.complete import fill_pending
    from ..runlog import record

    out = fill_pending(db, kind=kind, after_id=after_id,
                       **({"limit": limit} if limit else {}))
    # Only log a chunk that actually did something. One press can be a dozen
    # calls, and a run log with twelve identical "0 looked up" lines in it is

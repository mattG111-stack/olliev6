...
def _has_room_counts() -> bool:
    """Does the fixture carry the inputs the hedonic actually uses?

    The committed fixture is real Auckland sales, and the export it came from
    carries no bedroom or bathroom counts at all. The model can still be
    measured against the ceiling on it — that is a property of the whole
    estimator — but "beats the raw council figure" is a claim about the hedonic
    doing work, and it cannot do that work without room counts.

    Skipping it on this fixture is not the old silent skip: the reason names the
    exact missing column, and the ceiling test below still runs on every commit.
    """
    if DATA is None or not DATA.exists():
        return False
    head = pd.read_csv(DATA, nrows=200, low_memory=False)
    beds = pd.to_numeric(head.get("key_bedrooms"), errors="coerce")
    return beds is not None and beds.notna().any()
...
@pytest.mark.skipif(not _has_room_counts(),
                    reason="this fixture carries no key_bedrooms — the hedonic "
                           "cannot be judged against raw CV without them")
...
def test_valuation_error_stays_under_ceiling():
    ...
    Measured at 8.99% when the shrunk estimator landed, and 7.04% on the
    committed fixture of real Auckland sales — better, but read that with care:
    those sales sit against a council revision dated May 2024, so the CV they
    are measured from is unusually fresh. On the same data raw CV scores 7.05%,
    which is the finding rather than a footnote — where the council figure is
    current, a CV-ratio model has almost nothing left to add.

    The ceiling leaves headroom for data refreshes without letting a real
    regression through: the old tight-comp cascade scored 10.06% and fails this.
    """

...
from ..promoters import attribute
...
class SignUpIn(BaseModel):
    ...
    phone: str | None = None
    # The promoter's referral code, from ?ref= on the sign-up link. Optional and
    # never trusted: an unknown, expired or self-referring code is ignored and
    # the account is created exactly as it would have been. A referral that
    # cannot be recorded must never be the reason a signup fails.
    ref: str | None = Field(default=None, max_length=32)
    # Which of the promoter's ads sent them, if the link was tagged. Same
    # treatment as ref: never trusted, never a reason to fail a signup.
...
def sign_up(body: SignUpIn, db: Session = Depends(get_db)) -> SignUpOut:
    ...
    # Attribution happens HERE and nowhere else: at account creation, once. A
    # code presented at sign-in, or by someone who already has an account, does
    # nothing — they are not a new customer, and a promoter is paid for bringing
    # in customers, not for being the last link someone happened to click.
    attribute(db, u, body.ref, body.campaign)
...
def delete_user(
    ...
    # and "we deleted the account so we cannot say what we owed you" is not a
    # position to be in. Deactivating stops their link and keeps the ledger.
    promoter = db.query(Promoter).filter(Promoter.user_id == u.id).first()
    if promoter is not None:
        earned = db.query(Commission).filter(Commission.promoter_id == promoter.id).count()
        if earned:
            raise HTTPException(
                status_code=409,
                detail=f"{u.email} has {earned} commission record(s). Deactivate the "
                       f"promoter instead — deleting would erase what they were owed.")
        try:
            with db.begin_nested():
                db.query(ReferralClick).filter(
                    ReferralClick.promoter_id == promoter.id).delete(synchronize_session=False)
                db.query(Referral).filter(Referral.promoter_id == promoter.id).delete(
                    synchronize_session=False)
                db.query(Promoter).filter(Promoter.id == promoter.id).delete(
                    synchronize_session=False)
        except Exception as exc:
            log.warning("could not clear promoter rows for user %s: %s: %s",
                        user_id, type(exc).__name__, exc)

    # A referred CUSTOMER can be deleted, and their commission rows go with them
    # — the row cannot outlive the referral it points at. The export CSV is the
    # copy that survives, which is why a payout run works from it.
    try:
        with db.begin_nested():
            mine = [r.id for r in db.query(Referral).filter(Referral.user_id == u.id).all()]
            if mine:
                db.query(Commission).filter(Commission.referral_id.in_(mine)).delete(
                    synchronize_session=False)
    except Exception as exc:
        log.warning("could not clear commissions for user %s: %s: %s",
                    user_id, type(exc).__name__, exc)

...
def _band_index(floor: float, band: float) -> int:
    """Which size band a floor area falls in. Bands are multiplicative, so a
    120 m² and a 132 m² home sit together while 120 and 200 never do."""
    return int(math.log(max(floor, 1.0)) / math.log(1.0 + band))


def _paired_diffs(rows: list, room: str, other: str, value: str,
                  level: str, band: float, min_cell: int) -> tuple[list[float], list[float]]:
    """Median value gap between n and n+1 rooms, computed WITHIN matched cells.

    A cell is a set of sales that are alike in everything we are controlling for,
    so the only thing separating the two groups inside it is the extra room.
    Returns the per-cell gaps and the per-cell FLOOR AREA gaps, the second being
    how you can tell whether a comparison was really like-for-like.
    """
    from collections import defaultdict
    cells: dict = defaultdict(lambda: defaultdict(list))
    for r in rows:
        k = getattr(r, room, None)
        v = getattr(r, value, None)
        if k is None or not v:
            continue
        key: tuple = ()
        if level in ("like", "size"):
            if not r.floor_area_m2:
                continue
            key += (_band_index(r.floor_area_m2, band),)
        if level == "like":
            o = getattr(r, other, None)
            if o is None:
                continue
            key += (int(o),)
        cells[key][int(k)].append(r)

    diffs: list[float] = []
    gaps: list[float] = []
    for lv in cells.values():
        usable = {n: g for n, g in lv.items() if len(g) >= min_cell}
        if len(usable) < 2:
            continue
        base = max(usable, key=lambda n: len(usable[n]))
        pair = base + 1 if base + 1 in usable else (base - 1 if base - 1 in usable else None)
        if pair is None:
            continue
        lo, hi = (base, pair) if pair > base else (pair, base)
        v_lo = _median([getattr(r, value) for r in usable[lo]])
        v_hi = _median([getattr(r, value) for r in usable[hi]])
        if v_lo is None or v_hi is None:
            continue
        diffs.append(v_hi - v_lo)
        f_lo = _median([r.floor_area_m2 for r in usable[lo] if r.floor_area_m2])
        f_hi = _median([r.floor_area_m2 for r in usable[hi] if r.floor_area_m2])
        if f_lo and f_hi:
            gaps.append(f_hi - f_lo)
    return diffs, gaps


def _matched_step_effect(rows: list, room: str, value: str = "sale_price",
                         *, band: float = 0.12, min_cell: int = 3):
    """What one more bedroom / bathroom is worth, comparing like with like.

    The old version compared every 3-bed sale in the suburb against every 4-bed
    sale. That number is not the value of a bedroom: 4-bed homes are also bigger,
    often newer, and frequently a different property type, so the figure bundled
    roughly forty square metres of extra floor area into what was labelled as one
    room. Someone deciding whether to partition a lounge was reading the price of
    an extension.

    So the comparison is now made inside matched cells, and it falls back one
    control at a time when the data cannot support the strictest one:

      like-for-like  same property type, same size band, same count of the OTHER
                     room. This isolates reconfiguring space you already have.
      same size      same property type and size band.
      same type      same property type only.
      unadjusted     the old behaviour, kept only as a last resort and labelled.

    Returns (dollars, basis, floor_gap_m2, cells). floor_gap_m2 is the median
    floor-area difference between the two groups actually compared — near zero on
    a like-for-like read, and large on an unadjusted one, which is exactly how you
    can see the confound rather than having to trust the label.
    """
    other = "baths" if room == "beds" else "beds"

    typed = [r for r in rows if r.property_type]
    if typed:
    ...
    for level, basis, source in (
        ("like", "like-for-like", typed),
        ("size", "same size", typed),
        ("type", "same type", typed),
        ("all", "unadjusted", rows),
    ):
        diffs, gaps = _paired_diffs(source, room, other, value, level, band, min_cell)
        if diffs:
            return _median(diffs), basis, _median(gaps), len(diffs)
...
class MarginalEffect(BaseModel):
    ...
    note: str | None = None
    # How strictly the comparison was matched: "like-for-like" | "same size" |
    # "same type" | "unadjusted". Anything below like-for-like is measuring more
    # than the room, and the UI says so.
    basis: str | None = None
    # Median floor-area gap between the two groups compared. Near zero means the
    # extra room really was the only difference; a large number means the figure
    # includes the extra space that came with it.
    floor_gap_m2: float | None = None
    cells: int | None = None
...
def suburb_stats(suburb: str, region: str = "Auckland",
    ...
    effects: list[MarginalEffect] = []
    bed_d, bed_basis, bed_gap, bed_cells = _matched_step_effect(rows, "beds")
    effects.append(MarginalEffect(
        key="bedroom", dollars=bed_d, basis=bed_basis if bed_d else None,
        floor_gap_m2=bed_gap, cells=bed_cells or None,
        note=None if bed_d else "not enough sales to measure"))

    bath_d, bath_basis, bath_gap, bath_cells = _matched_step_effect(rows, "baths")
    bath_days = (_matched_step_effect(rows, "baths", "days_on_market")[0]) if days else None
    effects.append(MarginalEffect(
        key="bathroom", dollars=bath_d, days=bath_days,
        basis=bath_basis if bath_d else None, floor_gap_m2=bath_gap,
        cells=bath_cells or None,
    ...
    if by_month:
        # Anchor on the later of "now" and the newest month in the data, so a
        # month with no sales shows as a genuine zero rather than vanishing —
        # a quiet month IS the signal when you are asking which way the market
        # is going.
        end = max(latest_month or current_month, current_month)
        for per in _months_range(_shift_period(end, MONTH_WINDOW - 1), end):
            got = by_month.get(per, [])
            m_svc = _median([r.sale_price / r.cv_numeric for r in got
                             if r.sale_price and r.cv_numeric])
            monthly.append(MonthPoint(
                month=per,
                sales=len(got),
                median_days=_median([r.days_on_market for r in got
                                     if r.days_on_market and r.days_on_market > 0]),

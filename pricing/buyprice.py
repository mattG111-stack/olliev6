...
from .glm import POOL_PREMIUM, canonical_type, predict, title_code
from .pool import PoolPremium, pool_flags, to_pool_status
...
LAND_TOL = 0.15                 # ±15%
MIN_COMPS = 2
# The outer recency bound: a ROLLING window, counted back from the most recent
# sale in the data rather than from a date written into the source.
#
# It used to be "on or after 1 July 2025", a fixed point. That is a window only
# on the day it is written. By August 2026 it had quietly become thirteen
# months, next year it would be twenty-five, and the engine would drift further
# from the market every month while the constant still looked deliberate.
#
# Counted from the newest sale on file, not from today, so the bound means the
# same thing whether it runs against this week's upload or a year-old export —
# and a gap in deliveries cannot silently empty the engine.
#
# Two years is the OUTER bound, not the working window. The sale/CV ratio below
# reads the last six months first and only reaches further back for a suburb too
# thin to answer — cutting the engine itself to six months would leave those
# suburbs with nothing local at all, and a thin suburb falling through to the
# regional median is worse than one using its own slightly older sales.
SOLD_WINDOW_MONTHS = 24

# Kept only so an old import does not break; nothing reads them any more.
SOLD_FROM_YEAR = 2025
SOLD_FROM_MONTH = 7
...
#
# Re-measured on 54,573 Auckland house sales, each valued using only sales that
# came BEFORE it, which is the only way to ask the question honestly:
#
#     6 -> 12 -> 24, need 8       6.81%   <- kept
#     6 months flat               6.80%
#     3 -> 6, need 8              6.96%
#     3 -> 6, need 20             6.90%
#     3 -> 6 -> 12 -> 24, need 8  6.97%
#     raw council CV              7.14%
#
# Starting at three months is worse at every threshold tried. Three months of
# one suburb's house sales is a handful, and the median of a handful is noise —
# while the market itself moves slowly enough that the freshness wins nothing
# back: month of sale explains under 1% of the spread in sale/CV.
RATIO_WINDOWS_MONTHS = (6, 12, 24)
RATIO_MIN_COMPS = 8

# Below this the recency bound is not applied at all. Following the market is the
# goal; having no engine is not a way of following anything.
MIN_ENGINE_SALES = 200
...
# The last like-for-like tier: same suburb and type, same size, room counts not
# required. Four sales is enough here — measured across thresholds from 2 to 20,
# the answer barely moves, so the lowest count that is still a median rather than
# a coin toss is the one that fires most often.
SIZE_TOL = 0.20
SIZE_MIN_COMPS = 4
MATCH_MIN_COMPS = 2
...
class BuyResult:
    ...
    comps_matched: int
    # How the subject's pool (or lack of one) was handled against the comps:
    #   "matched"  — every comp shares the subject's pool status
    #   "adjusted" — too few did, so their prices were restated onto it
    #   "n/a"      — no comps, or nothing on either side has a pool
    pool_basis: str = "n/a"
    pool_pct: float | None = None   # the premium used, when one was
    pool_scope: str | None = None   # where that premium was measured
...
class CompEngine:
    ...
    def __init__(self, sold_df: pd.DataFrame):
        ...
        # Recency — the last SOLD_WINDOW_MONTHS of sales, counted back from the
        # newest one on file. The point is to follow the market rather than the
        # average of the last few years: measured on 54,573 Auckland sales, with
        # every sale valued using only sales that came BEFORE it,
        #
        #     raw council CV     7.14%
        #     3 months           6.97%
        #     6 months           6.80%
        ...
        #     everything         6.84%
        #
        # Three months is too thin to hold a suburb together; past a year the
        # engine starts averaging a market that has moved.
        # Dates arrive in either spelling — M/D/YYYY from the original scraper,
        # ISO from current ingest — so they go through the shared parser rather
        # than a split on "/". That split did not fail loudly on ISO input: it
        # produced one column, the shape test was false, and the entire recency
        # bound was skipped. Every sale ever loaded then entered the engine, so
        # 1990s sales priced today's houses with nothing on screen to say so.
        ...
        if date_col is not None:
            ...
            t = yy * 12 + mm
            newest = t.max()
            if newest == newest:                  # not NaN — some date parsed
                recent = t > newest - SOLD_WINDOW_MONTHS
                # Never cut to nothing. A region whose latest delivery is thin,
                # or a test fixture that happens to span less than the window,
                # must still produce an engine — an empty one does not fail, it
                # answers with a sale/CV ratio of exactly 1.0 and every
                # valuation silently becomes the council figure.
                if recent.fillna(False).sum() >= MIN_ENGINE_SALES:
                    df = df[recent.fillna(False)].copy()
                    yy, mm = yy.reindex(df.index), mm.reindex(df.index)

        df["ct"] = df["property_type"].map(canonical_type)
        # Whether each comp has a pool. Read once, here, so every tier and every
        # premium measurement below agrees on what "has a pool" means.
        df["pool"] = pool_flags(df)

        # Month index (year*12 + month) so the ratio lookup can take a window.
        df["_t"] = (yy * 12 + mm).reindex(df.index) if yy is not None else float("nan")
        ...
        # What a pool is worth in each area, measured off these same sales. Used
        # only when there are too few like-for-like comps to compare against.
        self.pool = PoolPremium(df)
    ...
    def _v4(row) -> float | None:
        p = predict(
            ...
            # Deliberately pool-free, for every comp. This value is only ever
            # used as the denominator of sale ÷ value, and keeping the pool out
            # of it means that ratio prices the pool from the SALES — locally,
            # and only once. See buy_price.
    ...
    def buy_price(self, *, suburb, district, property_type, beds, baths, land,
                  asking, v4_value, cv, pool=False) -> BuyResult:
        try:
        ...
        want_pool = pool in (True, 1, "1", "True", "true", "TRUE", "Yes", "yes")
        ...
        pool_basis, pool_pct, pool_scope = "n/a", None, None
        if comps is not None:
            # ---- Pools: compare like with like, or restate onto the subject ----
            # Every comp's own v4 is computed WITHOUT a pool premium (see _v4),
            # so the ratio below carries whatever pool status the comp set has.
            # Two ways to make that status the subject's:
            #
            #   matched   enough sales share it — use only those, and no premium
            #             is assumed anywhere in the calculation.
            #   adjusted  too few do, so each comp's sale price is restated onto
            #             the subject's status using the gap measured in this
            #             area between homes with a pool and homes without.
            #
            # Either way the ratio ends up on the subject's footing, which is why
            # the subject's own v4 has its flat model premium removed below —
            # applying both would count the pool twice.
            prices = comps["sale_price"]
            if "pool" in comps.columns:
                same = comps[comps["pool"] == want_pool]
                if len(same) >= MIN_COMPS:
                    comps, prices = same, same["sale_price"]
                    pool_basis = "matched" if comps["pool"].any() or want_pool else "n/a"
                elif comps["pool"].any() or want_pool:
                    pf = self.pool.premium(suburb=suburb, district=district,
                                           property_type=property_type)
                    pool_basis, pool_pct, pool_scope = "adjusted", pf.pct, pf.scope
                    prices = pd.Series(
                        [to_pool_status(p, comp_pool=bool(cp), subject_pool=want_pool,
                                        pct=pf.pct)
                         for p, cp in zip(comps["sale_price"], comps["pool"])],
                        index=comps.index)

            ratios = (prices / comps["comp_v4"])
            ratios = ratios[(ratios >= RATIO_LO) & (ratios <= RATIO_HI)]
            # The subject's v4 arrives with the flat nationwide pool premium
            # already in it. The comp ratio now prices the pool from local
            # sales, so the flat one comes back out rather than stacking.
            base = v4_value / POOL_PREMIUM if want_pool else v4_value
            area = base * float(ratios.mean()) if len(ratios) else v4_value
            n = int(len(ratios))
        else:
        ...
        return BuyResult(round(buy), round(area), tier, n,
                         pool_basis=pool_basis, pool_pct=pool_pct,
        ...
        if not base.empty:
            ...
            if fl:
                tiers.append(("floor", base[base["floor_area_m2"].between(
                    (1 - SPEC_TOL) * fl, (1 + SPEC_TOL) * fl)], SPEC_MIN_COMPS))
            # Only when a room count was actually matched. With neither beds nor
            # baths recorded this tier is the whole suburb wearing a
            # like-for-like label — it fired first, returned a suburb-wide
            # answer, and the size tier below could never be reached.
            if b is not None or ba is not None:
                tiers.append(("beds_baths", base, SPEC_MIN_COMPS))

        # Same size, same suburb, whatever the room counts.
        #
        # Every tier above needs the bed AND bath count to match exactly, so a
        # listing missing either got nothing from this method at all and fell
        # straight through to the suburb-wide ratio. That is why it fired on 14%
        # of properties: not because like-for-like comps are rare, but because
        # the definition of like-for-like was four conditions wide.
        #
        # Measured on 12 months of Auckland house sales, each valued using only
        # sales that came before it:
        #     same suburb + floor within 20%, min 4    6.76%   (fires on 90%)
        #     the suburb-wide ratio                    6.81%   (fires on 100%)
        # Better than the suburb ratio and available almost everywhere, which is
        # the combination the strict tiers could not offer.
        if fl:
            tiers.append(("size_only", g[g["floor_area_m2"].between(
                (1 - SIZE_TOL) * fl, (1 + SIZE_TOL) * fl)], SIZE_MIN_COMPS))

        for tier, m, need in tiers:
            if m.empty:
                continue
            rr = m["sale_cv"].dropna()

...
# WHO IS ASKING, AND ON WHOSE KEY. The research tool spawns its own model calls
# and needs the same credentials this run is using, but `dispatch(name, args)`
# takes only what the model passed — a key threaded through every tool signature
# would put a secret in the one place it must never be: an argument a language
# model fills in. A context variable set at the top of the run and read by the
# one tool that needs it keeps the key out of the model's reach entirely.
CALLER: contextvars.ContextVar[dict] = contextvars.ContextVar(
    "assistant_caller", default={})
...
def _run_anthropic(
    ...
    *, deadline: Any = _UNSET, max_iterations: int = MAX_ITERATIONS,
    ...
    # An identity-linked key is tied to a person rather than standing alone, and
    # the API refuses it unless the request also names the workspace it acts in:
    #
    #   400 anthropic-workspace-id is required when authenticating with an
    #   identity-linked API key
    #
    # Older keys do not need it and are unaffected by it being absent, so this
    # is sent only when there is one to send.
    ...
    #
    # `web_ok` is what makes a wrong guess about the block's shape survivable.
    # This was written on a machine that cannot reach the API to confirm it, and
    # a tool the API rejects would take down EVERY question rather than one
    # feature. On a refusal that names the tool it is dropped and the call is
    # retried once; Ollie loses the lookup and keeps working.
    web_ok = bool(websearch.ALLOWED_DOMAINS)
    convo = list(messages)
    used: list[str] = []
    queries: list[str] = []
    ...
    for i in range(max_iterations):
        ...
        try:
            ...
            if web_ok and websearch.is_tool_rejection(exc):
                # Only when the refusal names the search tool. A key problem or
                # a bad model name must still surface as itself rather than be
                # hidden behind a retry for a feature nobody asked about.
                web_ok = False
                try:
                    resp = _call(False)
...
def run(provider: str, api_key: str, system: str, messages: list[dict],
    ...
    CALLER.set({"provider": provider, "api_key": api_key,
                "model": ANTHROPIC_MODEL if provider == "anthropic" else OPENAI_MODEL,
                "workspace_id": workspace_id})
    try:
        kw = {"deadline": deadline, "max_iterations": max_iterations,
              "on_step": on_step}

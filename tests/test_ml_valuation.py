...
# ---- the switch ------------------------------------------------------------
def test_the_switch_is_an_off_override_not_an_on_switch(db_session):
    """This defaulted to OFF and that was wrong.

    should_ship already refuses any model that does not beat both the council
    figure and the estimator running today, on sales it never saw. Requiring a
    human to then agree adds no safety - the measurement IS the decision - and
    means a valuation measured as better sits unused until somebody remembers a
    button. Absent means use it; only an explicit off turns it off.
    """
    store.train_and_store(db_session, _sales(2500))
    assert store.enabled(db_session) is True          # nothing set, and it runs
...
def test_a_model_that_passed_the_gate_is_used_without_anyone_pressing_anything(db_session):
    """The gate IS the decision.

    should_ship already refuses anything that does not beat both the council
    figure and the estimator running today, on sales it never saw. Requiring a
    human to then agree adds no safety and means a valuation measured as better
    sits unused until somebody remembers a button.
    """
    row = store.train_and_store(db_session, _sales(2500))
    assert row.shipped and row.is_active
    assert store.enabled(db_session) is True

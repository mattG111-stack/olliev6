...
from pricing import cashflow as CF
from pricing import scoring as SC
from pricing import subdivision as SD
from pricing import zones as Z
from pricing import assumptions as A
from pricing.assumptions import (CV_IMPLAUSIBLE_VS_ASKING,
                          MAX_VALUE_VS_ASKING, parse_money)
from pricing.buyprice import DISCOUNT as BUY_DISCOUNT
from prior_price import (ADVERTISED_BASIS, DERIVED_BASIS, GUIDE_BASIS,
                           derived_asking,
                           is_placeholder_price)
from pricing.comps import SoldDataset
from pricing.glm import building_key, canonical_type
from pricing.glm import predict as glm_predict
...
# One definition, in assumptions, because the load needs the same one — reading
# "$1.35M" with the plain float() path is what lost 5,104 last-sale prices.
...
# A dwelling with NO floor area cannot be size-checked: the size cap cannot fire,
# the comps are matched on size, and the CV-ratio value has no independent sanity
# check behind it. So the deal flag comes off. The valuation still shows — what a
# place is worth is a fair question either way; calling it a DISCOUNT is the claim
# that needs the size.
#
# This used to apply only above $2.5M, on the reasoning that a missing floor most
# distorts the margin at the top of the market. That is true and it is not the
# point: a $900,000 house with no floor area is just as unsized as a $4.6M one,
# and there are far more of them. On a real batch, 787 listings of 9,102 had no
# floor area and 89 of them were flagged as deals, 38 at high confidence.
#
# A SECTION is the exception the rule needs. Bare land has no floor area because
# it has no building, which is not a gap in the data — it is the data. Those are
# valued on land and keep their deal signal.
NO_FLOOR_DEAL_MAX = 2_500_000       # kept: still the tier used for reporting
...
# How far below the LOWER of the asking price and the CV an expected sale may
# land before we stop believing our own arithmetic.
#
# Only used on the path where asking and CV already disagree, so neither is a
# yardstick on its own. What the pair still gives is a floor: whichever of them
# is the broken one, the property is not worth a fraction of both. 22 Michaels
# Avenue came out at 41% of its own asking price, which is not a cheap house,
# it is a method that ignored 1,157 m2 of Ellerslie land.
SALE_VS_PUBLISHED_FLOOR = 0.60
...
def _rescale_band(edge, computed_around, shown) -> float | None:
    """Move a confidence band from the number it was measured on to the number
    that is actually printed.

    The band is a percentage of the estimate by construction (±4% on the asking
    path, ±14% on the fallback), so the same proportion applied to the published
    figure is the same statement about uncertainty — not a new one.

    Returns None rather than a guess whenever the move cannot be made honestly:
    no band, no anchor to measure it against, or a published figure that is
    missing. A band that has quietly become "the published value ± nothing" is
    worse than no band at all, because it reads as certainty.
    """
    try:
        e = float(edge)
        around = float(computed_around)
        target = float(shown)
    except (TypeError, ValueError):
        return None
    # NaN and infinity: != itself catches NaN, the membership test the rest.
    for x in (e, around, target):
        if x != x or x in (float("inf"), float("-inf")):
            return None
    if around <= 0 or target <= 0:
        return None
    return round(e * (target / around) / 1000.0) * 1000.0
...
def run(for_sale_df: pd.DataFrame, sold: SoldDataset,
    ...
    from ingest import _detect_listing_type  # noqa
    from pricing.buyprice import CompEngine
    from pricing.subdivision import SectionRates
    ...
    for _row_idx, r in df.iterrows():
        ...
        listing_type = _detect_listing_type(r.get("price_display"),
                                            r.get("price_numeric"),
                                            r.get("sale_method"))
        ...
        #
        # What survives, and what does not, both matter:
        #
        #   THE VALUATION SURVIVES. What comparable houses sold for is ours to
        #   say and never depended on the vendor naming a price. So the listing
        #   keeps its value and its range, and subdivision, rent and yield are
        #   worked out exactly as before.
        #
        #   THE MARGIN DOES NOT, because a margin is the valuation MINUS what is
        #   being asked, and there is no subtraction to do. It is refused
        #   outright further down (see deal_block), with the reason recorded.
        #
        #   NOR DOES THE BUY PRICE, for the same reason: "what you can pay" is a
        #   figure capped at the asking price, and a cap against nothing is a
        #   number we would be making up. See buy_price_out below.
        # "guide" is in this list and NOT in LISTING_TYPES_WITH_ASKING, which is
        # the whole point of it: the number is published because a vendor did
        # name it, and the valuation is not anchored to it because it is a floor
        # the house will sell above.
        asking = (r.get("price_numeric")
                  if listing_type in ("fixed", "guide", "", None) else None)
        ...
        # A guide says so. The number is real and advertised, but calling it
        # "advertised" flat would put "Enquiries over $699,000" on the screen as
        # though the vendor were asking $699,000 — which is the one distinction
        # the wording exists to make.
        asking_basis = (GUIDE_BASIS if (asking and not _ph_now
                                        and listing_type == "guide")
                        else ADVERTISED_BASIS if (asking and not _ph_now)
                        else None)
        ...
        # Read from the feed's own sale_method FIRST, for the same reason the
        # classification is: the words only appear in the price line when the
        # scraper happened to copy them there, and "Enquiries over" lives in a
        # column of its own. The text check stays for every source that has no
        # such column.
        _disp = str(r.get("price_display") or "").lower()
        asking_is_guide = listing_type == "guide" or any(k in _disp for k in (
        ...
        bp = comp_engine.buy_price(
            ...
            # NOT capped at a guide. "What you can pay" is capped below the
            # asking price, and a floor the house will sell above is not one —
            # capping at $699,000 would put our estimate below what the place
            # plainly costs. Without an asking the engine falls back to what
            # comparable land and houses actually sell for, which is the honest
            # answer for a listing whose real price has not been named.
            land=r.get("key_land_area"),
            asking=(None if listing_type == "guide" else asking),
        ...
        if is_premium and asking and asking > 0:
            # The same buy-price discount the comp path uses, by name rather
            # than by a second copy of the number.
            bp.buy_price = round(BUY_DISCOUNT * float(asking))
            bp.area_value = None
            bp.comp_tier = None
            bp.comps_matched = 0
        ...
        # A building we have no size for. Not a price question — see the note on
        # NO_FLOOR_DEAL_MAX. Bare land is exempt: it has no floor area because it
        # has no building.
        no_floor_dwelling = bool(
            not _pos_num(r.get("key_floor_area"))
            and not A.is_vacant_type(r.get("property_type"))
        )
        ...
        no_advertised_price = (listing_type not in ("fixed", "guide", "", None)
        ...
        elif no_floor_dwelling:
            deal_block = ("No floor area recorded, so the valuation cannot be "
                          "size-checked against comparable homes")
        ...
        if asking_v and cv_v and cv_v > CV_IMPLAUSIBLE_VS_ASKING * asking_v:
            ...
            # NOT `elif`. The too-good-to-be-true ceiling has to run whatever
            # the ratio test did, and writing it as the ratio test's alternative
            # meant a listing that CLEARED the evidence override skipped the
            # ceiling completely. Found by running a real case rather than by a
            # test: asking $500,000 against a $1,000,000 CV, 38 sold comps at
            # high confidence, valued $1,100,000 — 2.2x the asking, a 120%
            # margin — published as a deal. The override was built to let honest
            # cheap houses through, and it let this through with them.
            #
            # Read the figure BEFORE blanking it. Written the other way round
            # this crashed the whole ingest on the first listing that reached it
            # — "unsupported format string passed to NoneType.__format__" — and
            # took a 146 MB load down with it.
            if deal_value is not None and deal_value > MAX_VALUE_VS_ASKING * asking_v:
                _dv = deal_value
                deal_value = None
                deal_block = (f"Valuation ${_dv:,.0f} is "
                              f"{_dv / asking_v:.1f}x the ${asking_v:,.0f} "
        ...
        sd = SD.compute(
            ...
            # A lot count somebody set by hand while the batch was staged. The
            # whole point of setting it there is that it survives to live, so
            # the pricing run has to honour it rather than overwrite it with the
            # model's own cautious reading on the next re-price.
            lots_override=r.get("lots_override"),
        )
        ...
        if cv_incomplete:
            ...
            if mp and _size_controlled_match(mtier):
                ...
                #
                # And it needs to SAY so. This line dropped the deal signal
                # without setting a reason, so deal_block_reason came out blank
                # — the listing appeared in "why no deal" with an empty answer,
                # which is the exact thing that column was built to stop. The
                # reason is a good one; it just was not written down.
                display_value = round(mp)
                deal_value = None
                # `or` — the first reason wins, the same way the load reports a
                # row refused for two reasons under the first one. A listing
                # that reached here already blocked for a land-only council
                # record has a more specific answer than this one, and writing
                # over it swaps a diagnosis for a description.
                deal_block = deal_block or (
                    "Valued from comparable sales rather than the council "
                    "record — good enough to publish a value, not good enough "
                    "to call a discount")
            else:
            ...
            land_v2 = _pos_num(r.get("key_land_area"))

            # LIKE-FOR-LIKE FIRST, AND IT MUST INCLUDE THE LAND.
            #
            # This path used to go straight to floor area x a suburb rate, which
            # never looks at the section at all. On a small house standing on a
            # big site that is not a valuation, it is a valuation of the house
            # with the land thrown in free: 22 Michaels Avenue, a modest dwelling
            # on 1,157 m2 in Ellerslie, came out at $745,000 against an asking of
            # $1.80M and a CV of $2.5M. The arithmetic was right and the method
            # was wrong for the property.
            #
            # matched_sold_price already matches on beds, baths, LAND and floor
            # and is what the incomplete-CV path above uses. It was simply never
            # called here. Floor rate stays as the fallback's fallback, for a
            # listing with no comparable sales to match against.
            m2p, m2tier, _m2n = comp_engine.matched_sold_price(
                suburb=r.get("suburb"), district=r.get("district"),
                property_type=r.get("property_type"),
                beds=r.get("key_bedrooms"), baths=r.get("key_bathrooms"),
                land=land_v2, floor=floor_v2)
            ...
            by_beds = comp_engine.sold_price_cap(
                suburb=r.get("suburb"), district=r.get("district"),
                property_type=r.get("property_type"), beds=r.get("key_bedrooms"))
            if m2p and _size_controlled_match(m2tier):
                sold_based = round(m2p)
                sale_path = f"matched_sold:{m2tier}"
            else:
                sold_based = by_floor or (round(by_beds) if by_beds else None)
                sale_path = "sold_comps" if sold_based else "unresolved"

            # AND IF IT LANDS MILES BELOW BOTH PUBLISHED FIGURES, IT IS WRONG.
            #
            # We got here because the asking and the CV disagree and we cannot
            # tell which is broken — so neither can be used as the yardstick on
            # its own. But an answer far below BOTH of them is not a third
            # opinion, it is a method that has failed: whichever of the two is
            # right, it is nowhere near this. Blanked rather than published,
            # because a number on the page is acted on and a blank is not.
            floor_check = min([v for v in (asking_v, cv_v) if v] or [0])
            if sold_based and floor_check and \
                    sold_based < SALE_VS_PUBLISHED_FLOOR * floor_check:
                sold_based = None
                sale_path = "below_both_published"

            expected_sale = sold_based
            expected_sale_path = sale_path
        ...
        # What you can pay. TWO different figures wore this one name, and the
        # difference is where the development numbers went missing.
        #
        # A CORRUPTED asking — a placeholder (== CV or last-sold) or a guide
        # ("offers over") — must not produce one: 0.95 x a number that is not a
        # price lands miles off (169 Te Oneroa Way, $978k off a placeholder
        # $1.03M ask on a $2.5M-CV new build). Same for a row we could not value
        # at all: there is no basis for "what to pay".
        #
        # NO asking at all is a different thing entirely, and withholding it
        # here broke the site in a way that was worse than the gap it closed.
        # The comp engine still produces a figure for these — it is derived from
        # what comparable land sells for, not from any vendor's number — and
        # everything downstream USED it: the subdivision ran on `bp.buy_price`
        # and published "+19 lots, $4.69M net gain" on the card, while the
        # detail page recomputed from the stored buy price, found nothing there,
        # and drew a dash through every line of the development maths. Gross
        # sales, purchase, net gain, all "—", under a headline claiming $4.69M.
        #
        # One number or none. It is OUR estimate either way, labelled as one on
        # every screen it appears on, so publishing it is not a claim about what
        # a vendor is asking — and withholding it did not stop us relying on it,
        # it only stopped anyone seeing what we relied on.
        buy_price_out = (
            bp.buy_price
            if (bp.buy_price and bp.buy_price > 0
        ...
        rows.append({
            ...
            # The band has to describe the number on the page.
            #
            # It is computed around v.market_value — the AVM's own answer —
            # because glm.Prediction was written when that WAS the published
            # figure ("market_value is always the final number the user sees").
            # It is not any more: what a customer reads as "Our valuation" is
            # fair_value, the CV-anchored figure, and the two differ by a long
            # way on a third of rows. So the page was printing
            #
            #     Our valuation $3,181,962 ... likely range $728,000-$964,000
            #
            # which is not a range with a mistake in it, it is a range for a
            # different property's number.
            #
            # The band is a PERCENTAGE by construction — ±4% on the asking path,
            # ±14% on the fallback — so moving it onto the published value keeps
            # exactly the uncertainty that was measured and attaches it to the
            # figure it is printed beside.
            "range_low": _rescale_band(v.range_low, v.market_value, display_value),
            "range_high": _rescale_band(v.range_high, v.market_value, display_value),
            ...
            # deals." The valuation is ours and stays. A FLAG is different: it
            # is what puts a listing on a deal page, and every deal page is a
            # recommendation to act.
            #
            # is_underpriced already requires an asking price to exist — a
            # discount is measured against one. This one did not: it is simply
            # cashflow > 0, and the cashflow is worked out from a purchase price
            # we deliberately do not show, because for a listing with no
            # advertised price there is nothing to cap one against. So it could
            # put a house on the cashflow-positive page with no price on it and
            # no way to see what the return was calculated on.
            "is_cashflow_positive": (sig.is_cashflow_positive
                                     and not no_advertised_price),
            "is_subdividable": sd.is_subdividable,
            # Feasibility, which needs no price and so survives a listing that
            # has none. "Worth subdividing" is the line above and stays a deal
            # signal; this is the one a developer searches on.
            "can_subdivide": sd.can_subdivide,
        })
    ...
    if {"listing_type", "price_numeric"} <= set(result.columns):
        ...
        # "guide" belongs with "fixed" here and nowhere else: the vendor named
        # this figure, so it is not one of the search prices this block exists
        # to delete. Everything that makes it a FLOOR rather than a price is
        # enforced above — the valuation is not anchored to it, no margin is
        # measured from it, and the basis says so on every screen.
        _blank = ~_lt.isin(["fixed", "guide", ""])

...
from release import (publish_release, restorable_count,
                       restore_missing_listings, staged_summary)
...
class StagedGridRow(BaseModel):
    ...
    # HOW THE VENDOR IS SELLING IT, in the feed's own words — "auction",
    # "deadline treaty", "by negotiation".
    #
    # This column is why the batch could not be audited. 1,128 listings of 1,586
    # arrived as auctions and were published as fixed-price asks; the value that
    # said so was read during the load and thrown away, so nothing on this
    # screen contradicted the wrong answer — and asking_basis, one column over,
    # positively confirmed it by reading "advertised".
    #
    # The raw feed value rather than our classification of it: the
    # classification is the CONCLUSION, and a screen for checking a conclusion
    # has to show the evidence beside it, or the check is the model marking its
    # own work.
    sale_method: str | None = None
    listing_type: str | None = None        # what we concluded from it
...
def _grid_row(p: PropertyForSale) -> StagedGridRow:
    ...
    return StagedGridRow(
        ...
        sale_method=p.sale_method, listing_type=p.listing_type,
...
class DealFunnelOut(BaseModel):
    ...
    # Always expect 0 — see deal_funnel.py for why anything else is a
...
@router.get("/release/restorable")
def restorable(region: str = "Auckland", _: User = Depends(require_admin),
               db: Session = Depends(get_db)) -> dict:
    """How many listings a short upload archived that are still advertised.

    Read-only, so the button can say what it will do before anybody presses it.
    Normally zero — it is only above zero when a load went live over a larger
    book, which is the fault fixed in v1.51.
    """
    return restorable_count(db, region)


@router.post("/release/restore")
def restore(region: str = "Auckland", admin: User = Depends(require_admin),
            db: Session = Depends(get_db)) -> dict:
    """Put back the listings a short upload hid.

    Nothing was ever deleted — archiving a batch only hides it — so this needs
    no re-upload. Every listing in a previously-live batch that is missing from
    the live one, and whose advertisement has not been found gone, is copied
    across as it was: price, photographs, subdivision, holds and all.

    Safe to press twice; the second press finds nothing to do.
    """
    result = restore_missing_listings(db, region)
    if not result.get("restored"):
        raise HTTPException(status_code=409,
                            detail=result.get("reason", "Nothing to restore"))
    return result
...
def publish(region: str = "Auckland", admin: User = Depends(require_admin),
    ...
    result = publish_release(db, region, commit=False)
...
def portals_staged(region: str = "Auckland", cap: int = 200,
    ...
    portal's own columns and shown as theirs. See portals/fill.py.
    """
...
class FillOut(BaseModel):
    """One chunk's worth of work — what the job thread reports as it goes."""
    looked_up: int
    fields_filled: int
...
def _run_fill_job(job_id: int, *, kind: str) -> None:
    """Fill every pending listing that has a gap. On its own thread, to the end.

    IT DOES NOT TIME OUT, because nothing is waiting on it. Two thousand
    listings at a second each is half an hour, and that is fine: the request
    that started it came back immediately and this keeps going whether the
    browser is open, closed, or on another machine.

    That was the whole fault. The work used to happen inside the request, so
    "how long may this take" was decided by a proxy with a thirty-second
    patience rather than by how much there was to do — and what the browser
    reported was "500, no response body", which reads as a crash and was only
    ever a hang-up. Chunking made each request survivable but still required
    somebody to sit and watch a tab.

    Progress goes on the job row as it goes, so the page can show a live count
    and anyone can walk away from it.
    ...
    from staged_stages import _update, job_was_cancelled
    ...
    try:
        ...
        while True:
            ...
            if job_was_cancelled(db, job_id):
                # Stand down WITHOUT writing. job_was_cancelled is true for any
                # status that is not "running" — including "completed" — so
                # stamping "cancelled" here would erase a finished run's result
                # the moment a second worker noticed it. Whoever moved the row
                # off "running" has already said what happened; this thread's
                # only job is to stop.
                log.info("fill job %s is no longer ours — standing down", job_id)
                return
            ...
            # here is not a wrong answer, it is a thread looping for ever.
            if out["last_id"] <= after:
                log.warning("fill cursor stalled at %s — stopping", after)
                break
            after = out["last_id"]
            ...
            done = todo - out["remaining"]
            # Every write stamps last_progress_at (see staged_stages._update),
            # so a long run is never mistaken for a dead one.
            _update(db, job_id,
        ...
        record(db, stage="portals", event="pending_filled",
               ...
               level="warn" if total["unreachable"] else "info",
               detail=(f"{total['looked_up']:,} new listings looked up — "
                       f"{total['fields_filled']:,} blank fields filled, "
                       f"{total['council_records_found']:,} council records found, "
                       f"{total['not_found']:,} not on the council record, "
                       f"{total['unreachable']:,} never reached it"))
        _update(db, job_id, status="completed", progress_pct=100,
                completed_at=datetime.now(timezone.utc),
                rows_total=total["looked_up"],
                rows_inserted=total["fields_filled"],
                stage=(f"{total['fields_filled']:,} filled · "
                       f"{total['council_records_found']:,} records")[:60],
                error_message=None)
    except Exception as e:                        # noqa: BLE001
...
@router.post("/release/listings/fill", response_model=StageStarted)
def fill_new_listings(kind: str = "for_sale",
    ...
    RETURNS A JOB, IMMEDIATELY, like the sweep beside it. The work takes as
    long as it takes and nothing is waiting on it.
    """
    # One fill at a time. Two threads would look up the same rows twice — a

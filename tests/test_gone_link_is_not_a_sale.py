...
from portals import delisted as D
...
@pytest.mark.parametrize("code", [202, 301, 403, 429, 500, 503])
def test_failed_check_cannot_restore_hidden_listing(db_session, code):
    _live(db_session, 1)
    _sweep(db_session, _Client(404))
    row = db_session.query(PropertyForSale).one()
    before = row.link_dead_at
    out = _sweep(db_session, _Client(code))
    assert row.link_dead_at == before
    assert row.link_gone_count == 1
    assert out['came_back'] == 0
    assert out['unreachable'] == 1
    assert out['still_up'] == 0


def test_scheduled_slices_do_not_recheck_recent_rows(db_session):
    rows=_live(db_session,2)
    now=datetime.now(timezone.utc)
    a=_sweep(db_session,_Client(404),now=now,limit=1,due_only=True)
    b=_sweep(db_session,_Client(404),now=now+timedelta(minutes=30),limit=1,due_only=True)
    c=_sweep(db_session,_Client(404),now=now+timedelta(hours=1),due_only=True)
    assert a['checked']==b['checked']==1 and c['checked']==0
    assert all(row.link_gone_count==1 for row in rows)
    d=_sweep(db_session,_Client(200),now=now+timedelta(days=2),due_only=True)
    assert d['came_back']==2 and all(row.link_dead_at is None for row in rows)


def test_scheduled_commit_failure_is_not_success(db_session,monkeypatch):
    _live(db_session,1)
    def fail():raise RuntimeError('synthetic commit failure')
    monkeypatch.setattr(db_session,'commit',fail)
    with pytest.raises(RuntimeError,match='synthetic commit failure'):
        _sweep(db_session,_Client(404),due_only=True)

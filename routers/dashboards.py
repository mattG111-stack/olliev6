...
# The one definition of "the vendor actually named this price" — imported rather
# than spelled out, so renaming the basis cannot leave a string literal behind in
# SQL that silently matches nothing and quietly empties this panel.
...
from routers.properties import _SECTION_TYPES, _hide_bad_data

router = APIRouter(prefix="/api/dashboards", tags=["dashboards"])

# The most a price can move between two weekly loads and still be a PRICE
# CHANGE rather than a data change.
#
# A vendor who re-prices moves by ten or fifteen percent. One who moves by five
# hundred did not: something about how the figure was captured changed, and the
# panel was reporting that as market news. Half is generous — it keeps a genuine
# capitulation and excludes every headline on the batch that prompted this.
MAX_BELIEVABLE_PRICE_MOVE = 0.50
...
# "Is this a price a VENDOR NAMED?" — in SQL, once, for both sides of a price
# comparison. It was written out twice inside the movers query, which is how one
# side came to be missing a condition the other had.
#
# THREE ways a number can sit in asking_price without a vendor having said it:
#
#   the scraper copied the COUNCIL VALUATION into the price field
#   the scraper copied the LAST SALE PRICE into it
#   WE derived it — the carry-forward writes "last advertised, less 3%", rounded
#   to the nearest $10,000, whenever a vendor withdraws their price
#
# The first two were already refused here. The third was not, and it is the one
# that produces believable wrong answers rather than obvious ones: a listing
# whose price we derived one week and which advertises a real price the next
# shows a rise of a few percent, in round thousands, which is indistinguishable
# from a vendor putting their price up — except that no vendor did anything.
# The 501% headlines were the first two; a screenful of +3% to +11% is this one.
#
# NULL basis is treated as advertised: rows loaded before the basis was recorded
# have no way to say, and assuming they were derived would empty the panel.
_REAL_ASKING_SQL = """
    asking_price >= 10000
    AND COALESCE(asking_basis, '{advertised}') = '{advertised}'
    AND NOT COALESCE(cv_numeric > 0
             AND ABS(asking_price - cv_numeric) < 0.005 * cv_numeric, FALSE)
    AND NOT COALESCE(valuation_last_sold_value > 0
             AND ABS(asking_price - valuation_last_sold_value) < 1, FALSE)
""".format(advertised=ADVERTISED_BASIS)

# The movers statement, hoisted so the daily brief and its tests use ONE
# string. The test used to regex-scrape it back out of this file, which broke
# the moment the SQL stopped being a single literal — a test that reads its
# subject by pattern-matching source fails for reasons that have nothing to do
# with the behaviour it is guarding.
MOVERS_SQL = ("""
                -- Both sides have to be a price a VENDOR NAMED.
                --
                -- "$840k -> $5.05M, +501%" in Ranui is not a price rise. It is
                -- one week where the scraper wrote the council valuation into
                -- the price field and one week where it wrote the real asking,
                -- and the panel subtracted one from the other. Every headline
                -- rise on a real batch was this: +501%, +350%, +294%, +275%,
                -- +244%, none of them a decision any vendor made.
                --
                -- app.prior_price.is_placeholder_price is the rule for "is this
                -- a real price", and this query is the one place that was not
                -- using it. Same test, expressed in SQL: within half a percent
                -- of the council valuation, or the last sale price to the
                -- dollar, is the scraper filling a blank.
                -- COALESCE inside _REAL_ASKING_SQL, because a NULL cv or
                -- last-sale makes the comparison NULL, NOT NULL is NULL, and
                -- the WHERE drops the row. Three-valued logic turned a
                -- placeholder filter into a filter that also threw out every
                -- listing with no council record.
                WITH a AS (SELECT slug_id, id, address, suburb, asking_price
                           FROM properties_for_sale
                           WHERE import_batch_id = :prev AND slug_id IS NOT NULL
                             AND """ + _REAL_ASKING_SQL + """),
                     b AS (SELECT slug_id, id, address, suburb, asking_price
                           FROM properties_for_sale
                     ...
                     d AS (
                       ...
                       WHERE a.asking_price <> b.asking_price
                         -- And a believability band. A vendor moving a price by
                         -- more than half is not re-pricing, and whatever it is,
                         -- it is not something to headline as one. Excluded
                         -- rather than shown, because the panel's whole job is to
                         -- say what the market did this week.
                         AND ABS((b.asking_price - a.asking_price)
                                 / NULLIF(a.asking_price, 0)) <= :max_move
                     ),
                     -- A drop has to be a drop. Taking the five smallest and
                     -- the five largest of the SAME set means that in a thin
                     -- week the same listing appears in both columns — a 14%
                     -- price cut printed under "biggest price rises". The
                     -- direction was implied by the sort and never required.
...
class SuburbTrend(BaseModel):
    ...
    # Set only when the chart fell back to the portal: what our own sold records
    # hold for this suburb, so the page can say why it is not using them.
    sold_coverage: dict | None = None
...
# TWO, not three. This chart exists to answer one question — is this year above
# last year or below it — and two years is exactly what that takes. Demanding a
# third meant a suburb whose sold history starts in 2025 fell back to the portal
# aggregate, which stops whenever the scrape did: the reader asked "where is
# 2026" while their own 2026 sales sat in the database, excluded for want of a
# year nobody needed. The frontend refuses to draw fewer than two points anyway.
_SOLD_TREND_MIN_BUCKETS = 2


def _sold_trend_json(db: Session, suburb: str, region: str,
                     ptype: str | None = None
    ...
    rows = (db.query(PropertySold.sale_price, PropertySold.sold_date,
                     PropertySold.property_type)
            .filter(PropertySold.import_batch_id.in_(batch_ids),
                    PropertySold.suburb == suburb,
                    PropertySold.sale_price.isnot(None))
            .all())
    # One kind of home at a time when asked. A median across houses AND
    # apartments moves on the mix of what sold rather than on the market.
    # Matched through canonical_type because the source emits types in Chinese:
    # 独立屋 and 独立别墅 are both houses, and comparing raw strings would return
    # a fraction of them.
    want = ptype.strip().lower() if isinstance(ptype, str) and ptype.strip() else None
    if want:
        rows = [r for r in rows if canonical_type(r[2]).lower() == want]

    by_year: dict[int, list[float]] = {}
    by_month: dict[str, list[float]] = {}
    newest: tuple[int, int] | None = None
    for price, sold_date, _ptype in rows:
    ...
    yearly = _series(by_year, "year", "change_pct")
    # What our own sold records hold for this suburb, whether or not they were
    # enough to draw. Without it, falling back to the portal is silent, and a
    # chart ending before this year looks broken rather than explained.
    coverage = {
        "sales": sum(len(v) for v in by_year.values()),
        "years": len(by_year),
        "need_years": _SOLD_TREND_MIN_BUCKETS,
        "need_per_year": _SOLD_TREND_MIN_PER_BUCKET,
...
# A suburb's chart has to show more than a handful of years — a five-year line
# is not a history, and "how has this suburb moved" cannot be answered by two
# points. This is the bar the joined series aims to clear.
_MIN_USEFUL_YEARS = 6


def _splice_years(ours: str | None, theirs: str | None) -> str | None:
    """One yearly line covering every year either source knows about.

    Our own sold records are the better measure — they are this suburb's actual
    transactions and they include the year in progress — but they only reach as
    far back as the sold history that has been loaded. A suburb whose file
    starts last year drew a two-point chart, while the long aggregate sitting
    beside it knew a decade and stopped before this year. Neither on its own is
    what the reader asked for.

    So: our years win wherever we have them, and the older aggregate fills in
    the years before ours begin. The two must agree on a year they share, or the
    join is refused and our own series stands alone — a step in the line that
    came from swapping sources is worse than a short chart.
    """
    def _points(raw):
        if not raw:
            return {}
    ...
    mine, other = _points(ours), _points(theirs)
    if not mine:
        return theirs
    if not other:
        return ours
    ...
    earliest = min(mine)
    older = {y: p for y, p in other.items() if y < earliest}
    if not older:
        return ours                       # nothing to add

    # CHAIN-LINK the older years onto ours rather than refusing to join.
    #
    # The two sources sample the same market differently, so their medians for a
    # shared year rarely match to the dollar. Joining them raw draws a step at
    # the seam that belongs to us, not the market; refusing to join leaves a
    # two-point chart, which answers nothing. So the older series is scaled by
    # the ratio between the two on the year they share.
    #
    # What that preserves is the SHAPE — every year-on-year move in the older
    # history survives exactly — while the level is anchored to our own sales.
    # It is the same thing a statistics agency does when it rebases a long
    # series onto a new one, and it is the only way to show a decade and this
    # year on one honest line.
    scale = 1.0
    overlap = sorted(set(mine) & set(other))
    if overlap:
        y = overlap[0]
        a, b = float(mine[y]["median"]), float(other[y]["median"])
        if a > 0 and b > 0:
            scale = a / b

    merged = []
    for year in sorted(set(older) | set(mine)):
        if year in mine:
            src = dict(mine[year])
        else:
            src = dict(older[year])
            src["median"] = round(float(src["median"]) * scale)
        merged.append({k: v for k, v in src.items() if k != "change_pct"} | {"year": year})

    prev = None
    for p in merged:
        med = float(p["median"])
        p["change_pct"] = round((med - prev) / prev * 100, 1) if prev else 0.0
        prev = med
    return json.dumps({"points": merged})


def _iso_date(v) -> str | None:
    """The date part of a timestamp, whichever type the driver returned.

    Postgres hands back a datetime and SQLite a string, and assuming either one
    is how this file has broken before.
    """
    if v is None:
        return None
...
def suburb_trend(
    ...
    # === Our weekly trend ===
    #
    # The medians are taken in Python, not by PERCENTILE_CONT ... WITHIN GROUP.
    # That syntax is Postgres-only, so this endpoint raised on SQLite: it could
    # only ever be exercised against production, its browser test was written to
    # assert that it "fails visibly rather than blankly", and the whole trends
    # page shipped unverified for months. One suburb's listings per week is a
    # few dozen rows — small enough that the database has no advantage here, and
    # being able to test the page is worth more than the micro-optimisation.
    sql = text(
        """
        SELECT ib.id AS batch_id, ib.created_at AS batch_date,
               p.asking_price, p.market_value
    ...
    for batch_id, batch_date, asking, market in db.execute(
            sql, {"region": region, "suburb": suburb}).fetchall():
        b = per_batch.setdefault(int(batch_id), {
            "date": batch_date, "asking": [], "market": [], "n": 0})
        b["n"] += 1
        if asking is not None:
            b["asking"].append(float(asking))
        if market is not None:
            b["market"].append(float(market))
    ...
    points = [
        TrendPoint(
            batch_id=bid,
            # created_at arrives as a datetime on Postgres and a string on
            # SQLite; take the date either way rather than assuming one.
            batch_date=(_iso_date(b["date"]) or ""),
            median_asking=_med(b["asking"]),
            median_market_value=_med(b["market"]),
            listing_count=b["n"],
        )
        for bid, b in sorted(per_batch.items(), key=lambda kv: str(kv[1]["date"]))
    ]
    ...
    sold_yearly, sold_monthly, sold_coverage = _sold_trend_json(db, suburb, region, ptype)
    if isinstance(ptype, str) and ptype.strip():
        # The portal's aggregate covers the whole suburb, every type together, so
        # it cannot answer "houses only". Better to show nothing than to answer a
        # different question than the one asked.
        lt_yearly = lt_monthly = None
    # Every year either source knows about, in one line — see _splice_years.
    # Skipped when a property type is in play: the older aggregate covers the
    # whole suburb, every type together, so it cannot answer "houses only".
    if sold_yearly and lt_yearly:
        sold_yearly = _splice_years(sold_yearly, lt_yearly)
    ...
    return SuburbTrend(
        ...
        sold_coverage=sold_coverage if not sold_yearly else None,
        trend_source=source,
...
def compare_batches(
    ...
    # The same four rules as the movers panel on the brief. This endpoint had
    # none of them — no placeholder filter, no derived-price filter, no
    # believability band, and "risers" sorted descending without requiring the
    # change to BE a rise, so in a week of falling prices the least-bad drops
    # were printed as the biggest rises. One rule, two callers, and only one of
    # them had been fixed.
    movers = db.execute(text(
        ...
        """
    ), {"a": a, "b": b, "max_move": MAX_BELIEVABLE_PRICE_MOVE}).fetchall()
    risers = db.execute(text(
        ...
        -- A RISE has to be a rise. Sorting descending only puts the largest
        -- number first; in a week where every price fell, the least-bad drop
        -- is the largest number, and it was printed under "biggest rises".
        SELECT slug_id, address, suburb, pa, pb, change_pct FROM diff
        WHERE change_pct IS NOT NULL AND change_pct > 0
...
def dom_trend(
    ...
    # Every sold delivery. Sold history accumulates, so reading one batch here
    # drew the days-to-sell line out of whichever file arrived most recently —
    # a few hundred sales out of a hundred thousand, and nothing on the chart
    # to say the rest had dropped out.
    batch_ids = sold_batch_ids(db, region)
    if not batch_ids:
...
def conversion(region: str = "Auckland", limit: int = Query(60, ge=1, le=400),
    ...
    sold_ids = sold_batch_ids(db, region)
    live_b = _active(db, "for_sale", region)
    if not sold_ids or not live_b:
    ...
    found = conversion_opportunities(_sold_frame(db, sold_ids), live)
...
class MarketPoint(BaseModel):
    """One point on the market-tracking chart."""
    batch_id: int
    batch_date: str            # ISO date of the point
    label: str = ""            # what to print under it: a date, or a month
    listing_count: int
    median_asking: float | None
...
class MarketHistory(BaseModel):
    ...
    basis: str = "listings"       # "listings" = asking prices now, "sales" = sold history


# A chart of two points is a line between two points, not a trend. Below this
# many weeks of live listings, the sales already on file say more.
_MARKET_WEEKLY_ENOUGH = 4
_MARKET_MIN_POINTS = 3
# A month's median is only worth plotting if enough homes sold in it. One or two
# sales gives whichever house happened to change hands, not the market.
_MARKET_MONTH_MIN_SALES = 8
...
def _listing_market_points(db: Session, region: str, limit: int) -> list[MarketPoint]:
    """One point per published week of live listings, oldest first."""
    ...
    for b in reversed(batches):          # oldest first for a left-to-right chart
        ...
        points.append(MarketPoint(
            ...
            median_asking=_median_or_none([r.asking_price for r in rows if r.asking_price]),
            median_value=_median_or_none([r.fair_value for r in rows if r.fair_value]),
            median_days_to_sell=_median_or_none([r.predicted_days for r in rows if r.predicted_days]),
            underpriced=sum(1 for r in rows if r.is_underpriced),
        ))
    return points


def _sold_market_points(db: Session, region: str, months: int) -> list[MarketPoint]:
    """The region's market month by month, from the sales already on file.

    The chart above was gated on there being two weeks of live listings to
    compare, so a brand-new region drew nothing — while years of settled sales
    for that same region sat in the database, unread. Those sales are a better
    answer to "which way is the market moving" than two weeks of asking prices
    ever were: they are what people actually paid.

    The month in progress is left off. Its median is real but incomplete, and
    plotted next to finished months it reads as a fall that has not happened.
    """
    batch_ids = sold_batch_ids(db, region)
    if not batch_ids:
        return []
    rows = (db.query(PropertySold.sale_price, PropertySold.sold_date,
                     PropertySold.days_on_market)
            .filter(PropertySold.import_batch_id.in_(batch_ids),
                    PropertySold.sale_price.isnot(None))
            .all())

    buckets: dict[tuple[int, int], list[tuple[float, float | None]]] = {}
    for price, sold_date, dom in rows:
        ym = sold_year_month(sold_date)
        if not ym or not price:
            continue
        buckets.setdefault(ym, []).append((float(price), dom))
    if not buckets:
        return []

    today = date.today()
    current = (today.year, today.month)
    keys = sorted(k for k in buckets if k < current)
    keys = keys[-months:]

    points: list[MarketPoint] = []
    for year, month in keys:
        sales = buckets[(year, month)]
        if len(sales) < _MARKET_MONTH_MIN_SALES:
            continue
        points.append(MarketPoint(
            batch_id=year * 100 + month,
            batch_date=f"{year:04d}-{month:02d}-01",
            label=f"{_MONTH_NAMES[month - 1]} {year % 100:02d}",
            listing_count=len(sales),
            median_asking=_median_or_none([p for p, _ in sales]),
            median_value=None,           # nothing to compare a settled sale against
            median_days_to_sell=_median_or_none([d for _, d in sales]),
            underpriced=0,
        ))
    return points
...
def market_history(region: str = "Auckland", limit: int = Query(26, ge=2, le=104),
    ...
    """Market movement over time, drawn from whichever history is longer.

    The pulse tiles show only "right now"; this is which way the market is
    moving. Two sources can answer that, and the chart takes whichever has more
    to say:

      listings — median asking and our own valuation across each published week
                 of live listings. What sellers are asking, as it changes.
      sales    — median sale price month by month across every sale on file.
                 What buyers actually paid, going back as far as the data does.

    It used to be listings only, so a region drew nothing at all until a second
    week had been published — showing "not enough history" over a database
    holding years of sales. Now a short listings history defers to the sales.

    Medians are computed in Python rather than in SQL so this runs on any
    backend, and the listings side reuses the live-feed filter (_hide_bad_data)
    so the history counts the same population the app shows today.
    """
    # Called directly (tests, other routers) the Query() defaults arrive as
    # Query objects rather than numbers, and SQLAlchemy's .limit() raises on one.
    limit = limit if isinstance(limit, int) else 26
    months = months if isinstance(months, int) else 36

    weekly = _listing_market_points(db, region, limit)
    if len(weekly) >= _MARKET_WEEKLY_ENOUGH:
        return MarketHistory(region=region, points=weekly, basis="listings")

    monthly = _sold_market_points(db, region, months)
    if len(monthly) >= _MARKET_MIN_POINTS and len(monthly) > len(weekly):
        return MarketHistory(region=region, points=monthly, basis="sales")
    return MarketHistory(region=region, points=weekly, basis="listings")

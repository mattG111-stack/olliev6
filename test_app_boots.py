...
imports main, and a manual `from main import app` passed here because
    ...
    def walk(routes):
        for r in routes:
            ...
            if isinstance(r, APIRoute):
                out.append(r)
            # Preserve include-time dependencies such as the product paywall.
            if hasattr(r, "effective_route_contexts"):
                import copy
                for context in r.effective_route_contexts():
                    if isinstance(context.original_route, APIRoute):
                        effective = copy.copy(context.original_route)
                        effective.path = context.path
                        effective.dependant = context.dependant
                        out.append(effective)
                continue
            # Older versions may expose a nested router without contexts.
...
def test_the_lifespan_runs(db_session):
    ...
    The whole suite passed. `from main import app` passed. Because IMPORTING

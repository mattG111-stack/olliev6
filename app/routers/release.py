...
@router.post("/release/listings/sweep", response_model=dict)
def sweep_new_listings(hours: int = 24, cap: int = 300,
                       admin: User = Depends(require_admin),
                       db: Session = Depends(get_db)) -> dict:
    """Ask the portals what went on the market recently. Costs money — a real
    Apify run — so it is admin-only and never fires on a page load.
    ...
    if not token(db):
        # Said here rather than swallowed by the best-effort layer below, which
        # would report "0 found" and leave someone hunting a scraper bug that
        # is really a missing token.
        raise HTTPException(
            status_code=400,
            detail="No Apify token yet — add one in the Data connection panel.")
    from ..portals.listings import sweep

    return sweep(db, hours=hours, cap=cap)
...
@router.post("/release/listings/sweep-sold", response_model=dict)
def sweep_sold_endpoint(cap: int = 1000, admin: User = Depends(require_admin),
                        db: Session = Depends(get_db)) -> dict:
    """Ask the portals what has sold. Costs money, so admin-only and never on a
    page load. Runs weekly on its own when PORTALS_DAILY is on."""
    from ..portals.apify import token

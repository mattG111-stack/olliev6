...
# The operator reads these times as New Zealand times, so that is what the
# workbook prints. Matches app/routers/properties.py.
...
def _clean(v):
    if isinstance(v, str):
        ...
        if len(v) > _MAX_CELL:
            v = v[:_MAX_CELL] + "…"
        return v
    # A TIMESTAMP OUT OF POSTGRES CARRIES A TIMEZONE AND EXCEL WILL NOT TAKE IT.
    #
    #     TypeError: Excel does not support timezones in datetimes.
    #
    # openpyxl raises, the endpoint answers 500, and the operator gets "Internal
    # Server Error" where a workbook should be. It has never worked on the live
    # site and every test said it did, because the tests run on SQLite — which
    # does not store timezones, so it hands back a naive datetime and the line
    # below was never reached. The only way to see this was to run the suite
    # against the database production actually uses.
    #
    # Converted to New Zealand time BEFORE the timezone is dropped. Simply
    # discarding it would print UTC under a heading an operator reads as local
    # and put every time in the workbook half a day out — a wrong number is
    # worse than the error it replaced.
    if isinstance(v, datetime) and v.tzinfo is not None:
        return v.astimezone(NZ_TZ).replace(tzinfo=None)
    return v

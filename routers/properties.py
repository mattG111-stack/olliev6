...
class ForSaleRow(BaseModel):
    ...
    # === "Trade Me says" — for display only, never an input (see app/trademe.py) ===
...
def _hide_bad_data(q):
    ...
    return q.filter(
        ...
        # app/portals/delisted.py and cleared by it the moment the link answers
        ...
        # banner on it, answering 200 for ever. Set by app/sold_sweep.py.
...
def _sort_expression(order_by: str):
    ...
    if order_by == "margin_dollars":
        return PropertyForSale.fair_value - PropertyForSale.asking_price

...
# check in portals/delisted.py decides that, by opening it — and never
...
def _row_to_scrape(p) -> dict:
    ...
    return {
        ...
        # WHAT THE LINK CHECK HAS ALREADY LEARNED. Without these three the
        # carry hands the sweep a book of listings it has apparently never
        # looked at, every single upload — and the sweep works oldest-checked
        # first, so it would restart at the top of the book each week and never
        # reach the bottom of it. The delisting check is the ONLY thing that
        # takes a listing off the site now that a short file cannot, so a rota
        # that silently never completes is not a small thing.
        "link_checked_at": p.link_checked_at,
        "link_gone_count": p.link_gone_count,
        "link_last_result": p.link_last_result,
        # The day we first saw it. Carried, never refreshed — a house re-listed
        # into this week's batch has been on the market since the week it first
        # appeared, not since Monday. Lose this and every carried listing looks
        # brand new and days-to-sell is always about seven.
...
def _prune_old_batches(db: Session, batch_type: str, region: str, *, keep_last: int) -> int:
    ...
    THE LIVE BATCH IS NEVER PRUNED, whatever its age or position. The window is
    the newest N by upload date, and "newest" and "live" are not the same thing:
    a batch goes live when an operator presses Publish, and staged batches keep
    arriving in the meantime. Twelve uploads reviewed and not published would
    have pushed the batch the site is actually serving out of the window and
    deleted it — every listing on the site, gone, from a routine tidy-up at the
    end of an upload that changed nothing a customer could see.

    Rows here ARE deleted, unlike archiving, which only hides. That is what
    makes this the one place in the ingest that can lose data for good, and why
    the live batch is pinned rather than merely likely to be recent.
    """
    if keep_last <= 0:
        return 0
    keep_ids = [
...
def _to_dt(v):
    """A timestamp that has been through a DataFrame and back.

    The carry round-trips stored rows through pandas, which turns a datetime
    into a Timestamp and a None into NaT — and NaT is not None, so it reaches
    the column as a value and the write fails. Anything unreadable becomes None,
    which for the fields this is used on means "not checked yet": the safe
...
def _explode_sale_history(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    ...
    for _, row in df.iterrows():
        ...
        if len(history) <= 1:
            rows.append(base)
            continue
        # The sale the top-level columns describe. One of the history entries
        # is that same sale, and it is the only one the campaign fields below
        # belong to.
        current = canonical_sale_date(base.get("sold_listing_date"))
        for sale in history:
            ...
            # WHAT BELONGS TO THE CAMPAIGN, NOT TO THE SALE.
            #
            # Everything else on this row is the property as it is TODAY, which
            # the docstring above accepts for beds and floor area. These four
            # are different: they describe the marketing of ONE sale, and
            # copying them onto a sale from 1998 states that the 1998 campaign
            # ran for however many days the 2026 one has. That is not a missing
            # number, it is a wrong one, and a wrong number is worse — it is
            # believed, and it is what "how long do they take to sell" would
            # have averaged.
            #
            # Blanked rather than guessed. A sale-history entry carries a price
            # and a date and nothing else, so a days-to-sell figure for it does
            # not exist and must not appear to.
            #
            # Except on the one entry that IS the top-level sale. Those columns
            # were measured, and blanking them along with the rest would throw
            # away the only real campaign in the row to avoid fabricating the
            # others.
            if not (current and canonical_sale_date(when) == current):
                for campaign_field in ("days_on_market", "listing_date",
                                       "sale_method", "listing_published_date"):
                    r[campaign_field] = None
            rows.append(r)
            added += 1
...
def as_utc(v):
    """A datetime that can safely be compared with any other datetime.

    SQLite stores no timezone at all, so a value written as UTC-aware reads
    back naive; Postgres returns it aware. Both are in use — one under the
    tests, one in production — and comparing one kind against the other raises
    TypeError. That is the shape of fault that keeps a suite green while the
    live site fails, so the normalising happens here rather than at each of the
    places that compare.
    """
    if v is None:
        return None
...
def _house_keys(slug_id, address, suburb) -> list[str]:
    """Every way this row can be recognised as the same house next week.

    Both, not one. The slug is the portal's own identifier and is the better
    key while it lasts; it does not always last, because a listing re-posted
    under a new campaign comes back with a new slug and the same front door.
    Matching on either means a re-post is still the house we have been watching
    since March rather than a brand-new listing.
    """
    keys = []
    slug = str(slug_id or "").strip()
    if slug:
        keys.append(f"slug:{slug}")
    addr = address_key(address, suburb)
    if addr:
        keys.append(f"addr:{addr}")
    return keys


def _stamp_first_seen(db: Session, batch, region: str) -> int:
    """Carry the first-seen date onto houses we have seen before.

    THE ROW WRITTEN THIS WEEK IS A NEW ROW. That is the trap. Carry-forward
    copies the whole record for houses MISSING from the new file, and those
    keep their dates — but a house still being advertised arrives in the new
    file, is written fresh, and knows nothing about the weeks before it.

    Left there, every listing still on the market resets to today on every
    load, and "how long do houses take to sell" answers seven days for
    everyone, forever. It would not look like a fault. It would look like the
    fastest market in the country.

    So the earliest date any previous batch holds for this house wins. Earliest
    rather than latest, because the question is when it first appeared.
    """
    db.flush()
    rows = (db.query(PropertyForSale)
    ...
    if not rows:
        return 0
    prior = [b.id for b in db.query(ImportBatch.id).filter(
        ImportBatch.batch_type == BatchType.FOR_SALE.value,
        ImportBatch.region == region,
        ImportBatch.id != batch.id,
        # Batches customers actually saw. A staged batch that was later
        # discarded never described a real week on the market.
    ...
    for slug, address, suburb, seen in db.query(
            PropertyForSale.slug_id, PropertyForSale.address,
            PropertyForSale.suburb, PropertyForSale.first_seen_at).filter(
            PropertyForSale.import_batch_id.in_(prior),
            PropertyForSale.first_seen_at.isnot(None)).all():
        seen = as_utc(seen)
        for key in _house_keys(slug, address, suburb):
    ...
    for r in rows:
        was = None
        for key in _house_keys(r.slug_id, r.address, r.suburb):
            v = earliest.get(key)
            if v is not None and (was is None or v < was):
                was = v
        have = as_utc(r.first_seen_at)
        if was is not None and (have is None or was < have):
            r.first_seen_at = was
            stamped += 1
    return stamped
...
def ingest_for_sale(
    ...
    *,
    ...
    # One timestamp for the whole load, so every listing that first appears in
    # this upload shares a first-seen date rather than drifting by the seconds
    # the pricing run takes.
    ...
    #
    # THIS RUNS WHETHER OR NOT THE BATCH GOES STRAIGHT LIVE, and it did not use
    # to. It sat behind `if publish:`, which was true back when an upload went
    # live the moment it finished — and then the audit screen was added, every
    # real upload started arriving as publish=False, and the carry became dead
    # code on the only path anybody uses. Nothing said so: the batch loaded, the
    # review screen showed exactly the rows the file contained, and the loss only
    # appeared when Publish archived the batch holding the other 8,609 listings.
    #
    # A 672-row file replacing a 9,281-row book is not a market that shrank by
    # 93% overnight, it is a scrape that caught less this time. A listing leaves
    # the site when its ADVERTISEMENT is gone — the daily link check decides
    # that, by opening it — and never because the newest spreadsheet was short.
    carried = carry_forward(db, region, for_sale_df)
    if len(carried):
        for_sale_df = pd.concat([for_sale_df, carried], ignore_index=True)
    # Archiving stays behind `publish`. On the staged path the live batch has to
    # keep serving the site until somebody presses Publish, and publish_release
    # archives it then.
    if publish:
    ...
    for _, row in enriched.iterrows():
        ...
        rec = PropertyForSale(
            **payload,
            ...
            # Blank for a listing arriving from a file, carried for one arriving
            # from last week's batch. See _row_to_scrape: without these the
            # delisting sweep is handed a book it has apparently never checked,
            # every upload, and its rota never gets past the first few hundred.
            link_checked_at=_to_dt(row.get("link_checked_at")),
            link_gone_count=_to_int(row.get("link_gone_count")),
            link_last_result=_to_str(row.get("link_last_result")),
            # Carried if this house has been seen before, stamped now if not.
            first_seen_at=_to_dt(row.get("first_seen_at")) or _now,
        )
        db.add(rec); inserted += 1
        if inserted % 500 == 0:
            db.flush()
    ...
    # And the date each house first appeared, before retention can delete the
    # batch that holds it. Ordering matters: pruning runs below, and a first-seen
    # date read after its only surviving copy is gone is no date at all.
    _carried_seen = _stamp_first_seen(db, batch, region)
    if _carried_seen:
        print(f"  [ingest] {_carried_seen:,} listing(s) have been advertised "
              f"since before this load — their first-seen date is kept")
    ...
    from pricing.audit import audit_for_sale_batch, serialise
    try:

...
def _matching_query(db: Session, batch_id: int, p: dict, *, apply_goals: bool = True):
    ...
    if p["min_price"] is not None:
        q = q.filter(PropertyForSale.asking_price >= p["min_price"])
    if p["max_price"] is not None:
        q = q.filter(PropertyForSale.asking_price <= p["max_price"])
    ...
    if apply_goals:
        ...
        if "subdividable" in goals:
            clauses.append(PropertyForSale.is_subdividable.is_(True))
...
def preview(
    ...
    subdividable = q.filter(PropertyForSale.is_subdividable.is_(True)).count()

...
def test_sold_and_new_listing_sweeps_are_daily():
    """Both feeds refresh daily under the direct-collector scope."""
    from worker import build_jobs

    by = {j.name: j.every for j in build_jobs()}
    assert by["sold sweep"] == by["new listings sweep"] == 86400
